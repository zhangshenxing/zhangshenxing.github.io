...
\begin{enumerate}
\item 如果 $f(x)>0$ 且 $\displaystyle\lim_{x\to\infty}f(x)=0$, 则 $\displaystyle\lim_{x\to\infty}\bigl[1+f(x)\bigr]^{1/f(x)}=$\fillblankframe{}.
\item 设 $y=\sin(x^2+1)$, 则 $\diff y=$\fillblankframe{}.
\item 极限 $\displaystyle\lim_{n\to\infty}\left(\frac1{n^2-1}+\frac2{n^2-2}+\cdots+\frac n{n^2-n}\right)=$\fillblankframe{}.
\item 曲线 $y=2\ln(x+1)$ 在点 $(1,2\ln2)$ 处的切线方程为\fillblankframe{}.
\item 若 $e^{y-1}=1+xy$, 则 $\dfrac{\diff y}{\diff x}\bigg|_{x=0}=$\fillblankframe{}.
\item 如果函数 $f(x)$ 的定义域是 $(0,+\infty)$, 且 $x=0$ 是曲线 $y=f(x)$ 的垂直渐近线, 那么 $\displaystyle\lim_{x\to0^+}\frac1{f(x)}=$\fillblankframe{}.
\end{enumerate}
...
\textbf{1.} \fillblankframe{$e$}, 
\textbf{2.} \fillblankframe{$2x\cos(x^2+1)\diff x$}, 
\textbf{3.} \fillblankframe{$1/2$}, \\
\textbf{4.} \fillblankframe{$y=x-1+2\ln 2$},
\textbf{5.} \fillblankframe{$1$},
\textbf{6.} \fillblankframe{$0$}.
...
\KaoShiXingShi{闭卷}
\ZhuanYeBanJi{少数民族预科班}
\KaoShiRiQi{2022年7月15日10:00-12:00}
\MingTiJiaoShi{集体}
\maketitle
...
\begin{enumerate}
\item 当 $x\to\infty$ 时, $\dfrac1x$ 和(~~~~)是等价无穷小.
\xx{$\tan\dfrac1x$}{$\tan x$}{$e^x$}{$e^{1/x}$}
\item 若当 $x\to0$ 时, $\tan(e^x-1)\cdot\sin x$ 和 $x^n$ 是同阶无穷小, 则 $n=$(~~~~).
\xx{$0$}{$1$}{$2$}{$3$}
\item 设 $f(x)=\arctan\dfrac1{x^2}$, 则 $x=0$ 是 $f(x)$ 的(~~~~).
\xx{可去间断点}{跳跃间断点}{第二类间断点}{连续点}
...
\draw[very thick,smooth,domain=-1.5:1.5] plot (\x, {\x*\x-1});
\draw
	(-0.3,-0.3) node {$O$}
	(1.8,-0.3) node {$x$}
	(-0.3,1.8) node {$y$};
\end{tikzpicture}
\end{center}
\item 设函数 $f(x)$ 在点 $x=0$ 处可导, 且 $f(0)=0$, 则 $\displaystyle\lim_{x\to0}\frac{f(x^2)-xf(x)}{x^2}=$(~~~~).
\xx{$0$}{$f'(0)$}{$2f'(0)$}{$-f'(0)$}
\item 如果点 $(x_0,y_0)$ 是曲线 $y=f(x)$ 的极值点, 则 $f'(x_0)=$(~~~~).
\xx{$0$}{$\infty$}{不存在}{$0$ 或不存在}
\end{enumerate}
...
\textbf{3.} \fillblankframe{$1/2$},\\
\textbf{4.} \fillblankframe{$y=x+1$},
\textbf{5.} \fillblankframe{$1/2$},
\textbf{6.} \fillblankframe{$y=x$}.
...
因此最大值为 $2$, 最小值为 $-3$. \score2

\textbf{6. （8分）【证明】}

\textbf{证法一}: 设 $f(x)=e^x-ex$, 则 \score2
\[f'(x)=e^x-e\ge0. \score2\]
因此 $f(x)$ 在 $(1,+\infty)$ 上单调递增, 从而 \score2
\[f(x_2)\ge f(x_1),\quad e^{x_2}-ex_2\ge e^{x_1}-ex_1. \score2\]

\textbf{证法二}: 设 $f(x)=e^x$, 则 $f(x)$ 在 $[x_1,x_2]$ 上连续, $(x_1,x_2)$ 内可导. \score2

\indent 由拉格朗日中值定理, 存在 $\xi\in(x_1,x_2)$ 使得
\[\frac{f(x_2)-f(x_1)}{x_2-x_1}=f'(\xi), \score2\]
即
\[\frac{e^{x_2}-e^{x_1}}{x_2-x_1}=e^\xi\ge e. \score2\]
所以 $e^{x_2}-e^{x_1}\ge e(x_2-x_1)$. \score2

\newpage
\textbf{7. （8分）【证明】}

\indent 设 $F(x)=x^2f(x)$, \score2\\
则 $F(x)$ 在 $[0,1]$ 上连续, $(0,1)$ 内可导, \score1\\
且 $F(0)=0,F(1)=f(1)=0$. \score1

\indent 由罗尔中值定理, 存在 $\xi\in(0,1)$ 使得 $F'(\xi)=0$. \score2\\
由于 $F'(x)=x^2f'(x)+2xf(x)$ 且 $\xi\neq0$, 所以 \score1\\
$\xi f'(\xi)+2f(\xi)=1$. \score1

\textbf{8. （8分）【解】}

(1) 
\[f'(x)=3x^2-6x=3x(x-2). \score1\]
当 $0<x<2$ 时, $f'(x)<0$. 当 $x>2$ 或 $x<0$ 时, $f'(x)>0$. \score1\\
因此 $[0,2]$ 是 $f(x)$ 的单减区间,\\
$(-\infty,0]$ 和 $[2,+\infty)$ 是 $f(x)$ 的单增区间. \Score{(1分, 写成开区间不扣分)}\\
所以 $f(x)$ 的极小值为 $f(2)=1$, 极大值为 $f(0)=5$. \score1

(2) 
\[f''(x)=6x-6=6(x-1). \score1\]
当 $x>1$ 时, $f''(x)>0$. 当 $x<1$ 时, $f''(x)<0$. \score1\\
因此 $[1,+\infty)$ 是曲线 $y=f(x)$ 的凹区间,\\
$(-\infty,1]$ 是曲线 $y=f(x)$ 的凸区间, \Score{(1分, 写成开区间不扣分)}\\
拐点为 $(1,3)$. \score1
