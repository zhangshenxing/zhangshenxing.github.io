...
设 $F(x)=x^{2022}f(x)$, \score2\\
则 $F(x)$ 在 $[0,1]$ 上连续, $(0,1)$ 内可导, \score1\\
且 $F(0)=0,F(1)=f(1)=0$. \score1\\
由罗尔中值定理, 存在 $\xi\in(0,1)$ 使得 $F'(\xi)=0$. \score2\\
...
\tigan{一、填空题（每题3分，共18分）}
\begin{enumerate}
\item $\displaystyle\lim_{x\to0}\bigl(1+x^2\bigr)^{1/x^2}=$\fillblank{}.
\item 设 $y=\cos(2x+1)$, 则 $\diff y=$\fillblank{}.
\item 极限 $\displaystyle\lim_{n\to\infty}\left(\frac1{n^2+1}+\frac2{n^2+2}+\cdots+\frac n{n^2+n}\right)=$\fillblank{}.
\item 曲线 $y=e^x$ 在点 $(0,1)$ 处的切线方程为\fillblank{}.
\item 若 $x-y+1=e^y$, 则 $\dfrac{\diff y}{\diff x}\bigg|_{x=0}=$\fillblank{}.
\item 曲线 $y=x+\dfrac1x$ 的斜渐近线是\fillblank{}.
\end{enumerate}
...
\textbf{1.} \fillblank{$e$},
\textbf{2.} \fillblank{$-2\sin(2x+1)\diff x$},
\textbf{3.} \fillblank{$1/2$},\\
\textbf{4.} \fillblank{$y=x+1$},
\textbf{5.} \fillblank{$1/2$},
\textbf{6.} \fillblank{$y=x$}.
