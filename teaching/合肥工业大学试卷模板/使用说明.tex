...
\lstset{language=[LaTeX]TeX,
  ...
  morekeywords={\maketitle, \titlesep, \BiaoTi, \XueNian, \XueQi, \KeChengDaiMa, \KeChengMingCheng, \XueFen, \KeChengXingZhi, \KaoShiXingShi, \ZhuanYeBanJi, \KaoShiRiQi, \MingTiJiaoShi, \XiZhuRenQianMing, \tigan, \scorebox, \score, \Score, \sscore, \lscore, \Lscore, \fillblank, \xx, \notice, \xuanzeti, \yihang, \erhang, \sihang, XeLaTeX},
  keywordstyle=\color{winered},
  morekeywords=[2]{hfutexam, shijuan, datizhi, cankaodaan, simple, nofangzheng, flalign, 5cm},
...
\lstinline|\score{数值}| 用于在参考答案一行结尾处生成得分点的虚线.\score2
\lstinline|\Score{(2分, 缺少常数得1分)}| 自定义得分内容.\Score{(2分, 缺少常数得1分)}
...
\begin{flalign*}
&&aaaaaaa&=bbbbbbb&\Score[3cm]{(2分, 缺少常数得1分)}\\		% 指定长度
&&aaaa&=bbbb&\sscore3\\		% 预设 2cm, 且只需要输入分数
&&aaaaaa&=bbbbbb&\score4\\		% 预设 4cm, 且只需要输入分数
&&aaaaa&=bbbbb&\lscore5		% 预设 8cm, 且只需要输入分数
\end{flalign*}
