...
\vspace{\baselineskip}
\textbf{1.} \fillblank{8em}{$e$}, 
\textbf{2.} \fillblank{8em}{$2x\cos(x^2+1)\diff x$}, 
\textbf{3.} \fillblank{8em}{$1/2$}, 

\vspace{\baselineskip}
\textbf{4.} \fillblank{8em}{$y=x-1+2\ln 2$},
\textbf{5.} \fillblank{8em}{$1$},
\textbf{6.} \fillblank{8em}{$0$}.
...
% 得分点根据长度不同有4条命令 \sscore(0.5倍) \score(1倍) \lscore(2倍) \Lscore(3倍)
% 行间公式需要使用 flalign* 环境, 每行有偶数个 &, 开头总有两个&&
\textbf{1. （8分）【解】}
...
\end{flalign*}
可得驻点 $x=-\dfrac13,1$.\Lscore2

\indent 由于
\begin{flalign*}
&&f(-2)=-10,\quad f(2)=2,\quad f\left(-\frac13\right)=\frac5{27},\quad f(1)=-1,&&\score2
\end{flalign*}
因此最大值为 $2$, 最小值为 $-10$.\lscore2

\textbf{6. （8分）【证明】}

\textbf{证法一}: 设 $f(x)=\tan x-x$, 则\lscore2
\begin{flalign*}
&&f'(x)=\frac1{\cos^2x}-1=\tan^2x\ge0.&&\score2
\end{flalign*}
因此 $f(x)$ 在 $\left(-\dfrac\pi2,\dfrac\pi2\right)$ 上单调递增, 从而\lscore2
\begin{flalign*}
&&f(x_2)\ge f(x_1),\quad\tan x_2-\tan x_1\ge x_2-x_1.&&\score2
\end{flalign*}

\textbf{证法二}: 设 $f(x)=\tan x$, 则 $f(x)$ 在 $[x_1,x_2]$ 上连续, $(x_1,x_2)$ 内可导.\sscore2
...
\end{flalign*}
所以 $\tan x_2-\tan x_1\ge x_2-x_1$.\lscore2
\newpage

\textbf{7. （8分）【证明】}

设 $F(x)=x^{2022}f(x)$,\lscore2\\
则 $F(x)$ 在 $[0,1]$ 上连续, $(0,1)$ 内可导,\lscore1\\
且 $F(0)=0,F(1)=f(1)=0$.\lscore1\\
由罗尔中值定理, 存在 $\xi\in(0,1)$ 使得 $F'(\xi)=0$.\score2\\
由于 $F'(x)=x^{2022}f'(x)+2022x^{2021}f(x)$ 且 $\xi\neq0$,\score1\\
所以 $\xi f'(\xi)+2022f(\xi)=1$.\lscore1
...
因此 $(0,2]$ 是 $f(x)$ 的单减区间, $[2,+\infty)$ 是 $f(x)$ 的单增区间.\score1\\
...
因此 $(0,2\sqrt3]$ 是曲线 $y=f(x)$ 的凹区间, $[2\sqrt3,+\infty)$ 是曲线 $y=f(x)$ 的凸区间,\sscore1\\
拐点为 $\left(2\sqrt3,\ln(2\sqrt3)+\dfrac16\right)$.\lscore1
