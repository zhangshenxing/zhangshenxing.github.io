...
\begin{frame}{柯西-古萨定理: 推导}
	...
	+\ii\oint_C(v\diff x+u\diff y).
	\]
	\onslide<+->
	由格林公式和C-R方程可知
	\[\oint_Cf(z)\diff z=-\iint_D(v_x+u_y)\diff x\diff y
	+\ii\iint_D(u_x-v_y)\diff x\diff y=0.
	\]
	\onslide<+->
	也可以从
	\[\oint_Cf(z)\diff z=-\iint_D\frac{\partial f}{\partial \ov z}\diff z\diff \ov z=2\ii\iint_D\frac{\partial f}{\partial \ov z}\diff x\diff y=0
	\]
	看出.
\end{frame}
...
\begin{frame}{例: 柯西-古萨定理计算积分}
	\onslide<+->
	\begin{example}
		求 $\displaystyle\oint_C\frac1{z(z^2+1)}\diff z$, 其中 $C:|z-\ii |=\dfrac12$.
	\end{example}
...
\begin{frame}{例: 复合闭路定理的应用}
	...
	\begin{theorem@}
		当 $a$ 在 $C$ 的内部时,
	\[
		\oint_C\frac{\diff z}{(z-a)^{n+1}}=\begin{cases}2\pi\ii,&n=0;\\0,&n\neq 0.\end{cases}
	\]
	\end{theorem@}
\end{frame}
...
\begin{frame}{例: 复合闭路定理的应用}
	\onslide<+->
	\begin{example}
		求 $\displaystyle\int_\Gamma\frac{\ee^z}z\diff z$, 其中 $\Gamma=C_1+C_2^-$, $C_1:|z|=2, C_2:|z|=1$.
	...
	\begin{solution}
		...
		\onslide<+->{由复合闭路定理可知 $\displaystyle\int_\Gamma\frac{\ee^z}z\diff z=0$.}
	\end{solution}
\end{frame}
