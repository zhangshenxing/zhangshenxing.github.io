...
\begin{frame}{典型例题: 利用原函数求积分}
	...
	\end{solution}
	\onslide<+->
	因此之前的例子中 $\displaystyle\int_0^{3+4\ii}z\diff z=-\frac72+12\ii$, 无论从 $0$ 到 $3+4\ii$ 的路径如何.
\end{frame}
...
\begin{frame}{典型例题: 利用原函数求积分}
	\onslide<+->
	\begin{example}
		求 $\displaystyle\int_0^{\pi\ii}z\cos z^2\diff z$.
	\end{example}
...
\begin{frame}{典型例题: 利用原函数求积分}
	...
	\]
			...
			\[
				...
				\visible<+->{=i\sin i+\cos i-1=\ee^{-1}-1.}
	\]
		}\vspace{-.5\baselineskip}
	\end{solution}
	\onslide<+->
...
\begin{frame}{典型例题: 利用原函数求积分}
	\onslide<+->
	\begin{example}
		求 $\displaystyle\int_1^{1+\ii} z \ee^z\diff z$.
	\end{example}
	\onslide<+->
	\begin{solution}
		由于 $f(z)=z\ee^z$ 处处解析,
		\onslide<+->{%
			且 $\displaystyle\int z \ee^z\diff z=\int z\diff \ee^z=z\ee^z-\int \ee^z\diff z=(z-1)\ee^z+C$,
		}\onslide<+->{%
			因此 $\displaystyle \int_1^{1+\ii} z \ee^z\diff z=(z-1)\ee^z\big|_1^{1+\ii}
			\visible<+->{=i\ee^{1+\ii}=e(-\sin 1+\ii\cos 1)}$.
		}
	\end{solution}
	\onslide<+->
	\begin{exercise}
...
\begin{frame}{典型例题: 利用原函数求积分}
	...
	\onslide<+->
	\begin{example}
		设 $C$ 为沿着 $|z|=1$ 从 $1$ 到 $\ii$ 的逆时针圆弧, 求 $\displaystyle\int_C\frac{\ln(z+1)}{z+1}\diff z$.
	\end{example}
