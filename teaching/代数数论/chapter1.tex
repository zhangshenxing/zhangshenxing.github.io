...
代数\nounen{数域}{number field}指的是有理数域 $\BQ$ 的有限扩张. 正如整数环 $\BZ$ 的算术性质对于 $\BQ$ 的重要性, 本章中我们将对数域的整数环 $\CO_K$ 进行研究, 这包括 $\CO_K$ 的线性结构、唯一分解性、单位群的结构等内容, 并利用其回答整二元二次型表整数问题.

设 $\BF_q$ 为 $q$ 元有限域, $t$ 为一未定元. 我们称 $\BF_q[t]$ 的有限扩张为\nounen{函数域}{function field}.
...
对于 $\alpha\in L$, 映射 $T_\alpha:x\mapsto \alpha x$ 给出了 $K$ 线性空间 $L$ 到自身的线性变换.
由于迹和行列式在线性代数中的重要地位, 我们给出如下定义.

\begin{definition}{迹和范数}{trace and norm}
分别称线性映射 $T_\alpha$ 的迹和行列式为 $\alpha$ (在扩张 $L/K$ 下)的\nounen{迹}{trace}和\nounen{范数}{norm}\footnote{注意这不是泛函分析中的范数.}, 记为 $\Tr_{L/K}(\alpha)$\index{Tr L K@$\Tr_{L/K}$} 和 $\bfN_{L/K}(\alpha)$\index{N L K@$\bfN_{L/K}$}.
\end{definition}
...
定义 $\alpha_1,\dots,\alpha_n\in L$ 关于 $L/K$ 的\nounen{判别式}{discriminant}\index{disc@$\disc$}为
...
设 $A\subseteq B$ 是两个含幺交换环. 如果 $b\in B$ 被一个 $A$ 系数首一多项式零化, 称 $b$ 在 $A$ 上\nounen{整}{integral}. 这样的元素全体称为 $A$ 在 $B$ 中的\nounen{整闭包}{integral closure}.
\end{definition}
...
数域 $K$ 的\nounen{整数环}{ring of integers} $\CO_K$\index{OK@$\CO_K$} 指的是 $\BZ$ 在 $K$ 中的整闭包, 其中的元素被称为\nounen{代数整数}{algebraic integers}.
\end{definition}
换言之, 代数整数是整系数多项式的根.
...
$\fa$ 的\nounen{判别式}{discriminant} $\Delta_\fa\in\BZ$\index{Delta a@$\Delta_\fa$} 是指它的任意一组整基的判别式. 特别地, 如果 $\fa=\CO_K$, 我们也称它的整基为 $K$ 的整基, 它的判别式为 $K$ 的判别式 $\Delta_K$\index{Delta K@$\Delta_K$}.
\end{definition}
...
关于 $p$ 的\nounen{艾森斯坦多项式}{Eisenstein polynomial} 指的是首一多项式 $f(T)=T^n+a_{n-1}T^{n-1}+\dots+a_0 \in\BZ[T]$ 满足 $p\mid a_{n-1},\dots,a_0$ 且 $p^2\nmid a_0$. 通过分解 $f(T)\bmod p$ 不难看出艾森斯坦多项式总是不可约的.
...
\begin{enumerate}
	\item 称 $\sigma$ 为\nounen{无穷素位}{infinity place}.
	如果 $\sigma(K)\subseteq \BR$, 即  $\ov\sigma=\sigma$, 称 $\sigma$ 为\nounen{实嵌入}{real embedding}或\nounen{实素位}{real place}, 否则称之为\nounen{复嵌入}{complex embedding}或\nounen{复素位}{complex place}. 我们视一对复嵌入为同一个复素位.
	\item 如果 $K$ 没有复素位, 称 $K$ 为\nounen{全实域}{totally real field}; 如果 $K$ 没有实素位, 称 $K$ 为\nounen{全虚域}{totally imaginary field}.
\end{enumerate}
\end{definition}
...
\subsection{分圆域的整数环}
设 $N\ge 3, \zeta_N\in\BC$ 是 $N$ 次\cnen{本原}{primitive}单位根, 即 $\zeta_N=e^{2c\pi i/N}, \gcd(c,N)=1$.
...
\begin{theorem}{}{Dedekind domain admits unique prime ideal decomposition}
...
\begin{definition}{分式理想}{fractional ideal}
	设 $\CO$ 是戴德金整环. 对于 $K=\Fr \CO$ 的非零子集 $\fa$, 如果存在 $\CO$ 中的非零元 $c$ 使得 $c\fa$ 为 $\CO$ 的理想, 则称 $\fa$ 为 $\CO$ 的一个\nounen{分式理想}{fractional ideal}. 换言之, 分式理想是 $K$ 的有限生成非零 $\CO$ 子模.
\end{definition}
...
我们将形如 $(\alpha)=\alpha\CO_K$, $\alpha\in K^\times$ 的分式理想称为\nounen{主分式理想}{principal fractional ideal}, 记 $\CP_K$\index{P K@$\CP_K$} 为主分式理想全体构成的群.
我们有群的正合列\footnote{见定义~\ref{def:exact}.}
	\[\xymatrix{
	1\ar[r] &\CO_K^\times\ar[r] &K^\times \ar[rr]^{\alpha\mapsto (\alpha)} &&\CI_K\ar[r] &\Cl_K\ar[r] &1
	},\]
其中 $\CO_K^\times$\index{O K times@$\CO_K^\times$} 为 $K$ 的\nounen{单位群}{group of units}, 即 $\CO_K$ 中全体单位, $\Cl_K=\CI_K/\CP_K$\index{ClK@$\Cl_K$} 为 $K$ 的理想\nounen{类群}{class group}. 
可以看出, 单位群和类群描述的是``数和理想的差异'', 特别地, 类群表达了``素元分解成立的程度''.

记 $\mu_K$\index{mu K@$\mu_K$} 为 $K$ 中单位根全体.
...
类群的大小被称为\nounen{类数}{class number} $h_K$. 类数为 $1$ 即指 $\CO_K$ 为主理想整环. 对于虚二次域 $\BQ(\sqrt{d})$, $d<0$, 高斯猜想并由贝克 \cite[I]{Baker1966} 和 Stark \cite{Stark1967} 证明它的类数等于 $1$ 当且仅当
