...
\[|\bfN_{K/\BQ}(a)|\le \left(\frac{2}{\pi}\right)^{r_2}\sqrt{|\Delta_K|}[\CO_K:\fa].\]
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.4.11.}
$\bfN((a))=|\bfN_{K/\BQ}(a)|,\forall a\in K^\times$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.4.12.}
证明 $\fa$ 所有元素的范数生成的 $\BZ$ 的理想为 $\bfN\fa \BZ$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.4.13.}
证明对称凸集
\[X=\set{(z_\tau)\in K_\BR\mid \suml_\tau |z_\tau|<t}\]
的体积为 $\vol(X)=2^{r_1}\pi^{r_2} \frac{t^n}{n!}$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.4.19.}
计算 $K=\BQ(\sqrt{d}),d=-1,-2,-3,-5,-7,2,3,5$ 的类数.
\begin{proof}[解答]
%在此解答
\end{proof}

\noindent{\bf 习题~1.4.20.}
证明 $K\ne \BQ$ 时, $|\Delta_K|\neq 1$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.4.21.}
证明当数域 $K$ 的次数趋于无穷时, $|\Delta_K|$ 趋于无穷.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.4.23.}
在相差一个单位的前提下, $\bfN_{K/\BQ}(\alpha)=a$ 的 $\alpha$ 只有有限多个.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.4.28.}
虚二次域的单位群是什么?
\begin{proof}[解答]
%在此解答
\end{proof}

\noindent{\bf 习题~1.4.29.}
