...
\begin{center}
姓名：张三 \qquad \qquad 学号：BA00000000
\end{center}
\vspace{0.3cm}

\noindent{\bf 习题~1.1.1.}
证明 $\bfN_{L/K}(\alpha\beta)=\bfN_{L/K}(\alpha)\bfN_{L/K}(\beta),
\Tr_{L/K}(\alpha+\beta)=\Tr_{L/K}(\alpha)+\Tr_{L/K}(\beta)$.
\begin{proof}
%在此证明
%\[a^2+b^2=c^2\ \text{行间公式}\]
%$a^2+b^2=c^2$
\end{proof}

\noindent{\bf 习题~1.1.2.}
对于 $\alpha\in L,a\in K$, 证明 $\bfN_{L/K}(a\alpha)=a^n\bfN_{L/K}(\alpha),\Tr_{L/K}(a\alpha)=a\Tr_{L/K}(\alpha)$, 其中 $n=[L:K]=\dim_K L$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.1.9.}
举一个不可分扩张的例子, 使得双线性型 $(x,y)\mapsto\Tr_{L/K}(xy)$ 退化.
\begin{proof}[解答]
%在此解答
\end{proof}

\noindent{\bf 习题~1.2.2.}
如果 $b\in B$ 在 $A$ 上代数, 即被一个 $A$ 系数非零多项式零化, 则 $b$ 在 $A$ 上整当且仅当其首一极小多项式($\in\Fr A[x]$)是 $A$ 系数的. 因此整闭包是整闭的.
\begin{proof}
%在此证明
\end{proof}
...
\noindent{\bf 习题~1.2.7.}
利用此法计算 $\sqrt[3]{2}+\sqrt{5}$ 的任一零化多项式.
\begin{proof}[解答]
%在此解答
\end{proof}

\noindent{\bf 习题~1.2.8.}
证明整闭包构成一个环. 如果 $B$ 在 $A$ 上代数, 则 $B$ 的分式域等于该整闭包的分式域.
\begin{proof}
%在此证明
\end{proof}
...
\noindent{\bf 习题~1.2.17.}
设 $d\neq 0,1$ 是无平方因子整数. 当 $d\equiv 1\mod 4$ 时 $\Delta_{\BQ(\sqrt{d})}=d$; 当 $d\equiv 2,3\mod 4$ 时 $\Delta_{\BQ(\sqrt{d})}=4d$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.2.23.}
证明 $1,\theta,\half(\theta+\theta^2)$ 是数域 $\BQ(\theta)$ 的一组整基, 其中 $\theta^3-\theta-4=0$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.2.24.} (选做)
证明 $\Delta_K\equiv 0,1\mod 4$ (搜索 Stickelberger's discriminant relation).
\begin{proof}
%在此证明
\end{proof}
