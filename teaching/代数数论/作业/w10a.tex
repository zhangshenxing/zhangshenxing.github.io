...
\noindent{\bf 习题~3.2.12.}
(1) 验证 $\Ga(X,Y)=X+Y$ 是形式群.

(2) 验证 $\Gm(X,Y)=X+Y+XY$ 是形式群.

(3) 设 $f(X)=a_1 X+a_2X^2+\dots\in R[[X]]$, $a_1\in R^\times$, 则存在 $f^{-1}(X)\in R[[X]]$ 使得 $f(f^{-1}(X))=f^{-1}(f(X))=X$. 此时 $\CF(X,Y)=f^{-1}(f(X)+f(Y))$ 是形式群.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~3.2.13.}
设 $R$ 是 $\BQ$ 代数. 对于任意 $R$ 上形式群 $\CF$, 存在唯一的形式群同构 $\log_\CF:F\simto \Ga$, 使得 $\log_\CF(X)\equiv X\mod\deg 2$.
\begin{proof}
%在此证明
\end{proof}


\noindent{\bf 习题~3.2.14.}
