...
\noindent{\bf 习题~3.3.116.}
设 $i=0,-1$.

(1) 对于任意多个 $G$ 模 $A_k$, $\RH^i(G,\oplus_k A_k)=\prod_k \RH^i(G,A_k)$. 

(2) 对于任意多个 $G$ 模 $A_k$, $\RH^i(G,\prod_k A_k)\inj \prod_k \RH^i(G,A_k)$ 是单射.

(3) 对于 $G$ 模正向系 $A_k$, $\RH^i(G,\ilim_k A_k)=\ilim_k \RH^i(G,A_k)$.
\begin{proof}
%在此证明
\end{proof}
