...
\noindent{\bf 习题~2.3.10.}
如果完备度量空间中闭球套的半径趋于 $0$, 则闭球套的交非空.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~2.3.18.}
设 $K$ 是完备离散赋值域, $K^\ur$ 是它的极大非分歧扩张, $\kappa$ 是它的剩余域. 证明 $K^\ur/K$ 的所有子扩张和 $\kappa^\sep/\kappa$ 的所有子扩张一一对应.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~2.3.24.}
设 $L/K$ 是一个完全分歧且是温分歧扩张, $w|v$ 是 $L$ 和 $K$ 的赋值. 证明 $L/K$ 的所有子扩张和 $w(L^\times)/v(K^\times)$ 的所有子群一一对应.
\begin{proof}
%在此证明
\end{proof}
