...
\noindent{\bf 习题~1.2.28.}
研究下列域的无穷素位: 
\env{enumerate}{
\item 二次域 $\BQ(\sqrt{d})$, 其中 $d\neq 0,1$ 为无平方因子整数.
\item 分圆域 $\BQ(\zeta)$, 其中 $\zeta=e^{2\pi i/n}$, $n$ 为正整数.
\item 三次域 $\BQ(\gamma)$.
}
\begin{proof}[解答]
%在此解答
...
\noindent{\bf 习题~1.3.10.}
分式理想全体构成一个交换群 $\CJ_K$, 幺元为 $(1)=\CO_K$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.3.12.}
设 $d\neq 0,1$ 是平方自由的整数, $K=\BQ(\sqrt{d})$. 对于素数 $p\nmid 2d$, $p\CO_K$ 是素理想当且仅当 $x^2\equiv d\mod p$ 无解.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.3.13.}
自选 [Neu] 习题 1.3.4 至 1.3.10 的若干题.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~1.4.3.}
$\vol(\Gamma)=\sqrt{|\det\pair{v_i,v_j}|}$ 且不依赖于基的选取.
\begin{proof}
%在此证明
\end{proof}
