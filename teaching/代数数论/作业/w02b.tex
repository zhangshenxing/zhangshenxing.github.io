...
\noindent{\bf 习题~2.1.6.}
设 $K=\BQ$. 对于素数 $p$, 定义 $v_p(p^n\frac{a}{b})=n$, 其中 $a,b\in\BZ,p\nmid ab$. 则 $v_p$ 是 $\BQ$ 的一个加性赋值, $|\cdot|_p=p^{-v(\cdot)}$ 是 $\BQ$ 的一个乘性赋值.
\begin{proof}
%在此证明
\end{proof}
...
(1) 设 $K$ 为 $\BC$ 上的亚纯函数全体, $x\in \BC$, 定义 $\ord_x(f)$ 为 $f$ 在 $x$ 处的阶, $f\in K^\times$.

(2) 设 $k$ 为一个域, $k[[T]]$ 为形式幂级数环, 即
\[k[[T]]=\set{\sum_{n=0}^{+\infty} a_n T^n\mid a_n\in k}.\]
令 $K=k((T))=k[[T]][T^{-1}]$ 为其分式域. 定义 $v(\suml_{n=m}^{+\infty} a_nT^n)=m$, $a_m\neq 0$.

(3) 设 $K=\BQ(i)$, $\alpha=3^k\frac{a+bi}{c}$, 其中 $a,b,c\in\BZ,3\nmid c$ 且 $3\nmid a$ 或 $b$. 定义
\[v_3(\alpha)=\max\set{n\mid \alpha\in 3^n\BZ[i]}=k.\] 

(4) 设 $K=\BQ(i)$, $\alpha=2^k a+2^\ell bi$, 其中 $a,b$ 是奇数. 定义 
\[v_{1+i}(\alpha)=\max\set{n\mid \alpha\in(1+i)^n\BZ[i]}=\begin{cases}
2\min\set{k,\ell},\quad&\text{如果}\ k\neq\ell;\\
2k+1,&\text{如果}\ k=\ell.
\end{cases}\]
