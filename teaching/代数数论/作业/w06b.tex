...
\noindent{\bf 习题~2.4.39.}
设 $k$ 是一个域, 计算 $\Omega^1_{A/k}$, $A=k[x,y], k[x]/(x^2+1), k[x,y]/(xy-1), k(x).$
\begin{proof}[解]
%在此解答
\end{proof}

\noindent{\bf 习题~2.4.40.}
证明对于任意交换的 $A$ 代数 $A'$, $\Omega^1_{B\otimes_A A'/A'}=\Omega^1_{B/A}\otimes_A A'$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~2.4.42.}
如果 $S\subset A$ 是一个乘法集, 则 $\fd_{S^{-1}B/S^{-1}A}=S^{-1}\fd_{B/A}$.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~2.4.43.}
如果戴德金环 $R$ 只有有限多个素理想, 则 $R$ 是主理想整环.
\begin{proof}
%在此证明
\end{proof}

\noindent{\bf 习题~2.4.47.}
(1) 证明 $f(x)=x^3+x^2-2x+8\in\BQ[x]$ 是不可约多项式. 令 $\theta$ 是它的一个根, $K=\BQ(\theta)$.

(2)证明 $2$ 在 $K$ 中完全分解.

(3) 对每个 $\alpha\in\CO_K$, $\set{1,\alpha,\alpha^2}$ 是偶数, 因此它不可能是一组整基.
\begin{proof}
%在此证明
\end{proof}
