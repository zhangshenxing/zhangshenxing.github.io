...
\subsection{抽象伽罗瓦理论}
考虑射影有限群 $G$ 和它的闭子群 $\set{G_E}_{E\in X}$. 我们称 $E$ 是(抽象)\noun{域}, 存在域 $k$ 和 $\bar k$ 满足 $G_k=G,G_{\bar k}=1$. 

如果 $G_L\subseteq G_K$, 我们记 $K\subseteq L$ 或 $L/K$, 并称之为\noun{域扩张}. 如果 $G_L$ 是开子群, 即 $n=[G_K:G_L]$ 有限, 称之为 $n$ 次\noun{有限扩张}. 如果 $G_L\triangleleft G_K$, 称之为\noun{正规}或\noun{伽罗瓦扩张}, 并定义 $G(L/K)=G_K/G_L$. 容易知道 $G=\ilim G(K/k)$, 其中 $K/k$ 取遍所有有限伽罗瓦扩张. 类似地, 我们可以定义域的交、复合、共轭以及循环扩张、阿贝尔扩张等概念, 即对应的 $G_E$ 复合、交、共轭以及 $G(L/K)$ 是循环群、阿贝尔群等.
