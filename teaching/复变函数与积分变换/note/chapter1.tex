...
\begin{example}
  设 $z_1=10-5\ii,z_2=4+3\ii$, 求 $\ov{\Bigl(\ddfrac{z_1}{z_2}\Bigr)}$.
\end{example}
...
\begin{solution}
  ...
  \[
    ...
    =1-2\ii,
  \]
  故 $\ov{\Bigl(\ddfrac{z_1}{z_2}\Bigr)}=1+2\ii$.
\end{solution}
...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[cstdot]
      ...
      \fill[main] (B) circle node[above right] {$\arctan\ddfrac yx$};
      \fill[main] (C) circle node[right] {$\arctan\ddfrac yx$};
      \fill[second] (D) circle node[above left] {$\arctan\ddfrac yx+\cpi$};
      \fill[third] (E) circle node[below] {$\cpi$};
      \coordinate (F) at (-1.6,-.7);
      \fill[fifth] (F) circle node[left] {$\arctan\ddfrac yx-\cpi$};
      \fill[third] (G) circle node[left] {$\ddfrac\cpi2$};
      \fill[third] (H) circle node[right] {$-\ddfrac\cpi2$};
    \end{scope}
  \end{tikzpicture}
...
\begin{solution}
  ...
  \[
     \sqrt[4]{1-\ii}
    =\sqrt[8]2\exp\frac{\Bigl(-\ddfrac\cpi4+2k\cpi\Bigr)\ii}4,\quad k=0,1,2,3.
  \]
  于是该方根全部值为
  \[
...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}[scale=.5]
    \draw[cstcurve,cstnra,third] (-1.5,0)-- node[above] {$w=\ddfrac1z$} (1.5,0);
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{分式线性变换 $w=\ddfrac1z$\footnotemark}
\end{figure}
