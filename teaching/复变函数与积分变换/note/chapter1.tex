...
莱布尼兹 (Leibniz) 对此有如下评论: {\itshape 圣灵在分析的奇观中找到了超凡的显示, 这就是那个理想世界的端兆, 那个介于存在与不存在之间的两栖物, 那个我们称之为虚的 $-1$ 的平方根.}
...
我们将复平面的 $x,y$ 轴称为\emph{实轴}和\emph{虚轴}.
当 $\Im z=0$ 时, $z$ 是实数, 它落在实轴上.
称不是实数的复数是\emph{虚数}.
...
\begin{figure}[hbpt]
  \centering
  \begin{tikzpicture}
    \draw[cstaxis] (-3,0)--(5,0);
    \draw[cstaxis] (0,-1)--(0,3);
    \coordinate (O) at (0,0);
    \coordinate [label=above:\textcolor{main}{$z_1$}] (Z1) at (1,2);
    \coordinate [label=left:\textcolor{main}{$z_2$}] (Z2) at (-2.5,.5);
    \coordinate [label=above left:\textcolor{second}{$z_1+z_2$}] (P) at ($(Z1)+(Z2)$);
    \coordinate [label=right:\textcolor{third}{$z_1-z_2$}] (M) at ($(Z1)-(Z2)$);
