...
\begin{figure}[H]
  ...
  \begin{minipage}{.48\textwidth}
    \centering
    \begin{tikzpicture}
      \filldraw[cstcurve,cstfill1] (.8,0) circle (2.6 and 2);
      \coordinate (R) at (0,-.8);
...
% \begin{wrapfigure}{o}[1cm]{2.1cm}
%   \begin{center}
%     \includegraphics[height=2cm]{images/qr.png}\par
%     \footnotesize
%     \textcolor{fifth}{微视频}\par
%     辐角主值的计算
%   \end{center}
% \end{wrapfigure}

我们固定选择 $\Arg z$ 中位于区间 $(-\cpi,\cpi]$ 的那个, 并称之为\nouns{辐角主值}\index{fujiao@辐角!fujiaozhuzhi@辐角主值}\footnote{%
  ...
  \begin{minipage}{.51\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \begin{scope}[scale=\r]
        ...
        \coordinate (I1) at (0,.3);
        \coordinate (I2) at (0,-.4);
      \end{scope}
      \begin{scope}[cstdot]
        \fill[main] (A) circle node[above right] {$\arctan\dfrac yx$};
        \fill[second] (B) circle node[above left] {$\arctan\dfrac yx+\cpi$};
        \fill[fifth] (C) circle node[left] {$\arctan\dfrac yx-\cpi$};
        \fill[main] (D) circle node[right] {$\arctan\dfrac yx$};
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[shift={(5,0)}]
      ...
      \draw[decorate,decoration={brace,amplitude=5,aspect=.6},cstfill2] (O)--(Z12) node[pos=.6,below,sloped] {$\abs{z_1+z_2}$};
      ...
      \draw[decorate,decoration={brace,amplitude=5,aspect=.6},cstfill2] (O)--(Z123) node[pos=.6,below,sloped] {$\abs{z_1+z_2+z_3}$};
      \begin{scope}[cstcurve,cstra,fourth]
        \draw (O)--(Z1);
        \draw (Z1)--(Z12);
        \draw (Z12)--(Z123);
...
\begin{figure}[H]
  ...
  \end{minipage}
\end{figure}
\footnotetext{对于这种向外无限延伸的, 阴影部分描成圆形或方形是无关紧要的, 都表示同一个区域.}

\begin{exampleenum}
  \item 在\ref{fig:domain} 中, 阴影部分(不包含线条部分)中任意两点可用折线连接, 因此它是一个区域.
  这些线条割痕\footnote{\emph{割痕}\index{gehen@割痕}是指在区域内去掉的一条曲线.}和点构成了它的边界.
...
\begin{figure}[H]
  \begin{minipage}{.27\textwidth}
    ...
    \begin{tikzpicture}
      \def\t{{atan(1.5)}}
      \fill[cstfille1] (-1.8,-1.5) rectangle (1.8,1.5);
      \filldraw[cstcurve,main,domain=-\t:\t,smooth,fill=white] plot ({sec(\x)},{tan(\x)});
      \filldraw[cstcurve,main,domain=-\t:\t,smooth,fill=white] plot ({-sec(\x)},{tan(\x)});
      \draw[cstaxis] (-1.8,0)--(1.8,0);
      \draw[cstaxis] (0,-1.5)--(0,1.5);
    \end{tikzpicture}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[scale=.5]
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{分式线性映射 $w=\dfrac1z$}
\end{figure}

形如 $\dfrac{az+b}{cz+d}$ 的映射叫作\nouns{分式线性映射}\index{fenshixianxingyingshe@分式线性映射}, 它总把直线和圆映成直线或圆.
...
\begin{people}[images/Riemann.png]
