...
\begin{definition}
	\begin{enumpar}
		...
		\item 如果部分和数列 $\set{s_n}_{n\ge 1}$ 极限存在, 则称 $\suml_{n=1}^\infty z_n$ \noun{收敛}, 并记 $\suml_{n=1}^\infty z_n=\liml_{n\to\infty}s_n$ 为它的\noun{和}. 否则称该级数\noun{发散}.
	\end{enumpar}
\end{definition}
...
\begin{theorem}\label{thm:absolute-convergent}
	如果实数项级数
	\[
...
由正项级数的判别法可以得到如下结论: 设
\begin{enumpar}
	\item \noun{比值法}\footnote{又名\emph{达朗贝尔判别法}.}: $\lambda=\displaystyle\lim_{n\to\infty}\abs{\frac{z_{n+1}}{z_n}}$ (假设存在或为 $+\infty$), 或
	\item \noun{根式法}\footnote{又名\emph{柯西判别法}.}: $\lambda=\displaystyle\lim_{n\to\infty}\sqrt[n]{\abs{z_n}}$ (假设存在或为 $+\infty$),\footnote{
...
\begin{definition}
	\begin{enumpar}
		...
		\item 对于 $z_0\in D$, 如果级数 $\suml_{n=1}^\infty f_n(z_0)$ 收敛, 则称 \noun{$\suml_{n=1}^\infty f_n(z)$ 在 $z_0$ 处收敛}, 相应级数的值称为它的\noun{和}.
...
\begin{figure}[!hbt]
	...
	\begin{tikzpicture}
		...
		\draw[cstcurve,cstra] (-1.2,0)--(-2.4,0)
			node[left] {都有可能};
	\end{tikzpicture}
	\caption{幂级数的收敛范围}
\end{figure}
...
\begin{enumerate}
	\item \noun{比值法}\footnote{又名\emph{达朗贝尔公式}.}: $r=\displaystyle\lim_{n\to\infty}\abs{\frac{c_{n+1}}{c_n}}$ (假设存在或为 $+\infty$), 或
	\item \noun{根式法}\footnote{又名\emph{柯西公式}.}: $r=\displaystyle\lim_{n\to\infty}\sqrt[n]{|c_n|}$ (假设存在或为 $+\infty$).\footnote{一般情形下, 我们有\emph{柯西-阿达马公式}: $r=\displaystyle\lim_{k\to\infty}\max_{n\ge k}\sqrt[n]{|c_n|}$.}
\end{enumerate}
