...
\begin{center}
	...
	\begin{tabular}{cccc} 
		&\alert{发散}&\alert{条件收敛}&\alert{绝对收敛}\\
		\textcolor{main}{\emph{发散}}&发散&发散&发散\\ 
		\textcolor{main}{\emph{条件收敛}}&发散&条件收敛&条件收敛\\ 
		\textcolor{main}{\emph{绝对收敛}}&发散&条件收敛&绝对收敛
	\end{tabular}
	\begin{tikzpicture}[overlay,fourth]
		\draw[cstcurve] (-7.21,.81)--(-.12,.81);
		\draw[cstcurve] (-5.2,1.46)--(-5.2,-1.54);
		\draw[decorate,decoration={brace,amplitude=8},thick,main] (-6.4,-2.4)--(-6.4,-.4);
		\draw[decorate,decoration={brace,amplitude=8},thick,second] (-4.3,.3)--(0.7,.3);
		\draw (-1.8,.8) node[second] {实部级数}
			(-7,-1.4) node[align=center,main] {虚\\部\\级\\数};
	\end{tikzpicture}
\end{center}
...
\begin{proof}
	{\enumnum1 因为级数收敛, 所以 $\liml_{n\to\infty}c_n z_0^n=0$.故存在 $M$ 使得 $|c_nz_0^n|<M$.对于 $|z|<|z_0|$,
