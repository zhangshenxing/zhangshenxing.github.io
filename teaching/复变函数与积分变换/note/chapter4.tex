...
\begin{definition}
  \begin{enumpar}
    \item 设 $\{z_n\}_{n\ge1}$ 是复数列. 表达式 $\sumf1 z_n$ 称为复数项\noun{无穷级数}.\footnotemark
...
将上述结论中的 $\lambda$ 换成 $\lambda=\liml_{n\ra\infty}\sqrt[n]{\abs{z_n}}$ 也成立, 叫做\noun{根式审敛法}\footnote{
  又名\emph{柯西判别法}.
...
\end{definition}
\footnotetext{尽管 $z=a$ 时 $(a-z)^0$ 无意义, 但为了简便我们约定幂级数在 $z=a$ 时取值为 $c_0$.}
...
若将上述结论中的 $r$ 换成 $r=\liml_{n\ra\infty}\sqrt[n]{\abs{c_n}}$ 也成立, 叫做\noun{根式法}\footnote{
  又名\emph{柯西公式}.
...
设函数 $f(z)$ 在区域 $D$ 解析, $z_0\in D$ 到 $D$ 边界的距离为 $d$.
...
\begin{solution}
  ...
  \end{align*}
  显然 $f(z)$ 在 $z=1$ 处连续, 因此由\thmref{定理}{thm:abel-second} 可得
  \[
...
\begin{example}
  ...
  \begin{figure}[!hbt]
    \centering
    \begin{tikzpicture}
      \filldraw[cstcurve,fourth,cstfill4] (0,0) circle ({sqrt(2)});
      \filldraw[cstcurve,main,cstfill1] (0,0) circle (1);
      \draw[cstdash] (1,1)--(-3,1)
        node[below right] {$f=g$}
        node[above right] {$f\neq g$};
      \draw[cstra,main] (0,0)--node[above left] {$1$}({cos(35)},{sin(35)});
      \draw[cstra,fourth] (0,0)--({sqrt(2)*cos(-20)},{sqrt(2)*sin(-20)}) node[right] {$\sqrt2$};
      \draw[cstdash] (1,1)--(2.3,2.3);
      \fill[cstdot] (1,1) circle
        node[right] {$1+\ii $};
...
\begin{enumpar}
  ...
  \item 若 $R_1=R_2$, 则该双边幂级数只在圆周 $|z-z_0|=R_1$ 上可能有收敛的点.
    此时没有收敛域.
...
从而\alert{双边幂级数的和函数也是解析的, 且可以逐项求导、逐项积分}.
...
\begin{figure}[!htb]
  \centering
  \begin{tikzpicture}
    \filldraw[cstcurve,second,cstfill] (0,0) circle (1.6);
    ...
    \draw[cstra,second] (Z)--(Z1);
    \draw[cstra,main] (Z)--(Z2);
    \fill[cstdot,second] (Z1) circle;
    \fill[cstdot,main] (Z2) circle;
    \draw[second,cstra] (0,0)--({1.6*cos(35)},{1.6*sin(35)})
      node[right] {$K_2$}
      node[pos=.75,below] {$R$};
...
\begin{solution}\delspace
  \begin{enumnopar}[(i)]
    \item 设 $0<|z|<|\xi|$, 则
      ...
      \]
    \item 设 $|\xi|<|z|<|\eta|$. 根据\thmref{定理}{thm:rational-function-laurent}, $c_n$ 是 $a_n=\displaystyle	\frac{n+1}{\eta^{n+2}}-\frac1{\xi^{n+1}}$ 中 $|\lambda|\ge |\eta|$ 对应的项, 即 $c_n=\displaystyle	\frac{n+1}{\eta^{n+2}}$. 从而
      \[
         f(z)
...
由\thmref{定理}{thm:test-removable} 和\thmref{定理}{thm:test-pole} 可得:
\begin{theorem}
