...
\[
  ...
  =\liml_{\delt z\ra 0}\dfrac{\delt u+\ii\delt v}{\delt z}
\]
等于其沿着水平方向和竖直方向的极限
\[
  \liml_{\delt x\ra 0}\dfrac{\delt u+\ii\delt v}{\delt x}=u_x+\ii v_x,\qquad
  \liml_{\delt y\ra 0}\dfrac{\delt u+\ii\delt v}{\ii\delt y}=-\ii u_y+v_y,
\]
...
\begin{theorem}
  \begin{enumpar}
    ...
    \item 若 $u_x,u_y,v_x,v_y$ 在区域 $D$ 上处处连续, 且满足C-R方程, 则 $f(z)$ 在 $D$ 上处处可导, 从而解析.
  \end{enumpar}
\end{theorem}
...
故
\[
    \lim_{n\ra\infty} \Bigl(1+\dfrac zn\Bigr)^n
  =\ee^x(\cos y+\ii\sin y).
\]
...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[xshift=-4cm]
      ...
      \def\wd{.3}
      \foreach \i in {-4,-3,...,4}
        \draw[cstcurve,main] ({\wd*\i},-1.4)--({\wd*\i},1.4);
      \foreach \i in {-4,-3,...,4}
        \draw[cstcurve,second,cstra] (-1.4,{\wd*\i})--(1.6,{\wd*\i});
    \end{scope}
    \begin{scope}[xshift=4cm]
      \draw[cstaxis] (-2.5,0)--(2.5,0);
      \draw[cstaxis] (0,-2)--(0,2);
      \foreach \i in {-4,-3,...,4}
        \draw[cstcurve,main] (0,0) circle ({.3*pow(1.5,\i)});
      \foreach \i in {-4,-3,...,4}
        \draw[cstcurve,second,cstra] (0,0)--({2*cos(35*\i)},{-2*sin(35*\i)});
    \end{scope}
  \end{tikzpicture}
  \caption{指数函数 $w=\ee^z$}
\end{figure}
...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[xshift=4cm]
      \def\r{1.3}
      \fill[cstfille5,pattern color=fifth] (0,0)--({\r*cos(40)},{\r*sin(40)}) arc (40:-40:\r)--cycle;
...
设
\[
  z=\cos w=\dfrac{\ee^{\ii w}+\ee^{-\ii w}}2,\]
...
\begin{solution}
  由于
  \[
    \sin z=\dfrac{\ee^{\ii z}-\ee^{-\ii z}}{2\ii}=2,
  \]
  我们有
  \[
  ...
  \begin{enumnopar}[(i)]
    ...
    \begin{align*}
      a&=\lim_{z\ra1} (z-1)f(z)
        =\lim_{z\ra1} \dfrac{1}{(z-2)(z+2)}
        =-\frac13,\\
      b&=\lim_{z\ra2} (z-2)f(z)
        =\lim_{z\ra2} \dfrac{1}{(z-1)(z+2)}
        =\frac14,\\
      c&=\lim_{z\ra-2} (z+2)f(z)
        =\lim_{z\ra-2} \dfrac{1}{(z-1)(z-2)}
        =\frac1{12}.
    \end{align*}
    因此
    \[
    ...
    \begin{align*}
      a&=\lim_{z\ra1} (z-1)f(z)
        =\lim_{z\ra1} \dfrac{1}{(z-2)^2}
        =1,\\
      b&=\lim_{z\ra2} \bigl((z-2)^2f(z)\bigr)'
        =\lim_{z\ra2} \Bigl(\dfrac{1}{z-1}\Bigr)'
        =-1,\\
      c&=\lim_{z\ra2} (z-2)^2f(z)
        =\lim_{z\ra2} \dfrac{1}{z-1}
        =1.
    \end{align*}
    因此
    \[
...
然而, 若我们选择另一条曲线 $C'$ 来构造连续单值分支, 可能这两个函数在 $z_1$ 处取值不同.
...
本章所需掌握的重点如下:
\begin{enumpar}
  \item 能熟练使用\thmCR 求函数的可导点和解析点, 以及函数的导数.
  \item 熟知指数函数 $\ee^z$ 的定义和性质.
  \item 熟知对数函数 $\Ln z$ 及其主值的计算方式和性质.
  \item 熟知幂函数 $z^a$ 及其主值的计算方式和性质.
  \item 熟知 $\sin z$ 和 $\cos z$ 的定义、计算方式和性质.
  \item 会解含三角函数 $\sin z$ 或 $\cos z$ 的简单方程.
\end{enumpar}
...
\begin{homework}
  ...
  \begin{subex}
    ...
    \item 若函数 $f(z)=x^2-2xy-y^2+\ii(ax^2+bxy+cy^2)$ 在复平面上处处解析, 则 $a+b+c=$\fillblank{}.
    \item 若函数 $f(z)=\ee^{y}(\cos x+a\sin x)$ 在复平面上处处解析, 则常数 $a=$\fillblank{}.
