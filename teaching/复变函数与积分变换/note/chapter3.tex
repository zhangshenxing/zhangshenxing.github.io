...
\begin{figure}[hbpt]
	...
	\begin{tikzpicture}
		...
		\draw[cstcurve,second,smooth,domain=-65:30,cstwra] plot ({-1.4+0.7*cos(\x)}, {1.4+0.7*sin(\x)});
		\draw[cstcurve,second,smooth,domain=25:120,cstwra] plot ({-1.4+0.7*cos(\x)}, {1.4+0.7*sin(\x)});
		\draw[cstcurve,second,smooth,domain=115:210,cstwra] plot ({-1.4+0.7*cos(\x)}, {1.4+0.7*sin(\x)});
		\draw[cstcurve,second,smooth,domain=205:300,cstwra] plot ({-1.4+0.7*cos(\x)}, {1.4+0.7*sin(\x)});
	\end{tikzpicture}
	\caption{有向曲线}
\end{figure}
...
称 $n\ra\infty$, 分割的弧长 $\ra 0$ 时 $S_n$ 的极限为复变函数积分.
这二者是等价的.
...
\begin{figure}[hbpt]
	...
	\begin{minipage}{.48\textwidth}
		...
		\begin{tikzpicture}
			...
			\draw[cstcurve,second,cstra](0,0)--(1.5,2);
			\draw
				(2.5,1.2) node[second,align=center] {$z=(3+4i)t$\\$0\le t\le 1$};
		\end{tikzpicture}
	\end{minipage}
	\begin{minipage}{.48\textwidth}
		\centering
		\begin{tikzpicture}
			\draw[cstaxis](0,0)--(3,0);
			\draw[cstaxis](0,0)--(0,2.5);
			\draw[cstcurve,second,domain=0:1.5,cstra] plot({\x},{8*\x*\x/9});
			\draw
				(2.8,2) node[second,align=center] {$z=t+\dfrac49it^2$\\[1mm]$0\le t\le 3$};
		\end{tikzpicture}
	\end{minipage}
\end{figure}
...
\begin{figure}[hbpt]
	...
	\begin{minipage}{.48\textwidth}
		...
		\begin{tikzpicture}
			...
			\draw[cstcurve,second,cstra](0,0)--(2,2);
			\draw
				(2.4,0.9) node[second,align=center] {$z=(1+i)t$\\$0\le t\le 1$};
			\end{tikzpicture}
		\end{minipage}
		\begin{minipage}{.48\textwidth}
			\centering
			...
			\draw[cstaxis](0,0)--(0,2);
			\draw[cstcurve,cstra,second](0,0)--(0,1.5);
			\draw[cstcurve,cstra,second](0,1.5)--(1.5,1.5);
			\draw
				(0,.75) node[left,second,align=center] {$z=it$\\$0\le t\le 1$};
...
\begin{center}
	\begin{tikzpicture}
		\fill[cstcurve,main,rounded corners=0.5cm,cstfill3] (-2.5,-0.8) rectangle (2,1);
...
\exerciseanswer
\ansno{1.1.1} $-\dfrac12+\dfrac i2$.
\ansno{1.2.1} \enumnum1 $0$; \enumnum2 $0$.
\ansno{3.1.5} $v(x,y)=2x^3+3x^2y-6xy^2-y^3+C$.
