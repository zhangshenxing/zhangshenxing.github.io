...
%   设 $\delt s_k$ 为弧 $\warc{z_{k-1}z_k}$ 的长度, $\delt z_k=z_k-z_{k-1}$, 则 $\delt s_k\ge|{\delt z_k}|$.
%   于是
%   \[
...
% \begin{example}
%   求 $\doint_\Gamma\frac{\sin z}z\d z$, 其中 $\Gamma=C_1+C_2^-$, $C_1:|z|=2, C_2:|z|=1$.
% \end{example}
...
%   \begin{enuma}
%     \item \noun{偏积分法}: 通过 $v_y=u_x$ 解得 $v=\varphi(x,y)+\psi(x)$, 其中 $\psi(x)$ 待定. 再代入 $u_y=-v_x$ 中解出 $\psi(x)$.
%     \item \noun{不定积分法}: 对 $f'(z)=u_x-\ii u_y=v_y+\ii v_x$ 求不定积分得到 $f(z)$.
%   \end{enuma}
% \end{fifth}
...
\subsection{平面向量场与复变函数}

向量场可以理解为三维空间中的每一个点处关联了一个向量.
如果向量场中的每个向量都平行于某一个平面 $S$, 而且在垂直于 $S$ 的任意一条直线上的所有点处向量都相等, 则称这个向量场为\noun{平面向量场}.
...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}
    ...
    \end{scope}
    \begin{scope}[yshift=2cm,xslant=.6,xscale=1.2,yscale=.7]
      \filldraw[main,cstfill1] (-1.5,-.7) rectangle (1.5,1.4);
      \foreach \i in {1,2,3,4}{
    ...
    \begin{scope}[xshift=6.5cm,xslant=.7,xscale=1.2,yscale=.7]
      ...
      \draw[thick,cstra,second] (-2,-1)--++(1,0) node[above] {$\bfi$};
      \draw[thick,cstra,second] (-2,-1)--++(0,1.3) node[below right,inner sep=0pt] {$\bfj$};
    \end{scope}
  \end{tikzpicture}
  \caption{平面向量场}
\end{figure}

我们在平面 $S$ 内建立直角坐标系, 并分别记 $\bfi,\bfj$ 为 $x,y$ 正方向的方向向量.
