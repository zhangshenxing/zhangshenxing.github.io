    ...
    \draw[
      ...
      decoration={
        ...
        mark=at position 0 with {
          \fill[cstdot,fourth] circle node[left] {$A=z(a)$};
        },
        mark=at position .58 with {
          \arrow[rotate=-7]{Stealth}
        },
        mark=at position 1 with {
          \fill[cstdot,fourth] circle node[below] {$B=z(b)$};
        }
      },
      postaction={decorate}
    ] ({cos(-35)},{sin(-35)}) arc(-35:125:1);
    \draw[
      xshift=-17mm,
      yshift=12mm,
      cstcurve,
...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[cstdot,main]
      ...
      \fill[third] (Q0) circle node[right] {$B$};
      \fill[third] (0,0) circle node[left] {$A$};
    \end{scope}
  \end{tikzpicture}
  \caption{通过分割定义复积分}
    ...
    \draw[
      ...
      decoration={
        ...
        mark=at position .15 with {
          \arrow[rotate=-7]{Straight Barb}
          \node[above right] {$z=z_0+r\ee^{\ii\theta}$};
        }
      },
      postaction={decorate}
    ] (0,0) circle (1.3);
...
\end{itempar}\parnoindent
由此可见函数沿闭路积分为零,
与函数在闭路内部是否解析有关.

设 $C$ 是一条闭路, $D$ 是其内部区域.
设 $f(z)$ 在闭区域 $\ov D=D\cup C$ 上解析, 即存在区域 $B\supseteq\ov D$ 使得 $f(z)$ 在 $B$ 上解析.
为了简便, 我们假设 $f'(z)$ 连续,
则
\[
   \oint_C f(z)\d z
...
\begin{solution}
  ...
  \]
  由于 $\dfrac1z,\dfrac1{z+\ii}$ 均在 $|z-\ii |\le\dfrac12$ 上解析, 因此由\thmCG
  \[
    \oint_C\frac1z\d z
  ...
  \begin{minipage}{.48\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \draw [
        ...
        domain=0:360,
        samples=500,
      ] ({-.5+\r*cos(-50)},{.1+\r*sin(-50)}) arc(310:-40:\r)
        ;
    \end{tikzpicture}
...
\begin{proof}
  ...
  若 $a$ 在 $C$ 的内部, 令 $C_1$ 为以 $a$ 为圆心的圆周.
  由\thmCT 和\thmref{定理}{thm:circle-integral},
  \[
    \oint_C (z-a)^{n+1}\d z=\oint_{C_1}(z-a)^{n+1}\d z=0.\qedhere
  \]
\end{proof}

同理, 由\thmCT 和\thmref{定理}{thm:circle-integral} 可知当 $a$ 在 $C$ 的内部且 $n=0$ 时, 积分为 $2\cpi\ii $.
...
\begin{solution}
  函数 $\dfrac{2z-1}{z^2-z}$ 在 $\Gamma$ 内部有两个奇点 $0,1$.
  ...
  \[
     \oint_\Gamma\frac{2z-1}{z^2-z}\d z
    =\oint_{C_1}\frac1z\d z+\oint_{C_1}\frac1{z-1}\d z.
  \]
  最后由\thmref{定理}{thm:closed-power-integral} 得到
  \[
     \oint_\Gamma\frac{2z-1}{z^2-z}\d z
    =2\cpi\ii +0+0+2\cpi\ii
    =4\cpi\ii.
  \]
\end{solution}

\begin{example}
  求 $\doint_\Gamma\frac{\ee^z}z\d z$, 其中 $\Gamma=C_1+C_2^-$, $C_1:|z|=2, C_2:|z|=1$.
\end{example}
...
\begin{proof}
  ...
  \]
  我们需要比较上述两个积分, 其中 $z$ 到 $z+\delt z$ 取直线段.
  由 $f(z)$ 解析可知它连续, 因此 $\forall\varepsilon>0,\exists\delta>0$ 使得 $\delta<r$ 且当 $|\zeta-z|<\delta$ 时, $|f(\zeta)-f(z)|<\varepsilon$.
...
复变量情形要求是\alert{单连通区域上解析函数}, 实变量情形要求是\alert{闭区间上连续函数}.
究其原因, 都是为了保证原函数的存在性.
...
\begin{example}
  设 $C$ 为沿着 $|z|=1$ 从 $1$ 到 $\ii$ 的逆时针圆弧, 求 $\dint_C\frac{\ln(z+1)}{z+1}\d z$.
\end{example}
...
该积分通常使用参变量法来计算.
不过, 尽管被积函数并没有原函数, 但是我们可以通过 $C$ 的方程来将被积函数变形为 $z$ 的表达式, 从而可以使用原函数法来计算.
...
从\thmCI 可以看出, 被积函数分子解析而分母形如 $z-z_0$ 时, 绕闭路的积分可以使用该公式计算.
...
\begin{solution}
  ...
  \[
    ...
    =2\cpi\ii (3\zeta^2+7\zeta+1)|_{\zeta=z}
    =2\cpi\ii (3z^2+7z+1).
  \]
  因此
  \[
...
\begin{solution}
  ...
  \begin{align*}
    ...
    =&2\cpi\ii \Bigl(\frac{\ee^z}{z^2-1}\Big|_{z=0}+\frac{\ee^z}{z(z+1)}\Big|_{z=1}+\frac{\ee^z}{z(z-1)}\Big|_{z=-1}\Bigr)\\
...
不过, 这种做法并不正确, 因为我们需要先说明 $f(z)$ 确实存在任意阶导数, 或者被积函数求导后确实还可积.
...
由高阶导数的柯西积分公式可知, 区域 $D$ 上的\alert{解析函数 $f(z)$ 一定任意阶可导}, 各阶导数仍然是解析的.
这一点与实变量函数有本质的区别.
...
使用时需要注意分母为 $(z-z_0)^n$ 时需要\alert{对 $f(z)$ 求 $n-1$ 阶导数而不是 $n$ 阶导数}.
...
\begin{exercise}
  $\doint_{|z-2\ii|=3}\frac1{z^2(z-\ii)}\d z=$\fillblank{}.
\end{exercise}
...
\begin{definition}
  若二元实函数 $u(x,y)$ 在区域 $D$ 内有二阶连续偏导数, 且满足拉普拉斯方程
  \[
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \delt v&=v_{xx}+v_{yy}=-u_{yx}+u_{xy}=0.\qedhere\end{align*}
\end{proof}
...
\begin{fifth}{共轭调和函数的计算方法}
  \begin{enumpar}
    ...
    \item \alert{不定积分法}: 对 $f'(z)=u_x-\ii u_y=v_y+\ii v_x$ 求不定积分得到 $f(z)$.
  \end{enumpar}
\end{fifth}
...
本章所需掌握的知识点如下:
\begin{enumpar}
  \item 理解\thmCG 和\thmCCC 的条件和结论, 并能熟练使用这两个定理.
  \item 能熟练使用有向曲线的参数方程来计算适用情形的复积分.
  \item 能熟练使用原函数法计算单连通区域上解析函数的积分.
  \item 能熟练使用柯西积分公式计算适用情形的复积分.
...
\begin{enumpar}
  ...
  \item 使用\thmCIH 时记错求导次数, 或记错系数.
  \item 对可以使用牛顿-莱布尼兹定理的情形, 使用曲线的参数方程来计算. 在少量情形下, 这样做也有可能能计算出来, 但往往计算量非常大.
  \item 误以为实部 $u$ 也是虚部 $v$ 的共轭调和函数.
\end{enumpar}
  ...
  \begin{subex}
    ...
    \item 函数\fillbrace{}在区域 $\Re z>0$ 内的积分与路径\emph{有关}.
      \begin{exchoice}(2)
        ...
        \item $2x+3y\ii$
      \end{exchoice}
    \item 设 $\displaystyle f(z)=\oint_{|\zeta|=2}\frac{\sin \zeta}{\zeta-z}\d \zeta$, 则 $f\Bigl(\dfrac\cpi6\Bigr)=$ \fillbrace{}.
      \begin{exchoice}(4)
        \item $\cpi\ii $
        \item $-\cpi\ii $
        \item $0$
    ...
    \item 设 $f(z)=\doint_{|\zeta|=4}\dfrac{\sin\zeta-\cos\zeta}{\zeta-z}\d\zeta$, 则 $f'(\cpi)=$\fillbrace{}.
      \begin{exchoice}(4)
        ...
        \item $\cpi\ii $
      \end{exchoice}
    % \item 设 $f(z)=\doint_{|\zeta|=2}\dfrac{\zeta^3+3\zeta}{\zeta-z}\d\zeta$, 则 $f'(\ii)=$\fillbrace{}.
    % \begin{exchoice}(4)
    %   \item $0$
    %   \item $3\ii$
    %   \item $-3\ii$
  ...
  \begin{subex}
    \item 计算下列积分.
      \begin{subsubex}
        ...
        \item $\dint_C \ov z\d z$, 其中 $C$ 是从 $\ii$ 到 $2+\ii$ 的直线段.
      \end{subsubex}
    \item 直接写出下列积分的值, 其中 $C:|z|=1$.
      \begin{subsubex}(2)
        \item $\doint_C\frac{\d z}{z-2}$;
        \item $\doint_C\frac{\d z}{\cos z}$;
        \item $\doint_C\frac{\ee^z}{(z-2i)^2}\d z$;
    ...
    \item 计算下列积分.
      \begin{subsubex}
        ...
        \item $\dint_C\cos^2z\d z$, 其中 $C$ 是从 $\ii$ 到 $i-\cpi$ 再到 $-\cpi$ 的折线段;
        \item $\dint_C(\ee^z+3z^2+1)\d z$, 其中 $C$ 是圆周 $|z|=2$ 的右半部分, 方向从 $-2i$ 到 $2i$;
        \item $\dint_C\cos^2z\d z$, 其中 $C$ 是从 $\ii$ 到 $i-\cpi$ 再到 $-\cpi$ 的折线段;
...
%     +\ii\iint_D(u_x-v_y)\d x\d y.
%   \]
%   \item 将上述等式改写为如下形式:
%     \[
%       \oint_C f(z)\d z=2i\iint_D\pdv f{\ov z}\d x\d y=0
%     \]
%   \item 计算
%     \begin{subsubex}(2)
%       \item $\doint_{|z|=r}\Im z\d z$;
%       \item $\doint_{|z|=r}\dfrac 1z\d z$;
%       \item* $\doint_{C}\Re z\d z$, 其中 $C$ 是 $0,i,1+\ii$ 构成的三角形闭路. 该结果与\thmref{例}{exam:re-z-integral} 有何联系?
%     \end{subsubex}
% \end{subex}
...
%   设
%   \[
%     C: z=a+r\ee^{\ii \theta}, \quad -\cpi<\theta_1<\theta<\theta_2<\cpi
%   \]
%   为一圆弧, $n$ 为整数, 求 $\dint_C \dfrac1{z-a}\d z$.
% \end{example}
...
%   \[
%      \int_C f(z)\d z
%     =\ln (z-a)\Big|_{a+r\ee^{\ii \theta_1}}^{a+r\ee^{\ii \theta_2}}
%     =(\theta_2-\theta_1)\ii.
%   \]
% \end{solution}

% 不难说明, 当 $\theta_1\ra (-\cpi)^+,\theta_2\ra \cpi^-$ 时, 该积分的极限是
% \[
%   \oint_{|z-a|=r}\frac1{z-a}\d z=2\cpi\ii .
% \]
