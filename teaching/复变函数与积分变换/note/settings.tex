...
% 选读
\newcommand{\optional}{\textcolor{black}{(选读内容)}}
% 文本色
\newcommand{\noun}[1]{{\color{main}{\heiti #1}\index{{#1}}}}
\newcommand{\nouns}[2]{{\color{main}{\heiti #1}\index{{#2}}}}
\renewcommand{\emph}[1]{{\kaishu #1}}
\newcommand{\alert}[1]{{\color{main}{\kaishu #1}}}
...
\newcommand{\setenumtype}[1]{
	\ifstrequal{#1}{(1)}{%
		\renewcommand{\thecustomenumi}{{\upshape\textcolor{main}{(\arabic{customenumi})}}}
	}{}
	\ifstrequal{#1}{(i)}{%
		\renewcommand{\thecustomenumi}{{\upshape\textcolor{main}{(\roman{customenumi})}}}
	}{}
  \setcounter{customenumi}{0}%
}
% 分段枚举环境
\newenvironment{enumpar}[1][(1)]{%
	\setenumtype{#1}\renewcommand{\item}{%
		\par%
	\refstepcounter{customenumi}\thecustomenumi{ }}}%
{\ignorespacesafterend}
% 首项不分段枚举环境
\newenvironment{enumnopar}[1][(1)]{%
	\setenumtype{#1}\renewcommand{\item}{%
    \ifnum\value{customenumi}=0\hspace{-1em}\else\par\fi%
  \refstepcounter{customenumi}\thecustomenumi { }}}%
{\ignorespacesafterend}
% 不分段枚举环境
...
% 填空题/选择题/判断题
\NewDocumentCommand\fillblank{O{3em} O{0ex} m}{\uline{\makebox[#1]{\raisebox{#2}{\alertn{#3}}}}}
\newcommand\fillbrace[1]{{\text{\upshape(\nolinebreak\hspace{0.5em minus 0.5em}\alertn{#1}\hspace{0.5em minus 0.5em}\nolinebreak)}}}
