...
% 		孤立奇点类型&洛朗级数特点&$\liml_{z\to z_0}f(z)$\\\hline
...
% 	\begin{tabular}{|c|c|c|}\hline
% 		$\infty$ 类型&洛朗级数特点&$\liml_{z\to\infty}f(z)$\\\hline
...
\begin{definition}
	...
	称
		\[\Res[f(z),z_0]:=c_{-1}=\frac1{2\pi i}\oint_Cf(z)\d z\]
...
\begin{theorem}[留数定理]
	...
	\[\oint_Cf(z)\d z=2\pi i\sum_{k=1}^n\Res[f(z),z_k].\]
\end{theorem}
...
\begin{proof}
	...
	{由复闭路定理,
	\[\oint_Cf(z)\d z=\sum_{k=1}^n\oint_{C_k}f(z)\d z
...
\begin{center}
	\begin{tikzpicture}[node distance=25pt]
		\node at (4.55,0.3) [cstnode2]	(end1){求出 $f(z)$ 的原函数 $F(z)$ 得到 $\displaystyle\int_Cf(z)\d z=F(b)-F(a)$};
		...
		\node at (1.5,6.5) [cstnode1]  (integral){定积分 $\displaystyle\int_Cf(z)\d z$ 的计算};
...
\begin{solution}
	\[I=\int_0^{+\infty}\frac{x^p}{x(x+1)}\d x\xto{\text{令}\ x=e^t}\int_{-\infty}^{+\infty}\frac{e^{pt}}{e^t+1}\d t.\]
	...
	{因此
		\[\int_l f(z)\d z
		=\int_R^{-R}\frac{e^{2p\pi i}\cdot e^{pt}}{e^t+1}\d t
		=-e^{2p\pi i}\int_{-R}^Rf(t)\d t.\]由于 $C_1:z=R+it,0\le t\le 2\pi$,因此
		\[\abs{\int_{C_1}f(z)\d z}\le \frac{e^{(p+1)R}}{e^R-1}\cdot 2\pi\to 0\quad(R\to+\infty).\]同理
		\[\abs{\int_{C_2}f(z)\d z}\le \frac{e^{-(p+1)R}}{1-e^{-R}}\cdot 2\pi\to 0\quad(R\to+\infty).\]
	}
	...
	{因此
		\begin{align*}
		&\left(\int_{-R}^R+\int_l+\int_{C_1}+\int_{C_2}\right)f(z)\d z\\
		=&\oint_Cf(z)\d z=2\pi i\Res[f(z),\pi i]=-2\pi ie^{p\pi i},
...
\begin{exercise}[2]{9}
	利用留数计算下述积分:
	\subex{$\displaystyle\oint_{|z|=4}\frac{\sin z}{z(z-\pi)}\d z$;}
	\subex{$\displaystyle\oint_{|z|=2}\frac{e^{2z}}{z(z-1)^2}\d z$;}
	\subex{$\displaystyle\oint_{|z|=\frac32}\frac{1-\cos z}{z^5}\d z,\quad m\in\BZ$;}
	\subex{$\displaystyle\oint_{|z|=1}\frac1{(z-\frac12)^9(z-2)^9}\d z$.}
\end{exercise}
\begin{solution}
