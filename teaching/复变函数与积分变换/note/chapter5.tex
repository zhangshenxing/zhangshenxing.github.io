...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}
    ...
    \draw
      (-2.5,0) node[above left] {$-(N+\dfrac12)$}
      (2.5,0) node[above right] {$N+\dfrac12$}
      (0,2.5) node[below right] {$(N+\dfrac12)\ii$}
      (0,-2.5) node[above left] {$-(N+\dfrac12)\ii$}
...
% 由此得到:
% \begin{theorem}[辐角原理]
%   设 $f(z)$ 在闭路 $C$ 上解析且非零, 在 $C$ 的内部的奇点均为极点, 则
%   \[
%     \frac1{2\cpi}\delt_C \arg f(z)
%     =N-P,
%   \]
