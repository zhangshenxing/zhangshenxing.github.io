...
\onslide<+->{从而 $z_1-z_2=k\pi(1+i)$.}
\onslide<+->{所以 $f(z)$ 的周期是 $\pi(1+i)$.}
\end{solution}
\onslide<+->
...
\onslide<+->{$\Ln(-1)=\ln1+i\Arg(-1)=(2k+1)\pi i, k\in\BZ$,}
\onslide<+->{主值是 $\pi i$.}
\end{solution}
\end{frame}
...
\draw (3,1.2) node[dcolorc] {$w=z^{1/n}$};
\end{tikzpicture}
\end{center}
\end{frame}
...
\onslide<+->{由于 $\cos z=\dfrac{e^{iz}+e^{-iz}}2=2$,}
\onslide<+->{我们有 $e^{2iz}-4e^{iz}+1=0$.}
\onslide<+->{于是 $e^{iz}=2\pm\sqrt 3$,}
\vspace{-4pt}
\onslide<+->{\[z=-i\Ln(2\pm\sqrt 3)=2k\pi\pm i\ln(2+\sqrt3),\quad k\in\BZ.\]}
\vspace{-14pt}
\end{solution*}
\end{frame}
