...
\begin{columns}
	\column{0.48\textwidth}
		\begin{exercise}
		求 $\ln(-1-\sqrt3 i)$.\vphantom{$\dfrac{2\pi i}3$}
		\end{exercise}
		\onslide<+->
	\column{0.48\textwidth}
		\begin{answer}
...
\onslide<+->
而当 $|n|\ge 2$ 时, \markatt{$\Ln z^n=n\Ln z$ 不成立}.
\onslide<+->
以上等式换成 $\ln z$ 均不一定成立.
\end{frame}
...
\onslide<+->
对于整数 $n$, 我们总有 \marknot{$(z^a)^n=z^{an}$}.
\onslide<+->
这里换成主值也成立.
\onslide<+->
而 \marknot{$z^a\cdot z^b=z^{a+b}$ 仅对主值总成立}.
\end{frame}
...
\onslide<+->
因此\marknot{当且仅当 $a=\pm\dfrac 1n,n\in\BZ_+$ 时, $\Ln z^a=a\Ln z$} 成立.
\onslide<+->
对于除此之外的 $a$, 该式子均不成立.
...
\onslide<+->
那么欧拉恒等式 \marknot{$e^{iz}=\cos z+i\sin z$ 对任意复数 $z$ 均成立}.
\end{frame}
