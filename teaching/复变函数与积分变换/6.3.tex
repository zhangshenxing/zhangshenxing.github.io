...
\begin{tikzpicture}[node distance=-20pt]
  ...
  \node[draw,align=center, right=116pt of a](b){象函数\\\markatt{$F(s)=\displaystyle\int_0^{+\infty}f(t)e^{-st}\diff t$}};
...
\end{align*}
\onslide<+->
即 \boxatt{$\msl[e^{kt}]=\dfrac1{s-k}$}.
\onslide<+->
特别地 \boxatt{$\msl[1]=\dfrac1s$}.
\end{solution}
\end{frame}
...
\vspace{-\baselineskip}
\[\eqatt{\msl[f'(t)]=sF(s)-f(0)},\]
\[\eqatt{\msl[f''(t)]=s^2F(s)-sf(0)-f'(0)},\]
...
因此
\begin{align*}
&\peq\eqatt{\msl[\sin{kt}]}=\frac1{2j}\left(\msl[e^{jkt}]-\msl[e^{-jkt}]\right)\\
&\visible<+->{=\frac1{2j}\left(\frac1{s-jk}-\frac1{s+jk}\right)}
\visible<+->{\eqatt{=\frac k{s^2+k^2}},\quad \Re s>\abs{\Im k}.}
\end{align*}
\end{solution}
\end{frame}
...
\end{exercise}
\begin{answer}
\boxatt{$\msl[\cos{kt}]=\dfrac s{s^2+k^2}$}, $\Re s>\abs{\Im k}$.
\end{answer}
\end{frame}
...
\onslide<+->
\vspace{-\baselineskip}
\[\eqatt{\msl[t^m]}=\msl[f(t)]\eqatt{=\frac{m!}{s^{m+1}}},\quad \Re s>0.\]
\onslide<+->
或由
\vspace{-\baselineskip}
\[\msl[t^m]=(-1)^m \msl[1]^{(m)}
...
\node[draw,fill=lightr] (p1){微分方程或积分方程};
\node[draw,right=110pt of p1] (p2){象函数的代数方程};
\node[draw,fill=lightr,below=of p1] (p3){原象函数(方程的解)};
