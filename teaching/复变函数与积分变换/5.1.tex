...
\begin{frame}{孤立奇点}
	...
	\begin{example}
		...
		\onslide<+->{%
			此时无法选取一个圆环域 $0<|z|<\delta$ 作 $f(z)$ 的洛朗展开, 因此我们不考虑这类奇点.
		}
		\onslide<3->
		\vspace{-9pt}
		\begin{center}
...
\begin{frame}{典型例题: 函数的极点}
	...
	\begin{example}
		...
		\onslide<+->{%
			由于 $\lim\limits_{z\to 0}z^2f(z)=1$, 因此 $0$ 是二阶极点.
		}
		\onslide<+->{%
			同理 $-2$ 是一阶极点.
		}
...
\begin{frame}{函数的零点, 极点和阶}
	...
	\onslide<+->
	不难看出, 如果 $\ord(f,z_0)\ge 0$, 则 $z_0$ 是可去奇点或解析点.
	\onslide<+->
...
\begin{frame}{典型例题: 函数的极点}
	...
	\onslide<+->
	\begin{example}
		单选题: (2021年B卷) $z=0$ 是函数 $f(z)=\dfrac{e^z-1}{z^2}$ 的\fillbrace{\visible<5->{A}} 阶极点.
		\xx{$1$}{$2$}{$3$}{$4$}
	\end{example}
