...
\fill[cstdot,notcolor] (2,0) circle;
\fill[cstdot,alecolor] (1,0) circle;
\fill[cstdot,notcolor] (0.6667,0) circle;
\fill[cstdot,alecolor] (0.5,0) circle;
\fill[cstdot,notcolor] (0.4,0) circle;
\draw
  (-0.3,0) node {$0$}
  (2,-0.3) node[notcolor] {$z_1$}
  (1,0.3) node[alecolor] {$z_2$}
  (0.6667,-0.3) node[notcolor] {$z_3$}
  (0.5,0.3) node[alecolor] {$z_4$}
  (0.4,-0.3) node[notcolor] {$z_k$};
\end{tikzpicture}
\end{center}
\vspace{-7pt}
\end{example}
...
\onslide<+->
因此 \marknot{$z_0$ 是 $f(z)$ 的 $m$ 阶极点当且仅当 $\lim\limits_{z\to z_0}(z-z_0)^mf(z)$ 存在且非零}.
...
\end{theorem}
\onslide<+->
如果 $\ord=-m$, 则 $z_0$ 是 $m$ 阶极点.
\onslide<+->
如果 $\ord\ge 0$, 则 $z_0$ 是可去奇点或解析点.
\end{frame}
