...
&\visible<.->{\qquad\qquad+i(\cos\theta_1\sin\theta_2+\sin\theta_1\cos\theta_2)\bigr]}\\
...
\onslide<+->
\begin{exercise}
求 $(\sqrt3+i)^{2022}=$\fillblank{\visible<+->{\alert{$-2^{2022}$}}}.
\end{exercise}
\end{frame}
...
\onslide<+->
\begin{exercise}
求 $\sqrt[6]{-1}=$\fillblank{\visible<+->{$\pm\dfrac{\sqrt3+i}2,\pm i,\pm\dfrac{\sqrt3-i}2$}}.
\end{exercise}
...
\onslide<+->{
除非给定单值分支
\[\sqrt z=\sqrt{|z|}\exp\left(\frac{i\arg z}2\right),\]
否则不能说 $\sqrt{-1}=i$.}
\end{answer}
\end{frame}
...
\begin{frame}[<*>]{作业}
  \begin{homeworks}
    \item 如果 $\left|\dfrac{z_2-z_1}{z_3-z_1}\right|=\left|\dfrac{z_1-z_3}{z_2-z_3}\right|$. 证明 $|z_1-z_2|=|z_2-z_3|=|z_3-z_1|$ 并说明这些等式的几何意义.
    \item 将 $\dfrac{(\cos \varphi+i\sin \varphi)^5}{(\cos \varphi-i\sin \varphi)^3}$ 写成三角形式和指数形式.
    \item 计算 \enumnumhk1 $(\sqrt3-i)^5$; \enumnumhk2 $(1+i)^6$; \enumnumhk3 $\sqrt[6]{-1}$; \enumnumhk4 $\sqrt[4]{-2+2i}$.
    \item(2020年期末B卷) 复数 $\left[\dfrac{(1+i)^2}2\right]^{2021}$ 的模是\fillblank{}.
    \item(2020年期末B卷) 求 $\sqrt[4]{-2}$.
    \item(2021年期末A卷) 解方程 $z^3+8=0$.
  \end{homeworks}
\end{frame}
}