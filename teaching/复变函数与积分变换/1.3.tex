...
\draw[cstcurve,alecolor,cstarrowto] (B)--(A);
\draw[cstcurve,alecolor,cstarrowto] (B)--(C);
\draw[cstcurve,alecolor] pic [draw, "$\theta$", angle eccentricity=1.4] {angle};
\coordinate (A) at (1.3,1.6);
\coordinate (C) at (-1.14,2.52);
\draw[cstcurve,notcolor,cstarrowto,visible on =<3->] (B)--(A);
\draw[cstcurve,notcolor,cstarrowto,visible on =<3->] (B)--(C);
\draw[cstcurve,notcolor,visible on =<3->] pic [draw, "$\theta$", angle eccentricity=1.3, angle radius= 0.7cm] {angle};
\draw 
  (1.9,2.6) node[alecolor] {$z=re^{i\theta}$}
  (2,-0.3) node[alecolor] {$1$}
  (1.6,1.5) node[notcolor,visible on =<3->] {$z_1$}
  (-1.1,2.7) node[notcolor,visible on =<3->] {$zz_1$};
\end{tikzpicture}
\end{center}
\end{frame}
