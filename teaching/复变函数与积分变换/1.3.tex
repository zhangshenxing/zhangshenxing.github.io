...
\onslide<+->{设 $\dfrac{z_1}{z_2}=re^{i\theta}$,}
\onslide<+->{则由乘法情形可知
\[rr_2=r_1,\quad \theta+\Arg z_2=\Arg z_1.\]}
\onslide<+->{因此 $r=\dfrac{r_1}{r_2},\theta=\theta_1-\theta_2+2k\pi$, 其中 $k\in\BZ$.\qedhere}
\end{proof*}
\end{frame}
...
\onslide<+->
\begin{exercise}
求 $(\sqrt3+i)^{2022}=$\fillblank[2cm]{\visible<+->{\alert{$-2^{2022}$}}}.
\end{exercise}
\end{frame}
...
\onslide<+->
显然
\[w_1=iw_0,\quad w_2=-w_0,\quad w_3=-iw_0.\]
\onslide<+->
...
\onslide<+->
\begin{exercise}
求 $\sqrt[6]{-1}=$\fillblank[5cm][2mm]{\visible<+->{$\pm\dfrac{\sqrt3+i}2,\pm i,\pm\dfrac{\sqrt3-i}2$}}.
\end{exercise}
\end{frame}
