...
\onslide<+->
这里关于辐角的两个等式的含义是指: 两边所能取到的值构成的集合相等,
\onslide<+->
即
...
\draw[cstcurve,alecolor,cstarrowto] (0,0)--(2,0);
\draw[cstcurve,alecolor,cstarrowto] (0,0)--(1.2,2.4);
\draw[cstcurve,alecolor,cstarrow1from] (0.3,0.6) arc(63:0:0.67);
\draw[cstcurve,notcolor,cstarrowto,visible on =<3->] (0,0)--(1.6,1.2);
\draw[cstcurve,notcolor,cstarrowto,visible on =<3->] (0,0)--(-0.6,3.3);
\draw[cstcurve,notcolor,cstarrow1from,visible on =<3->] (-0.2,1.1) arc(100:37:1.11);
\draw 
  (2,2.3) node[alecolor] {$z=re^{i\theta}$}
  (2,-0.3) node[alecolor] {$1$}
  (0.9,0.3) node[alecolor] {$\theta$}
  (1.9,1.2) node[notcolor,visible on =<3->] {$z_1$}
  (-1,3.1) node[notcolor,visible on =<3->] {$zz_1$}
  (0.3,1.4) node[notcolor,visible on =<3->] {$\theta$};
\end{tikzpicture}
\end{center}
\end{frame}
...
\draw[cstcurve,alecolor] (1,0)--(2,1)--(0.634,1.366)--cycle;
\draw[cstdash,alecolor] (1,0)--(2.366,-0.366)--(2,1);
\draw 
  (1.0,-0.2) node[alecolor] {$z_1$}
  (2.3,1) node[alecolor] {$z_2$}
  (0.4,1.2) node[alecolor] {$z_3$}
  (2.6,-0.3) node[alecolor] {$z_3'$};
\end{tikzpicture}
\end{center}
\end{solutions}
\end{frame}
...
\onslide<+->
注意 $\Arg(z^n)=n\Arg z$ 一般\markatt{不成立}, 除非 $n=\pm1$.
\onslide<+->
这是因为
...
\begin{align*}
\cos(n\theta)&=\Re\left[
\sum_{0\le k\le n}C_n^k \cos^{n-k}\theta(i\sin\theta)^k\right]\\
&\visible<+->{=\sum_{0\le 2k\le n}C_n^{2k} \cos^{n-2k}\theta(-\sin^2\theta)^k}\\
&\visible<+->{=\sum_{0\le k\le n/2}C_n^{2k} \cos^{n-2k}\theta(\cos^2\theta-1)^k.}
\end{align*}
\onslide<+->
因此 $\cos{n\theta}$ 是 $\cos\theta$ 的多项式.
\onslide<+->
这个多项式
\[g_n(T)=\sum_{0\le k\le n/2}C_n^{2k} T^{n-2k} (T^2-1)^k.\]
叫做切比雪夫多项式.
\onslide<+->
它在计算数学的逼近理论中有着重要作用.
\end{frame}
...
\onslide<+->
因此只需取 \marknot{$k=0,1,\dots,n-1$}.
\onslide<+->
故\markatt{任意一个非零复数的 $n$ 次方根有 $n$ 个值}.

\onslide<+->
这些根的模都相等, 且 $w_k$ 和 $w_{k+1}$ 辐角相差 $\dfrac{2\pi}n$.
\onslide<+->
因此\markatt{它们是以原点为中心, $\sqrt[n]r$ 为半径的圆的正接 $n$ 边形的顶点}.
\end{frame}
...
\draw[cstcurve,alecolor,cstarrowto] (0,0)--(1.5,0.3);
\draw[cstcurve,alecolor,cstarrowto] (0,0)--(-1.5,-0.3);
\draw[cstcurve,alecolor,cstarrowto] (0,0)--(-0.3,1.5);
\draw[cstcurve,alecolor,cstarrowto] (0,0)--(0.3,-1.5);
\draw[cstcurve,defcolor] (1.5,0.3)--(-0.3,1.5)--(-1.5,-0.3)--(0.3,-1.5)--cycle;
\draw
  (-0.2,-0.3) node {$0$}
  (1.8,0.3) node[alecolor] {$w_0$}
  (-0.8,1.3) node[alecolor] {$w_1$}
  (-1.8,-0.3) node[alecolor] {$w_2$}
  (0.3,-1.7) node[alecolor] {$w_3$};
\end{tikzpicture}
\end{center}
\end{frame}
