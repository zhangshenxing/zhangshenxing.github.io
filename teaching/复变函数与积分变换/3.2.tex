...
\onslide<+->{因此 $\displaystyle\int_C(z-a)^n\diff z=0$.}

\onslide<+->{当 $n\le -2$ 时, 如果 $a$ 不在 $C$ 的内部,}
...
\onslide<+->
\begin{example}
求 $\displaystyle\int_\Gamma\frac{2z-1}{z^2-z}\diff z$, 其中
$\Gamma$ 是由 \\
...
\onslide<+->
\begin{example}
求 $\displaystyle\int_\Gamma\frac{e^z}z\diff z$, 其中

\hspace{1.5cm}$\Gamma=C_1+C_2^-$,\rule{0pt}{12mm}

\hspace{6mm}$C_1:|z|=2,C_2:|z|=1$.\rule{0pt}{12mm}
\begin{tikzpicture}[overlay,xshift=4.5cm,yshift=1.6cm]
