...
\begin{frame}{例题: 柯西-古萨基本定理计算积分}
	\onslide<+->
	\begin{example}
		求 $\displaystyle\oint_C\frac1{z(z^2+1)}\diff z$, 其中 $C:|z-i|=\frac12$.
	\end{example}
	...
	\begin{solution}
		...
		\onslide<+->{由于 $\dfrac1z,\dfrac1{z+i}$ 在 $|z-i|\le\dfrac12$ 上解析,
			...
			=-\frac12\oint_C\frac1{z-i}\diff z=-\pi i.\]}
	\end{solution}
\end{frame}
...
\begin{frame}{例题: 复合闭路定理的应用}
	...
	\begin{alertblock@}
		当 $a$ 在 $C$ 的内部时,
		\[\oint_C\frac{\diff z}{(z-z_0)^{n+1}}=\begin{cases}2\pi i,&n=0;\\0,&n\neq 0.\end{cases}\]
	\end{alertblock@}
\end{frame}
...
\begin{frame}{例题: 复合闭路定理的应用}
	\onslide<+->
	\begin{example}
		求 $\displaystyle\int_\Gamma\frac{2z-1}{z^2-z}\diff z$, 其中 $\Gamma$ 是由 \\
		$2\pm i,-2\pm i$ 形成的矩形闭路.\rule{0pt}{6mm}
...
\begin{frame}{例题: 复合闭路定理的应用}
	\onslide<+->
	\begin{example}
		求 $\displaystyle\int_\Gamma\frac{e^z}z\diff z$, 其中 $\Gamma=C_1+C_2^-$,
		
		\vspace{2mm}
		$\qquad C_1:|z|=2,\qquad C_2:|z|=1$.

		\vspace{16mm}
		\begin{tikzpicture}[overlay,xshift=9cm,yshift=1.8cm]
