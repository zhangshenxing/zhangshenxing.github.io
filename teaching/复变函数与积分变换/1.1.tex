...
\onslide<+->
\begin{example}
实数 $x$ 取何值时, $(x^2-3x-4)+(x^2-5x-6)i$ 是: \enumnum1 实数; \enumnum2 纯虚数.
\end{example}
\onslide<+->
\begin{solution}
\enumnum1 $x^2-5x-6=0$, 即 $x=-1$ 或 $6$.

\visible<+->{\enumnum2 $x^2-3x-4=0$, 即 $x=-1$ 或 $4$.}
...
\begin{frame}{典型例题: 常见复数的幂次}
\onslide<+->
\begin{example}[2022年期末A卷]
化简 $1+i+i^2+\cdots+i^4=$\fillblank{\visible<4->{$1$}}.
\end{example}
\onslide<+->
\begin{solution}
根据等比数列求和公式,
\[1+i+i^2+\cdots+i^4=\frac{i^5-1}{i-1}
\visible<+->{=\frac{i-1}{i-1}=1.}\]
\vspace{-\baselineskip}
\end{solution}
\onslide<+->{
\begin{exercise}[2020年期末A卷]
...
\begin{frame}[<*>]{作业}
  \begin{homeworks}
    \item 判断题: $z$ 是实数当且仅当 $z=\ov z$.
    \item 判断题: $z$ 是纯虚数当且仅当 $z=-\ov z$.
    \item $z_1=-z,z_2=\ov z,z_3=-\ov{(-z)}$ 在复平面上对应的点分别与 $z$ 在复平面上对应的点是什么关系?
    \item 已知点 $z_1,z_2,z_3$. 点 $\frac12(z_1+z_2)$ 和 $\frac13(z_1+z_2+z_3)$ 表示什么点?
    \item 如果 $\dfrac{x+1+i(y-3)}{5+3i}=1+i$, 那么实数 $x,y$ 分别是多少?
    \item 证明共轭复数性质汇总中列出的结论.
    \item 证明 $z_1\cdot\ov{z_2}-\ov{z_1}\cdot z_2=2i\Im(z_1\cdot\ov{z_2})$.
    \item(2020年期末B卷) 设复数 $z=\dfrac{2+i}i-\dfrac{2i}{1-i}$, 求 $\Re z,\Im z,z\ov z$.
    \item(2021年期末A卷) 化简 $\dfrac{(1+i)^{101}}{(1-i)^{99}}=$\fillblank{}.
  \end{homeworks}
\end{frame}
}