...
\draw[dcolorb,line width=0.1cm,cstarrow1double,visible on=<8->] (6.2,0.8)--(7.4,0.8);
\draw 
  (-0.2,-0.2) node {$0$}
  ...
  (0.8,-0.7) node[dcolorb,visible on=<7->] {\textbf{复平面}}
  (9.5,1.5) node[dcolora,visible on=<8->] {$\overrightarrow{OZ}=(x,y)$}
  (9.8,-0.2) node[visible on=<8->] {$x$}
  (7.8,1.5) node[visible on=<8->] {$y$}
  (7.8,-0.2) node[visible on=<8->] {$O$}
  (9.0,-0.7) node[visible on=<8->] {向量}
  (6.8,0.3) node[dcolorb,visible on=<8->] {\small 一一对应};
\end{tikzpicture}
\end{center}
\end{frame}
