...
\begin{example}
求双边幂级数 $\displaystyle\sum_{n=1}^\infty\frac{2^n}{z^n}+\sum_{n=0}^\infty\frac{z^n}{(2+i)^n}$ 的收敛域与和函数, 其中 $a,b$ 为非零复数.
\end{example}
\begin{solution}
\indent
非负幂次部分收敛当且仅当 $|z|<|2+i|=\sqrt5$, 负幂次部分收敛当且仅当 $|z|>|2|=2$.
\onslide<+->
因此该双边幂级数的收敛域为 $2<|z|<\sqrt5$.
\onslide<+->
此时
\begin{align*}
\sum_{n=1}^\infty\frac{2^n}{z^n}+\sum_{n=0}^\infty\frac{z^n}{(2+i)^n}&=\frac{\dfrac 2z}{1-\dfrac 2z}+\frac1{1-\dfrac z{2+i}}=\frac{-iz}{(z-2)(z-2-i)}.
\end{align*}
\end{solution}
\end{frame}
...
这种做法会简便得多.
\onslide<+->
因此我们一般\alert{不用直接法}, 而是\abox{用双边幂级数的代数、求导、求积分运} \abox{算来得到洛朗级数}.
\begin{block}{另解}
\vspace{-\baselineskip}
...
\begin{frame}{典型例题: 求洛朗展开}
\begin{solutionc}
\enumnum3 由于 $\abs{\dfrac1z}<1,\abs{\dfrac2z}<1$, \onslide<+->
因此
\begin{align*}
f(z)&=\frac1{1-z}-\frac1{2-z}
...
故 $\displaystyle\oint_Cf(z)\diff z=2\pi i c_{-1}=0$.}
\end{solution}
\end{frame}
...
% \onslide<+->
% \begin{example}
% 求 $\displaystyle\oint_{|z|=2} \frac{z\exp\left(\frac1z\right)}{1-z}\diff z$.
% \end{example}
% \onslide<+->
% \begin{solution}
...
% \begin{align*}
% f(z)&=-\frac{\exp\left(\frac1z\right)}{1-\frac1z}
...
% \vspace{-0.3\baselineskip}
% \onslide<+->{
% 故 $\displaystyle\oint_Cf(z)\diff z=2\pi i c_{-1}=-4\pi i$.}
% \end{solution}
% \end{frame}
...
%     \indent \enumnumhk1 $0<|z|<1$; \hspace{2em} \enumnumhk2 $1<|z|<+\infty$.
%     \item(2020年B卷) 将函数 $f(z)=\dfrac1{(1-z)(z-2)}$ 分别在下列区域内展开成洛朗级数\\
%     \indent \enumnumhk1 $0<|z-1|<1$; \hspace{2em} \enumnumhk2 $2<|z|<+\infty$.
%     \item(2021年A卷) 将函数 $f(z)=\dfrac1{z^2+z-2}$ 分别在下列区域内展开成洛朗级数\\
%     \indent \enumnumhk1 $1<|z|<2$; \hspace{2em} \enumnumhk2 $0<|z-1|<1$.
