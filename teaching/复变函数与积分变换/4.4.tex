...
\begin{center}
\begin{tikzpicture}
\filldraw[cstcurve,draw=alecolor,cstfill] (0,0) circle (1.5);
\filldraw[cstdote,draw=alecolor] (0,0) circle;
\draw[notcolor,cstarrowto,thick] (0.048,0.036)--(1.2,0.9);
\draw
  (0.7,0.2) node[notcolor] {$R_2$}
  (-0.3,0) node[alecolor] {$z_0$}
  (0,-2) node {$0<|z-z_0|<R_2$};

\fill[cstfille,visible on=<+->] (2.5,-1.5) rectangle (5.5,1.5);
\filldraw[cstcurve,fill=white,draw=alecolor,visible on=<.->] (4,0) circle (1); 
\fill[cstdot,notcolor,visible on=<.->] (4,0) circle;
\draw[notcolor,cstarrowto,thick,visible on=<.->] (4,0)--(4.6,-0.8);
\draw
  (4.1,-0.6) node[notcolor,visible on=<.->] {$R_1$}
  (3.7,0) node[alecolor,visible on=<.->] {$z_0$}
  (4,-2) node[visible on=<.->] {$R_1<|z-z_0|<+\infty$};

\fill[cstfille,visible on=<+->] (6.5,-1.5) rectangle (9.5,1.5);
\filldraw[cstdote,draw=alecolor,visible on=<.->] (8,0) circle;
\draw
  (7.7,0) node[alecolor,visible on=<.->] {$z_0$}
...
\begin{solution}
\indent
由于 $\displaystyle\sum_{n=0}^\infty\frac{z^n}{b^n}$ 的收敛半径为 $|b|$, $\displaystyle\sum_{n=0}^\infty(az)^n$ 的收敛半径为 $\dfrac1{|a|}$,
\onslide<+->
因此该双边幂级数的收敛域为 $|a|<|z|<|b|$.
...
当 $|a|<|z|<|b|$ 时,
\begin{align*}
\sum_{n=1}^\infty\frac{a^n}{z^n}+\sum_{n=0}^\infty\frac{z^n}{b^n}&=\frac{a/z}{1-a/z}+\frac1{1-z/b}=\frac{(a-b)z}{(z-a)(z-b)}.\qedhere
\end{align*}
\end{solution}
\end{frame}
...
\onslide<+->
在圆环域 $1<|z|<+\infty$ 内,
\[f(z)=\frac1z-\frac1z\cdot\frac1{1-1/z}=-\frac1{z^2}-\frac1{z^3}-\frac1{z^4}-\cdots\]
\end{frame}
...
\begin{center}
\begin{tikzpicture}
\filldraw[cstcurve,cstfill,draw=notcolor] (0,0) circle (1.5);
\filldraw[cstcurve,fill=white,draw=alecolor] (0,0) circle (1);
\fill[cstdot,alecolor,visible on=<4->] (-1,0) circle;
\fill[cstdot,notcolor,visible on=<4->] (-1.48,-0.3) circle;
\fill[cstdot,black,visible on=<4->] (0,1.25) circle;
\draw[notcolor,cstarrowto,thick,visible on=<3->] (0,0)--(1.2,0.9);
\draw[alecolor,cstarrowto,thick,visible on=<3->] (0,0)--(0.6,-0.8);
\draw
  (-1.25,-0.15) node[visible on=<4->] {$\zeta$}
  (0.5,0.1) node[notcolor,visible on=<3->] {$R$}
  (0.1,-0.5) node[alecolor,visible on=<3->] {$r$}
  (-0.3,0) node {$z_0$}
  (0.8,-1) node[alecolor,visible on=<3->] {$K_1$}
  (1.4,1.2) node[notcolor,visible on=<3->] {$K_2$}
...
这种做法会简便得多.
\onslide<+->
因此我们一般\markatt{不用直接法}, 而是\markatt{用双边幂级数的代数、求导、求积分运算}来得到洛朗级数.
\begin{solution}[另解]
\vspace{-\baselineskip}
