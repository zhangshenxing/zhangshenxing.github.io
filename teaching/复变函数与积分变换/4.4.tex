...
\begin{frame}{典型例题: 求洛朗级数}
	...
	这种做法比直接计算积分更简便.
	\onslide<+->
	因此我们一般 \alert{不用直接法}, 而是\abox{用双边幂级数的代数、求导、求积分运算}来得到洛朗级数.
...
\begin{frame}{洛朗级数的特点\noexer}
	...
	\begin{itemize}
		...
		\item 有理函数在不同圆环域上的洛朗展开{\itshape 形式地相减}, 系数会有\emph{共同的通项形式} (形如 $\dfrac{P(n)}{\lambda^n}$ 的组合, 其中 $\lambda$ 是奇点, $P(n)$ 是多项式).
	\end{itemize}
\end{frame}
...
\begin{frame}{洛朗级数的特点\noexer}
	\onslide<+->
	例如 $f(z)=\dfrac{z^3-2z^2-z+3}{(z-1)(z-2)}=z+1+\dfrac1{z-2}-\dfrac1{z-1}$,
	\[f(z)=-\sum_{n<0}z^n+\half +\frac34z-\sum_{n\ge 2}2^{-n-1}z^n,\quad 1<|z|<2.\]
	\onslide<+->
	每个系数加 $1$ 得到
...
\begin{frame}{典型例题: 求洛朗展开}
	...
	\begin{solution}
		...
		\onslide<+->{因此当 $0<|z|<1$ 时,
			\[\resizet{1.2}{$\displaystyle f(z)=-\sum_{n=0}^\infty z^n+2\left(\sum_{n=0}^\infty z^n\right)'=-\sum_{n=0}^\infty z^n+2\sum_{n=1}^\infty nz^{n-1}
...
\begin{frame}{例题: 洛朗展开的应用}
	...
	\begin{solution}
		...
		\onslide<+->{我们在这个圆环域内求 $f(z)=\dfrac1{z(z+1)^2}$ 的洛朗展开.
			...
			\begin{align*}
			f(z)&=\frac1{z(z+1)^2}=\frac1{(z+1)^3}\cdot\frac1{1-\frac1{z+1}}\\
