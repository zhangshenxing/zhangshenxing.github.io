...
\begin{enumerate}
\item \alert{偏积分法}: 通过 \abox{$v_y=u_x$ 解得 $v=\varphi(x,y)+\psi(x)$}, 其中 $\psi(x)$ 待定.
\onslide<+->
再代入 \abox{$u_y=-v_x$ 中解出 $\psi(x)$}.
\item \alert{不定积分法}: 对 \abox{$f'(z)=u_x-iu_y=v_y+iv_x$ 求不定积分}得到 $f(z)$.
\end{enumerate}
\end{frame}
...
\begin{frame}[<*>]{作业}
	\begin{homeworks}
		\item(2020年A卷) 已知 $u(x,y)=x^3+kx^2y-3xy^2-2y^3$ 为调和函数, 求参数 $k$ 和 $v(x,y)$ 使得 $f(z)=u+iv$ 解析且 $f(z)=0$.
		\item(2020年B卷) 证明 $u(x,y)=y^3-3x^2y$ 为调和函数, 并求 $v(x,y)$ 使得 $f(z)=u+iv$ 解析且 $f(z)=0$.
		\item(2022年A卷) 已知 $v(x,y)=x^3+y^3-axy(x+y)$ 为调和函数, 求参数 $a$ 以及解析函数 $f(z)$ 使得 $v(x,y)$ 是它的虚部.
		\item(2021年B卷) 已知 $f(z)=x^2+2xy-y^2+i(y^2+axy-x^2)$ 为解析函数, 求参数 $a$ 和 $f'(z)$.
		\item(2021年A卷) 已知 $f(z)=y^3+ax^2y+i(bx^3-3xy^2)$ 为解析函数, $a,b$ 为实数, 求参数 $a,b$ 和 $f'(z)$.		
		\item(2021年A卷) 下列命题中, 正确的是\fillbrace
		\xx{设 $v_1,v_2$ 在区域 $D$ 内均为 $u$ 的共轭调和函数, 则必有 $v_1=v_2$}%
			{解析函数的实部是虚部的共轭调和函数}%
			{以调和函数为实部与虚部的函数是解析函数}%
