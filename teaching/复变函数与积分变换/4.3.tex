...
\onslide<+->
反过来, 解析函数是不是也一定可以在一点展开成幂级数呢? 也就是说是否存在\markdef{泰勒级数}展开?
...
\begin{center}
\begin{tikzpicture}
\filldraw[cstcurve,alecolor,smooth,cstfill] plot coordinates {(-2.25,0) (-1.5,-0.75) (0,-1.5) (1.05,-1.5) (1.35,0) (0,1.2) (-1.5,1.2) (-2.25,0)};
\draw[cstdash,defcolor,visible on=<2->] (0,0) circle (1);
\draw[cstcurve,notcolor,visible on=<3->] (0,0) circle (0.8);
\fill[cstdot,alecolor] (0,0) circle;
\fill[cstdot,defcolor,visible on=<2->] (0.4,0.4) circle;
\fill[cstdot,visible on=<3->] (0.6,-0.5) circle;
\draw[cstcurve,cstarrowto,alecolor,visible on=<3->] (0,0)--(-0.48,-0.64);
\draw
  (-0.25,0) node[alecolor] {$z_0$}
  (0.2,0.4) node[defcolor,visible on=<2->] {$z$}
  (0,-0.5) node[alecolor,visible on=<3->] {$r$}
...
\onslide<+->
由此可知, 解析函数在 $z_0$ 处\markatt{泰勒展开成立的圆域的最大半径是 $z_0$ 到最近奇点的距离}.
...
\onslide<+->
所以\markatt{解析函数的幂级数展开是唯一的}.
\onslide<+->
因此解析函数的泰勒展开不仅可以\marknot{直接求出各阶导数得到}, 也可以\marknot{利用幂级数的运算法则得到}.
\end{frame}
