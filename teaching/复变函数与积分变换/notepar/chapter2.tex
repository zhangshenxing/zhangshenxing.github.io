...
\begin{definition}
  ...
  \]
  存在, 则称 \nouns{$f(z)$ 在 $z_0$ 处可导}\index{kedao@可导}.
  这个极限值称为 \nouns{$f(z)$ 在 $z_0$ 处的导数}\index{daoshu@导数},记作 $f'(z_0)$.
  若 $f(z)$ 在区域 $D$ 内处处可导, 则称 \nouns{$f(z)$ 在 $D$ 内可导}\index{kedao@可导}.\footnotemark
\end{definition}
...
\begin{definition}
  ...
  \[
    ...
    =A\delt z+o(\delt z),
  \]
  其中 $o(\delt z)$ 表示 $\delt z$ 的高阶无穷小量, 则称 \nouns{$f(z)$ 在 $z_0$ 处可微}\index{kewei@可微}, 称 $A\delt z$ 为 \nouns{$f(z)$ 在 $z_0$ 的微分}\index{weifen@微分}, 记作 $\d w=A \delt z$.
\end{definition}
...
\begin{definition}
  \begin{enuma}
    \item 若 $f(z)$ 在 $z_0$ 的一个邻域内处处可导, 则称 \nouns{$f(z)$ 在 $z_0$ 处解析}\index{jiexi@解析}.
    \item 若 $f(z)$ 在区域 $D$ 内处处解析, 则称 $f(z)$ 在 $D$ 内解析, 或称 $f(z)$ 是 $D$ 内的一个\nouns{解析函数}\index{jiexi@解析}.\footnotemark
    \item 若 $f(z)$ 在 $z_0$ 处不解析, 则称 $z_0$ 为 $f(z)$ 的一个\nouns{奇点}\index{jidian@奇点}.
  \end{enuma}
\end{definition}

\footnotetext{也可叫\emph{全纯函数}\index{quanchunhanshu@全纯函数}或\emph{正则函数}\index{zhengzehanshu@正则函数}.}
...
\end{theorem}
\footnotetext{
  设 $g(w)$ 在 $\abs{z-z_0}<\eta$ 内处处可导, $f(w)$ 在 $\abs{w-g(z_0)}<\varepsilon$ 内处处可导.
  由于 $g(z)$ 在 $z_0$ 处连续, 因此存在 $\delta$, $0<\delta<\eta$, 使得当 $\abs{z-z_0}<\delta$ 时, $\abs{g(z)-g(z_0)}<\varepsilon$, 从而 $f\bigl(g(z)\bigr)$ 在 $z$ 处可导, 即在 $z_0$ 处解析.
}
...
\begin{theorem}[柯西-黎曼定理]
  ...
  \index{kexilimandingli@柯西-黎曼定理}
  $f(z)$ 在 $z=x+y\ii$ 处可导当且仅当 $u,v$ 在 $(x,y)$ 处可微, 且满足\nouns{柯西-黎曼方程}\index{kexilimanfangcheng@柯西-黎曼方程}:
  \[
...
\begin{people}[images/Cauchy.jpg]
...
柯西-黎曼方程可简称为 \nouns{C-R 方程}\index{crfangcheng@C-R 方程}.
...
\begin{definition}
  定义\nouns{指数函数}\index{zhishuhanshu@指数函数}\index{0ez@$\ee^z$}
  \[
    \exp z:=\ee^x(\cos y+\ii\sin y).
  \]
\end{definition}
...
由于 $\abs{\ee^{x+\ii y}}=\ee^x$, 因此 $\liml_{x\ra-\infty}\ee^{x+\ii y}=0$, $\liml_{x\ra+\infty}\ee^{x+\ii y}=\infty$.
...
\begin{definition}
  \begin{enuma}
    \item 定义\nouns{对数函数}\index{duishuhanshu@对数函数}\index{0lnz@$\Ln z$}
      \[
        \Ln z=\ln\abs{z}+\ii\Arg z.
      \]
      它是一个多值函数.
    \item 定义\nouns{对数函数主值}\index{duishuhanshu@对数函数!duishuhanshuzhuzhi@对数函数主值}\index{0lnz@$\ln z$}
      \[
        \ln z=\ln\abs{z}+\ii\arg z.
      \]
  \end{enuma}
...
\begin{definition}
  \begin{enuma}
    \item 设 $a\neq 0$, $z\neq 0$, 定义\nouns{幂函数}\index{mihanshu@幂函数}\index{0za@$z^a$}
      \[
        w=z^a=\ee^{a\Ln z}
        =\exp(a\ln\abs{z}+\ii a\Arg z).
      \]
    \item \nouns{幂函数的主值}\index{mihanshu@幂函数!mihanshudezhuzhi@幂函数的主值}为
      \[
...
\begin{definition}
  定义\nouns{余弦函数}\index{yuxianhanshu@余弦函数}\index{0cosz@$\cos z$}和\nouns{正弦函数}\index{zhengxianhanshu@正弦函数}\index{0sinz@$\sin z$}
  \[
...
于是欧拉恒等式\index{oulahengdengshi@欧拉恒等式} \alert{$\ee^{\ii z}=\cos z+\ii\sin z$ 对任意复数 $z$ 均成立}.
由定义可知
\[
...
\begin{itemize}[addsep]
  \item \nouns{正切函数}\index{zhengqiehanshu@正切函数}\index{0tanz@$\tan z$} $\tan z=\dfrac{\sin z}{\cos z},\ z\neq\bigl(k+\dfrac12\bigr)\cpi$;
  \item \nouns{余切函数}\index{yuqiehanshu@余切函数}\index{0cotz@$\cot z$} $\cot z=\dfrac{\cos z}{\sin z},\ z\neq k\cpi$;
  \item \nouns{正割函数}\index{zhenggehanshu@正割函数}\index{0secz@$\sec z$} $\sec z=\dfrac{1}{\cos z},\ z\neq\bigl(k+\dfrac12\bigr)\cpi$;
  \item \nouns{余割函数}\index{yugehanshu@余割函数}\index{0cscz@$\csc z$} $\csc z=\dfrac{1}{\sin z},\ z\neq k\cpi$,
\end{itemize}
其中 $k$ 是整数.
...
类似的, 我们可以定义双曲函数:
\begin{itemize}[addsep]
  \item \nouns{双曲正弦函数}\index{shuangquzhengxianhanshu@双曲正弦函数}\index{0chz@$\ch z$} $\ch z=\dfrac{\ee^z+\ee^{-z}}2=\cos \ii z$;
  \item \nouns{双曲余弦函数}\index{shuangquyuxianhanshu@双曲余弦函数}\index{0shz@$\sh z$} $\sh z=\dfrac{\ee^z-\ee^{-z}}2=-\ii\sin \ii z$;
  \item \nouns{双曲正切函数}\index{shuangquzhengqiehanshu@双曲正切函数}\index{0thz@$\tanh z$} $\tanh z=\dfrac{\ee^z-\ee^{-z}}{\ee^z+\ee^{-z}}=-\ii\tan \ii z,\ z\neq \bigl(k+\dfrac12\bigr)\cpi\ii, k\in\BZ$.
\end{itemize}
...
\]
因此\nouns{反余弦函数}\index{fanyuxianhanshu@反余弦函数}\index{0arccosz@$\Arccos z$}为 
\[
...
\begin{itemize}[addsep]
  \item \nouns{反正弦函数}\index{fanzhengxianhanshu@反正弦函数}\index{0arcsinz@$\Arcsin z$} $\Arcsin z=-\ii\Ln(\ii z+\sqrt{1-z^2})$;
  \item \nouns{反正切函数}\index{fanzhengqiehanshu@反正切函数}\index{0arctanz@$\Arctan z$} $\Arctan z=-\dfrac \ii 2\Ln\dfrac{1+\ii z}{1-\ii z},\ z\neq \pm i$;
  \item \nouns{反双曲余弦函数}\index{fanshuangquyuxianhanshu@反双曲余弦函数}\index{0archz@$\Arch z$} $\Arch z=\Ln(z+\sqrt{z^2-1})$;
  \item \nouns{反双曲正弦函数}\index{fanshuangquzhengxianhanshu@反双曲正弦函数}\index{0arshz@$\Arsh z$} $\Arsh z=\Ln(z+\sqrt{z^2+1})$;
  \item \nouns{反双曲正切函数}\index{fanshuangquzhengqiehanshu@反双曲正切函数}\index{0arthz@$\Arth z$} $\Arth z=\dfrac12\Ln\dfrac{1+z}{1-z},\ z\neq \pm1$.
\end{itemize}
...
称分子次数小于分母次数的有理函数为\nouns{真分式}\index{zhengfenshi@真分式}.
任何一个有理函数 $f(z)$ 都可以通过带余除法分解为一个多项式 $g(z)$ 和一个真分式之和.
若这个有理函数分母的零点均能求出, 则这个真分式又可以分拆为部分分式之和, 其中\nouns{部分分式}\index{bufenfenshi@部分分式}是指形如 $\dfrac{a}{(x-b)^k}$ 的真分式.
我们来介绍求这种分拆的一种方法.
设
\[
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}
      \fill[cstfille1] (-\w,-\w) rectangle (\w,\w);
      \cutline{-\u}{0}{\w-\u}{180}{main};
      \cutline{\u}{0}{\w-\u}{0}{main};
      \draw[cstaxis] (-\w,0)--(\w,0);
      \draw[cstaxis] (0,-\w)--(0,\w);
    \end{scope}
    \begin{scope}[shift={(5,0)}]
      \fill[cstfille1] (-\w,-\w) rectangle (\w,\w);
      \cutline{-\u}{0}{\w}{90}{main};
      \cutline{\u}{0}{\w}{-90}{main};
      \draw[cstaxis] (-\w,0)--(\w,0);
      \draw[cstaxis] (0,-\w)--(0,\w);
    \end{scope}
