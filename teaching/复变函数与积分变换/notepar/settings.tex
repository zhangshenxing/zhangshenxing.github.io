...
% \usepackage{stmaryrd}
% URL断行
\def\UrlBreaks{\do\/\do-\do.\do\a\do\b\do\c\do\w\do1\do7}
% 三线表列间距
\tabcolsep=6mm
% 公式间距
...
\newcommand{\tinyskip}{\vspace{3pt plus 2pt minus 2pt}}
% 选读
...
\tikzset{
  mpscale/.style    ={scale=1}, % (marginparwidth-0.1)/4
...
\newcommand{\qrcode}[3]{
  ...
  \begin{tikzpicture}
    \draw
      (0,1.6) node {\includegraphics[height=2cm]{#1}}
      (0,.4) node {\footnotesize{#2}}
      (0,0) node {\footnotesize{#3}};
  \end{tikzpicture}
\end{center}}
