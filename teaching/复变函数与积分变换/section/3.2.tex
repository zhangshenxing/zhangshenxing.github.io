...
\begin{frame}{积分路径无关与闭路积分}
	...
	\begin{center}
		\begin{tikzpicture}
			...
			\draw[cstcurve,second,cstwla,visible on=<2->] ({1+0.3*cos(135)},{1+0.3*sin(135)}) arc(135:-110:0.3);
		\end{tikzpicture}
	\end{center}
\end{frame}
...
\begin{frame}{多连通域边界与复合闭路}
	...
	\begin{center}
		\begin{tikzpicture}
			...
			\draw[cstcurve,main] (-1,0) circle(0.6);
			\draw[cstcurve,main,domain=85:90,cstwra,visible on=<2->] plot ({2.5*cos(\x)}, {1.5*sin(\x)});
