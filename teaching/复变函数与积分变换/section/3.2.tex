...
\begin{frame}{积分路径无关与闭路积分}
	...
	\onslide<+->
	这里, 如果闭合曲线 $C$ 不是闭路(有自相交的点), 也可以拆分为一些闭路的并.
	\onslide<1->
	\begin{center}
		\begin{tikzpicture}
			\coordinate (O);
...
\begin{frame}{柯西-古萨定理: 推导}
	...
	\[
		\oint_Cf(z)\diff z
		=\oint_C(u\diff x-v\diff y)+\ii\oint_C(v\diff x+u\diff y).
	\]
	\onslide<+->
	由格林公式\onslide<+->{和C-R方程可知}
	\[
		\oint_Cf(z)\diff z
		=-\iint_D(v_x+u_y)\diff x\diff y
		+\ii\iint_D(u_x-v_y)\diff x\diff y\onslide<7->{=0.}
	\]
	\onslide<+->
	也可以从
	\[
		\oint_Cf(z)\diff z
		=-\iint_D\frac{\partial f}{\partial \ov z}\diff z\diff \ov z
		=2\ii\iint_D\frac{\partial f}{\partial \ov z}\diff x\diff y=0
	\]
	看出.
\end{frame}
...
\begin{frame}{柯西-古萨定理}
	...
	设 $f(z)$ 在闭路 $C$ 上连续, $C$ 内部解析, 则 $\doint_Cf(z)\diff z=0$.
	\end{theorem*}

	\onslide<+->
	\begin{corollary}
	设 $f(z)$ 在\alert{单连通区域} $D$ 内解析, $C$ 是 $D$ 内一条闭合曲线, 则 $\doint_Cf(z)\diff z=0$.
	\end{corollary}
	\onslide<+->
	这里的闭合曲线可以不是闭路.
\end{frame}
...
\begin{frame}{典型例题: 柯西-古萨定理计算积分}
	...
	\begin{example}[nearnext]
		求 $\doint_{|z|=1}\frac1{2z-3}\diff z$.
	\end{example}
	\onslide<+->
	\begin{solution}[nearprev]
		由于 $\dfrac1{2z-3}$ 在 $|z|\le 1$ 上解析,
		\onslide<+->{%
			因此由柯西-古萨定理 $\doint_{|z|=1}\frac1{2z-3}\diff z=0$.
		}
	\end{solution}
	\onslide<+->
	\begin{exercise}
		\begin{enumerate}
			\item $\doint_{|z-2|=1}\frac1{z^2+z}\diff z=$\fillblankframe{$0$}.
			\item $\doint_{|z|=2}\dfrac{\sin z}{|z|}\diff z=$\fillblankframe{$0$}.
		\end{enumerate}
	\end{exercise}
\end{frame}
...
\begin{frame}{例题: 柯西-古萨定理计算积分}
	\onslide<+->
	\begin{example}
		求 $\doint_C\frac1{z(z^2+1)}\diff z$, 其中 $C:|z-\ii |=\dfrac12$.
	\end{example}

	\onslide<+->
	\begin{solution}
		注意到 $\dfrac1{z(z^2+1)}=\dfrac1z-\dfrac12\Bigl(\dfrac1{z+\ii}+\dfrac1{z-\ii }\Bigr)$.
		\onslide<+->{由于 $\dfrac1z,\dfrac1{z+\ii}$ 在 $|z-\ii |\le\dfrac12$ 上解析,
		}\onslide<+->{因此由柯西-古萨定理
			\[\oint_C\frac1z\diff z
			=\oint_C\frac1{z+\ii}\diff z=0,
	\]
		}\onslide<+->{
			\[\oint_C\frac1{z(z^2+1)}\diff z
			=-\half\oint_C\frac1{z-\ii }\diff z=-\pi\ii.\]}
	\end{solution}
\end{frame}
...
\begin{frame}{复合闭路定理}
	...
	\onslide<+->
	设 $C$ 是一个闭路或复合闭路, 若 $f(z)$ 在 $C$ 及其围成的区域(单连通或多连通)内解析, 则 $\doint_Cf(z)\diff z=0$.
\end{frame}
...
\begin{frame}{连续变形定理}
	\onslide<+->
	此外, 从复合闭路定理还可以看出, 在计算积分 $\doint_C f(z)\diff z$ 时, $C$ 的具体形状无关紧要, 只要其内部奇点不变, $C$ 可以任意变形.
	\onslide<+->
...
\begin{frame}{复合闭路定理}
	...
	\begin{proof}
		...
		\onslide<+->{%
			...
			因此
			\[
				\oint_{C_0}f(z)\diff z-\oint_{C_1}f(z)\diff z-\cdots-\oint_{C_n}f(z)\diff z=0.
			\]
		}\onslide<+->{%
			于是定理得证.\qedhere
		}\bigdel
...
\begin{frame}{例题: 复合闭路定理的应用}
	\onslide<+->
	\begin{example}[nearnext]
		证明对于任意闭路 $C$, $\dint_C(z-a)^n\diff z=0$, $n\neq -1$ 为整数.
	\end{example}
	\onslide<+->
...
\begin{frame}{例题: 复合闭路定理的应用}
	...
	\begin{theorem}
		当 $a$ 在 $C$ 的内部时,
	\[
		\oint_C\frac{\diff z}{(z-a)^{n+1}}=\begin{cases}
			2\pi\ii,&n=0;\\
			0,&n\neq 0.
		\end{cases}
	\]
...
\begin{frame}{例题: 复合闭路定理的应用}
	...
	\begin{example}[nearnext,sidepic,righthand width=144pt]
		求 $\dint_\Gamma\frac{2z-1}{z^2-z}\diff z$, 其中 $\Gamma$ 是由 $2\pm \ii,-2\pm \ii$ 形成的矩形闭路.
		\tcblower
		\begin{center}
			\begin{tikzpicture}
...
\begin{frame}{例题: 复合闭路定理的应用}
	...
	\begin{example}[nearnext,sidepic,righthand width=94pt]
		求 $\dint_\Gamma\frac{\ee^z}z\diff z$, 其中 $\Gamma=C_1+C_2^-$, $C_1:|z|=2, C_2:|z|=1$.
		\tcblower
		\begin{center}
	...
	\begin{solution}[nearprev]
		...
		\onslide<+->{由复合闭路定理可知 $\dint_\Gamma\frac{\ee^z}z\diff z=0$.}
	\end{solution}
\end{frame}
