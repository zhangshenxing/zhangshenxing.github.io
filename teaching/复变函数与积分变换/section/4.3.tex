...
\begin{frame}{典型例题: 泰勒展开的计算}
	...
	\begin{example}
		...
		\onslide<+->{因此
	\[
		\emphm{\ee^z=1+z+\frac{z^2}{2!}+\frac{z^3}{3!}+\cdots=\sum_{n=0}^\infty\frac{z^n}{n!},\quad\forall z.}\]}
...
\begin{frame}{典型例题: 泰勒展开的计算}
	...
	\begin{example}
		...
		\begin{align*}
			...
			\visible<+->{=\sum_{n=0}^\infty\frac{(iz)^n-(-\ii z)^n}{2\ii\cdot n!}}\\
	...
	\begin{solution}
		...
		\begin{align*}
			\frac1{3z-2}&=-\half\cdot\frac1{1-\dfrac{3z}2}
				\visible<+->{=-\half\sum_{n=0}^\infty\Bigl(\frac{3z}2\Bigr)^n}\\
...
\begin{frame}{典型例题: 泰勒展开的计算\noexer}
	...
	\]
	\onslide<+->
	\[a=\frac1{2\pi\ii} \oint_{|z-1|=0.1}f(z)\diff z=\dfrac{1}{(z-2)(z+2)}\Big|_{z=1}=-\frac13.
	\]
	\onslide<+->
...
\begin{frame}{典型例题: 泰勒展开的计算\noexer}
	...
	\]
	\onslide<+->
	\[a=\frac1{2\pi\ii} \oint_{|z-1|=0.1}f(z)\diff z=\Bigl(\dfrac{1}{(z-2)^3}\Bigr)'\Big|_{z=1}=-3,
	\]
	\onslide<+->
	\[b=\frac1{2\pi\ii} \oint_{|z-1|=0.1}(z-1)f(z)\diff z=\dfrac{1}{(z-2)^3}\Big|_{z=1}=-1.
	\]
	\onslide<+->
	类似可得 $c=3,d=-2,e=1$, 只是我们需要计算高阶导数.
	\onslide<+->
	因此
	\[f(z)=\sum_{n=0}^\infty\Bigl(3-(n+1)+\frac{-\frac32-\frac12(n+1)-\frac18(n+2)(n+1)}{2^n}\Bigr)z^n,\quad |z|<1.
	\]
\end{frame}
