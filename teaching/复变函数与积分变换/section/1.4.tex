...
\begin{frame}{连续曲线、闭路}
	\begin{itemize}
		\item 设 $x(t),y(t),t\in[a,b]$ 是两个连续函数.
		\item 参变量方程 $\begin{cases}
			x=x(t),& \\y=y(t),&
		\end{cases}t\in[a,b]$ 定义了一条\emph{连续曲线}.
		\item 这也等价于 $C:z=z(t)=x(t)+\ii y(t),t\in[a,b]$.
...
\begin{frame}{例: 连续曲线}
	\beqskip{3pt}
	\onslide<+->
	\begin{example}[near,sidepic,righthand width=4.6cm,leftupper=0mm]
		\begin{itemize}
			\item 直线段:
			\[
			...
			\]
			是简单曲线.
			\item 圆 $\abs{z-z_0}=R$ 的参数方程:
			\[
				z=z_0+R\ee^{\ii\theta},\quad\theta\in[0,2\pi].
			\]
			\item 椭圆 $\abs{z-\sqrt5}+\abs{z+\sqrt5}=6$ 的参数方程: 
			\[
				z=3\cos\theta+2\ii\sin\theta,\quad \theta\in[0,2\pi].
			\]
			\item 双纽线 $\abs{z^2-1}=1$ 是闭合曲线, 但不是闭路.
		\end{itemize}
		\meddel
		\tcblower\small
		\onslide<2->
		\begin{tikzpicture}
			\begin{scope}[scale=.7]
				...
				\end{scope}
				\begin{scope}[shift={(1,-1.7)},visible on=<3->]
				...
				\end{scope}
				\begin{scope}[shift={(0,-4)},visible on=<5->]
					\begin{scope}
				...
				\end{scope}
				\begin{scope}[shift={(3,-5.5)},visible on=<6->]
			...
			\begin{scope}[cstdot,fourth]
				...
				\fill[visible on=<3->] (C) circle node[below right] {$z(0)$};
				\fill[visible on=<3->] (D) circle node[above] {$z(2\pi)$};
				\fill[visible on=<5->] (E) circle;
				\fill[visible on=<5->] (F) circle;
			\end{scope}
		\end{tikzpicture}
	\end{example}
	\endgroup
...
\begin{frame}{邻域}
	\begin{itemize}
		\item 为了引入极限的概念, 我们需要考虑点的邻域.
		\item 类比于高等数学中的邻域和去心邻域, 我们在复变函数中, 称开圆盘
		\[
...
\begin{frame}{内部、外部、边界}
	\begin{itemize}
		...
		\end{enumerate}
		\item 内点都属于 $G$, 外点都不属于 $G$, 而边界点则都有可能.
		\item 这类比于区间的端点和区间的关系.
	\end{itemize}
	\onslide<1->
	\begin{center}
...
\begin{frame}{区域和闭区域}
	...
	\end{definition}
	\begin{itemize}
		\item 观察如下图案, 阴影部分(不包含线条部分)中任意两点可用折线连接, 因此它是一个区域.
		\onslide<3->
		\begin{tikzpicture}[scale=.8,overlay,shift={(11.5,-1.5)}]
		...
		\end{tikzpicture}
		\item 这些线条和点构成了它的边界.
		\item 区域和它的边界一起构成了\emph{闭区域}, 记作 $\ov D$.
		\item 它是一个闭集.
		\item 注意数学中边界的概念与日常所说的边界是两码事.
		\item 例如区域 $|z|>1$ 的边界是 $|z|=1$, 其闭区域是 $|z|\ge 1$.
	\end{itemize}
\end{frame}
...
\begin{frame}{闭路的内部和外部}
	\begin{itemize}
		\item 闭路 $C$ 把复平面划分成了两个区域, 一个有界一个无界.
		\item 分别称这两个区域是 $C$ 的\emph{内部}和\emph{外部}.
		\item $C$ 是它们的公共边界.
	\end{itemize}
	\onslide<1->
	\begin{center}
...
\begin{frame}{单连通区域和多连通区域}
	\begin{itemize}
		\item 在前面所说的几个常见区域的例子中, 我们在区域中画一条闭路.
		\item 除了圆环域之外, 闭路的内部仍然包含在这个区域内.
	\end{itemize}
	\onslide<+->
	\begin{definition}
	...
	\end{definition}
	\begin{itemize}
		\item 单连通区域内的任一闭路可以``连续地变形''成一个点.
		\item 这也等价于: 设 $\ell_0,\ell_1$ 是从 $A$ 到 $B$ 的两条连续曲线, 则 $\ell_0$ 可以连续地变形为 $\ell_1$ 且保持端点不动.
	\end{itemize}
	\onslide<4->
	\bigdel
	\begin{center}
...
\begin{frame}{例: 区域的特性}
	\onslide<+->
	\begin{example}[near,sidepic,righthand width=3.3cm,leftupper=0mm]
		\begin{itemize}
			\item $\Re(z^2)<1$.
			\item 设 $z=x+y\ii$, 则 $\Re(z^2)=x^2-y^2<1$.
			\item 这是无界的单连通区域.
		\end{itemize}
		\tcblower
		\onslide<3->{%
...
\begin{frame}{例: 区域的特性}
	...
	\begin{example}[near,sidepic,righthand width=3.3cm,leftupper=0mm]
		\begin{itemize}
			\item $\Bigabs{\dfrac1z}\le3$.
			\item 即 $|z|\ge\dfrac13$.
			\item 这是无界的多连通\alertn{闭区域}.
		\end{itemize}
		\tcblower
		\onslide<3->{%
		\begin{tikzpicture}[scale=.8]
