...
\begin{frame}{典型例题: 柯西积分公式的应用}
	...
	\begin{solution}
		...
		\onslide<+->{设 $C_1,C_2,C_3$ 分别为绕 $0,1,-1$ 的分离圆周.
			...
			\end{flalign*}}
		\begin{tikzpicture}[overlay]\begin{tikzpicture}[framed,xshift=-70mm,yshift=-25mm]
			\draw[cstaxis] (-2.1,0)--(2.1,0);
			\draw[cstaxis] (0,-2)--(0,2.1);
			\draw[cstcurve,dcolorb] (0,0) circle (1.8);
			\draw[cstcurve,dcolora,visible on=<3->] (1.2,0) circle(0.5);
			\draw[cstcurve,dcolora,visible on=<3->] (-1.2,0) circle(0.5);
			\draw[cstcurve,dcolora,visible on=<3->] (0,0) circle(0.5);
			\fill[cstdot,dcolorb,visible on=<2->] (0,0) circle;
			\fill[cstdot,dcolorb,visible on=<2->] (1.2,0) circle;
			\fill[cstdot,dcolorb,visible on=<2->] (-1.2,0) circle;
			\draw
				(1.6,1.6) node[dcolorb] {$C$}
				(-0.4,0.8) node[dcolora,visible on=<3->] {$C_1$}
				(1.2,-.8) node[dcolora,visible on=<3->] {$C_2$}
				(-1.2,-.8) node[dcolora,visible on=<3->] {$C_3$};
