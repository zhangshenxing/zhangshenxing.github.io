	...
	\begin{solution}
		...
		\onslide<+->{%
			...
			\begin{flalign*}
				...
				\visible<+->{=}&\visible<.->{2\pi i\left[\frac{e^z}{z^2-1}\bigg|_{z=0}+\frac{e^z}{z(z+1)}\bigg|_{z=1}+\frac{e^z}{z(z-1)}\bigg|_{z=-1}\right]}&&\\
				\visible<+->{=}&\visible<.->{2\pi i\left(-1+\frac e2+\frac{e^{-1}}2\right)=\pi i(e+e^{-1}-2).&&}
			\end{flalign*}
		}
		\begin{tikzpicture}[overlay,scale=.7,xshift=160mm,yshift=30mm]
			...
			\draw
				...
				(-0.4,0.8) node[main,visible on=<3->] {$C_1$}
				(1.2,-.8) node[main,visible on=<3->] {$C_2$}
				(-1.2,-.8) node[main,visible on=<3->] {$C_3$};
		\end{tikzpicture}
		\vspace{-\baselineskip}
	\end{solution}
\end{frame}
