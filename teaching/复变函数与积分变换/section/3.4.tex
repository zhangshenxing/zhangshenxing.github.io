...
\begin{frame}{柯西积分公式: 证明}
	...
	\begin{proof}
		...
		\onslide<+->{%
			...
			由 $\varepsilon$ 的任意性可知 
			$\doint_C\frac{f(z)}{z-z_0}\diff z=2\pi\ii f(z_0)$.\qedhere
		}
	\end{proof}
\end{frame}
...
\begin{frame}{典型例题: 柯西积分公式的应用}
	\onslide<+->
	\begin{example}
		求 $\doint_{|z|=3}\frac{\ee^z}{z(z^2-1)}\diff z$.
	\end{example}
	\onslide<+->
	\begin{solution}
...
\begin{frame}{高阶导数的柯西积分公式}
	...
	\]
	\onslide<+->
	那么由 $\doint_C \frac{\diff z}{(z-z_0)^n}$ 的性质可知上述公式右侧应当为 $f^{(n)}(z_0)$.
\end{frame}
...
\begin{frame}{典型例题: 使用高阶导数的柯西积分公式计算积分}
	...
	\onslide<+->
	\begin{example}
		求 $\doint_{|z|=2}\frac{\cos(\pi z)}{(z-1)^5}\diff z.$
	\end{example}
...
\begin{frame}{典型例题: 使用高阶导数的柯西积分公式计算积分}\small
	...
	\onslide<+->
	\begin{example}
		求 $\doint_{|z|=2}\frac{\ee^z}{(z^2+1)^2}\diff z.$
	\end{example}
	\onslide<+->
	\begin{solution}
	...
	\]
			...
			类似地, $\doint_{C_2}\frac{\ee^z}{(z^2+1)^2}\diff z=\frac{-(1+\ii)\ee^{-\ii }\pi}2$.
		}\onslide<+->{%
			故
			\begin{align*}
...
\begin{frame}{典型例题: 使用高阶导数的柯西积分公式计算积分}
	\onslide<+->
	\begin{example}
		求 $\doint_{|z|=1}z^n\ee^z\diff z$, 其中 $n$ 是整数.
	\end{example}
...
\begin{frame}{典型例题: 使用高阶导数的柯西积分公式计算积分}\small
	...
	\onslide<+->
	\begin{example}
		求 $\doint_{|z-3|=2}\frac1{(z-2)^2z^3}\diff z$ 和 $\doint_{|z-1|=2}\frac1{(z-2)^2z^3}\diff z$.
	\end{example}
	\onslide<+->
	\begin{solution}
		\begin{enumerate}
...
\begin{frame}{莫累拉定理}
	...
	\onslide<+->
	\begin{exercise}
		$\doint_{|z-2\ii|=3}\frac1{z^2(z-\ii )}\diff z=$\fillblankframe{$0$}.
	\end{exercise}
	\onslide<+->
	\begin{example}[莫累拉定理]
		设 $f(z)$ 在单连通区域 $D$ 内连续, 且对于 $D$ 中任意闭路 $C$ 都有 $\doint_Cf(z)\diff z=0$, 则 $f(z)$ 在 $D$ 内解析.
	\end{example}
	\onslide<+->
	\begin{proof}
		由题设可知 $f(z)$ 的积分与路径无关.
