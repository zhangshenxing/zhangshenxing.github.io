	...
	\begin{proof}
		\begin{center}
			\begin{tikzpicture}[scale=.8]
				...
				\draw[cstcurve,main,cstarrow1to] (3,-0.2)--(3,0.1);
				\filldraw[cstcurve,white,draw=second] (-2,0.2) circle (0.5);
				\draw[cstcurve,second,cstarrow1to] (-2,0.7) arc(90:135:0.5);
				\fill[cstdot] (-2,0.2) circle;
				\filldraw[cstcurve,white,draw=second] (0.2,-0.2) circle (0.5);
				\draw[cstcurve,second,cstarrow1to] (0.2,0.3)arc(90:135:0.5);
...
\begin{tikzpicture}[node distance=25pt]
	\node at (4.55,0.3) [cstnode2,visible on=<3->]	(end1){求出 $f(z)$ 的原函数 $F(z)$ 得到 $\displaystyle\int_Cf(z)\diff z=F(b)-F(a)$};
...
可能需要分段计算};
	\node at (9,2.2) [cstnode2,align=center, visible on=<9->]	(end4){用闭路外的孤立\\奇点的留数计算, \\最后取负号*};
	...
	\node at (9,6.5) [cstnode2,align=center, visible on=<8->]	(end3){用闭路内的孤立\\奇点的留数计算};

	\draw[cstnarrow,third,visible on=<2->] (integral.-160) -- (isclosed);
	\draw[cstnarrow,third,visible on=<3->] (isanalytic) -- node[left]{是} (end1.173);
	\draw[cstnarrow,third,visible on=<5->] (isclosed) -- node[above]{是} (issingle);
	\draw[cstnarrow,third,visible on=<7->] (issingle) -- node[above]{是} (ismany);
	\draw[cstnarrow,third,visible on=<3->] (isclosed) -- node[left]{否} (isanalytic);
	\draw[cstnarrow,third,visible on=<6->] (issingle) -- node[left]{否} (end2);
	\draw[cstnarrow,third,visible on=<4->] (isanalytic) -- node[above]{否} (end2);
	\draw[cstnarrow,third,visible on=<8->] (ismany) -- node[left]{闭路内的少} (end3);
	\draw[cstnarrow,third,visible on=<9->] (ismany) -- node[left]{闭路外的少} (end4);
\end{tikzpicture}
\end{center}
\end{frame}
