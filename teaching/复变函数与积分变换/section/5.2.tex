...
\begin{frame}{留数定理}
	\onslide<+->
	\begin{second}{复变函数积分计算方法 III: 留数定理}
...
可能需要分段计算};
	...
	\node at (9,6.3) [cstnode2,align=center, visible on=<8->]	(end3){用闭路内的孤立\\奇点的留数计算};
	...
	\draw[cstmra,fourth,visible on=<8->] (ismany) -- node[left]{闭路内的少} (end3);
	\draw[cstmra,fourth,visible on=<9->] (ismany) -- node[left]{闭路外的少} (end4);
\end{tikzpicture}
\end{center}
\end{frame}
