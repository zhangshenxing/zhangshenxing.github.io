...
\begin{frame}{幂函数的性质: $a$ 为分数时}
	...
	\onslide<+->
	\[
		z^{\frac pq}=\abs{z}^{\frac pq}\exp\biggl(\frac{\ii p(\arg z+2k\pi)}q\biggr),\quad k=0,1,\dots,q-1
	\]
	具有 $q$ 个值.
	\onslide<+->
...
\onslide<+->
	...
	\begin{center}
		\begin{tabular}{cccc} \topcolorrule
			...
			&&$n\ge0$ 时处处解析\\
			\multirow{-2}*{整数 $n$}&\multirow{-2}*{单值}&$n<0$ 时除零点外解析\\ \midcolorrule
