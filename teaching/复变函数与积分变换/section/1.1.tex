...
\begin{frame}{复平面}
	...
	将\emph{全体复数记作 $\BC$}.
	\onslide<+->
	那么 $\BC$ 自然构成一个二维实线性空间, 且 $\{1,i\}$ 是一组基. 
	\onslide<+->
...
\begin{frame}{实部和虚部, 虚数和纯虚数}
	...
	\begin{itemize}
		...
		\item 称 $z=x+yi$ 中 $x=\Re z$ 为 $z$ 的\emph{实部}; $y=\Im z$ 为 $z$ 的\alert{虚部}.
		\item 当虚部 $\Im z=0$ 时, $z$ 为实数, 它落在实轴上.
		\item 不是实数的复数是\textcolor{third}{虚数}.
	...
	\begin{figure}[hbpt]
		...
		\begin{minipage}{.48\textwidth}
			...
			\begin{tikzpicture}
				...
				\coordinate (I) at (0,.8);
				\draw (R) node[align=center,main,visible on=<4->] {实数 \\$0,1,\sqrt2,\pi,e$};
				\draw (I) node[align=center,second,visible on=<6->] {纯虚数 \\$i,-i,\pi i$};
				\draw[cstcurve,second,visible on=<6->] (I) circle (1.2 and .7);
				\draw 
					(3.7,0) node[align=center] {全\\体\\复\\数}
					(2,0) node[align=center,third,visible on=<5->] {虚数 \\$i,\pi i,\frac{-1+\sqrt 3 i}2$};
			\end{tikzpicture}
		\end{minipage}
	\end{figure}
\end{frame}
...
\begin{frame}{例题：判断实数和纯虚数}
	...
	\onslide<+->
	\begin{exercise}
		若 $x^2(1+i)+x(5+4i)+4+3i$ 是纯虚数, 则实数 $x=$\fillblankframe{$-4$}.
	\end{exercise}
\end{frame}
...
\begin{frame}{复数的乘除法}
	\onslide<+->
	\alert{规定 $i\cdot i=-1$} 并定义复数的乘法:
	\onslide<+->
	\begin{align*}
		z_1\cdot z_2&=(x_1+y_1i)(x_2+y_2i)=x_1\cdot x_2+x_1\cdot y_2i+y_1i\cdot x_2+y_1i\cdot y_2i\\
		&=(x_1x_2-y_1y_2)+(x_1y_2+x_2y_1)i,
	\end{align*}
	\onslide<+->
	由此可得复数的除法:
	\[\frac{z_1}{z_2}=\frac{(x_1+y_1i)(x_2-y_2i)}{x_2^2+y_2^2}=\frac{x_1x_2+y_1y_2}{x_2^2+y_2^2}+\frac{x_2y_1-x_1y_2}{x_2^2+y_2^2}i.\]
