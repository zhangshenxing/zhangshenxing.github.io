...
\begin{frame}{复数与解方程\noexer}
	...
	配方可得
	\[
		\Bigl(x+\dfrac b2\Bigr)^2=\dfrac{b^2-4c}4.
	\]
	\onslide<+->
	于是得到求根公式 $x=\dfrac{-b\pm\sqrt\Delta}2$, 其中 $\Delta=b^2-4c$.
	\begin{enumerate}
...
\begin{frame}{例题: 三次方程求解(三个实根)\noexer}
	...
	\end{example}
	\onslide<+->
	\begin{solution}[near,sidepic,righthand width=2.9cm,indent]
		类似地 $x=u+v$, 其中
		\[
			u^3+v^3=-6,\qquad uv=\frac 73.
		\]
		\onslide<+->{%
			于是 $u^3,v^3$ 满足一元二次方程 $X^2+6X+343/27=0$.
		}
		
		\onslide<+->{%
			该方程无实数解, 我们可以强行解得 $u^3=-3+\dfrac{10}9\sqrt{-3}$.
		}\onslide<+->{%
			于是
			\[
				u=\dfrac{3+2\sqrt{-3}}3,\dfrac{-9+\sqrt{-3}}6,\dfrac{3-5\sqrt{-3}}6,
			\]%
		}\onslide<+->{%
			\[
				v=\dfrac{3-2\sqrt{-3}}3,\dfrac{-9-\sqrt{-3}}6,\dfrac{3+5\sqrt{-3}}6,
			\]%
		}\onslide<+->{%
			\[
				x=u+v=2,-3,1.
...
\begin{frame}{一般情形三次方程的根\noexer}
	\onslide<+->
	一般地, 方程 $x^3+px+q=0$ 的解为($p=0$ 情形较简单, 这里不考虑)
	\[
