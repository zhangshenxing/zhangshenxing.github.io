...
\begin{frame}{例题：共轭复数证明等式}
	...
	\begin{proof*}
		...
		\onslide<+->{但我们利用共轭复数可以更简单地证明它.}

		\onslide<+->{由于 $\ov{z_1\cdot\ov{z_2}}=\ov{z_1}\cdot\ov{\ov{z_2}}=\ov{z_1}\cdot z_2$, 
		}\onslide<+->{因此
...
\begin{frame}{例题：共轭复数判断实数}
	...
	\begin{proof}
		...
		}\onslide<+->{故上述等式等价于 $z\ov z=1$, 即 $x^2+y^2=1$.\qedhere}
	\end{proof}
\end{frame}
...
\begin{frame}[<*>]{例题: 复数的代数计算}
	...
	\begin{solution}
		...
		\end{align*}
		\onslide<+->{因此 $\ov{\left(\dfrac{z_1}{z_2}\right)}=-\dfrac75+\dfrac15i$.}
	\end{solution}
\end{frame}
