		...
		\begin{tikzpicture}
			\begin{scope}
				...
				\draw
					...
					(2.8,.3) node[dcolorb,visible on=<4->] {\small 一一对应}
			...
			\begin{scope}[xshift=4cm]
				...
				\draw[cstdash] (0,1.2)--(1.6,1.2);
				\fill[cstdot,dcolora] (1.6,1.2) circle;
				\draw[dcolorb,Latex-Latex,line width=.5mm,visible on=<5->] (2.2,.8)--(3.4,.8);
				\draw
					...
					(-.2,-.2) node {$O$}
					(1,-0.7) node {直角坐标系}
					(2.8,.3) node[dcolorb,visible on=<5->] {\small 一一对应};
			\end{scope}
			\begin{scope}[xshift=8cm,visible on=<5->]
				\draw[cstaxis] (-.4,0)--(2,0);
	...
	\begin{center}
		\begin{tikzpicture}
			\begin{scope}
				...
				\draw[visible on=<6->,dcolorb,thick] (-0.95,.2)--(0.7,.2)--(0.7,0)[->];
				\draw[visible on=<8->,dcolorc,->,thick] (-0.61,1.3)--(0,1.3);
				\draw 
					(1.6,1.4) node[dcolora] {$z=x+yi$}
					(-0.2,-0.2) node {$0$}
					(2.2,.4) node[dcolorb,visible on=<3->] {实轴}
					(0.6,1.9) node[dcolorc,visible on=<4->] {虚轴}
					(0.8,-0.3) node[visible on=<5>,dcolorb] {$\Re z$}
					(0.6,.6) node[visible on=<5>,dcolorc] {$\Im z$}
					(-1.51,.1) node[visible on=<6->,cstnode,draw=dcolorb,text=dcolorb] {实数}
			...
			\begin{scope}[xshift=1cm]
				...
				\draw 
					(4.8,.35) node[visible on=<6->,align=center] {\small 实数 \\$0,1,\sqrt2,\pi,e$}
					(8.5,1) node[visible on=<6->,align=center,dcolora] {全\\体\\复\\数}
...
\begin{frame}{复数的加法与减法}
	...
	\begin{align*}
		...
		\visible<+->{z_1-z_2}&\visible<.->{=(x_1-x_2)+(y_1-y_2)i.}
	\end{align*}
	\onslide<+->
