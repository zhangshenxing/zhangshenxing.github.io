...
\begin{frame}{典型例题: 利用原函数求积分}
	...
	\begin{solution}
		由于 $f(z)=z$ 处处解析,
		\onslide<+->{且 $\displaystyle\int z\diff z=\half  z^2+C$,
		}\onslide<+->{因此
...
\begin{frame}{典型例题: 利用原函数求积分}
	\onslide<+->
	\begin{exercise}
		求 $\displaystyle\int_0^1 z\sin z\diff z=$\fillblank[4cm]{\visible<+->{$\sin 1-\cos 1$}}.
	\end{exercise}
	\onslide<+->
	\begin{example}
		...
		\begin{center}
			\begin{tikzpicture}[framed]
				...
				\draw[cstcurve,main,smooth,domain=0:360,visible on=<9->] plot ({0.7*(pi/180*\x-sin(\x))}, {0.7*(1-cos(\x))});
				\draw[cstcurve,second,visible on=<3>] (0,0.7) circle (0.7);
				\fill[cstdot,main,visible on=<3->] (0,0) circle;
				\draw[cstcurve,second,visible on=<4>] ({0.7*(pi/180*60)},0.7) circle (0.7);
				\fill[cstdot,main,visible on=<4->] ({0.7*(pi/180*60-sin(60))},{0.7*(1-cos(60))}) circle;
				\draw[cstcurve,second,visible on=<5>] ({0.7*(pi/180*120)},0.7) circle (0.7);
				\fill[cstdot,main,visible on=<5->] ({0.7*(pi/180*120-sin(120))},{0.7*(1-cos(120))}) circle;
				\draw[cstcurve,second,visible on=<6>] ({0.7*(pi/180*180)},0.7) circle (0.7);
				\fill[cstdot,main,visible on=<6->] ({0.7*(pi/180*180-sin(180))},{0.7*(1-cos(180))}) circle;
				\draw[cstcurve,second,visible on=<7>] ({0.7*(pi/180*240)},0.7) circle (0.7);
				\fill[cstdot,main,visible on=<7->] ({0.7*(pi/180*240-sin(240))},{0.7*(1-cos(240))}) circle;
				\draw[cstcurve,second,visible on=<8>] ({0.7*(pi/180*300)},0.7) circle (0.7);
				\fill[cstdot,main,visible on=<8->] ({0.7*(pi/180*300-sin(300))},{0.7*(1-cos(300))}) circle;
				\draw[cstcurve,second,visible on=<9>] ({0.7*(pi/180*360)},0.7) circle (0.7);
				\fill[cstdot,main,visible on=<9->] ({0.7*(pi/180*360-sin(360))},{0.7*(1-cos(360))}) circle;
			\end{tikzpicture}
		\end{center}
		\vspace{-.5\baselineskip}
	\end{example}
...
\begin{frame}{典型例题: 利用原函数求积分}
	...
	\begin{solution}
		...
		\vspace{-1.5\baselineskip}
			...
			&\visible<+->{=\half\left[\left(\ln\sqrt2+\frac\pi4i\right)^2-\ln^22\right]
