...
\begin{frame}{孤立奇点}
	...
	\begin{example}
		...
		\onslide<+->{因为 $\liml_{k\to+\infty} z_k=0$, 所以 $0$ 的任何一个去心邻域内都有奇点.
...
\begin{frame}{孤立奇点的分类}
	...
	\begin{center}
		...
		\begin{tabular}{ccc} \toprule
			孤立奇点类型&洛朗级数特点&$\liml_{z\to z_0}f(z)$\\ \toprule
...
\begin{frame}{例: 可去奇点}
	...
	\]
		...
		\onslide<+->{也可以从 $\liml_{z\to0}zf(z)=\sin 0=0$ 看出.
		}
	\end{example}
	...
	\]
		...
		\onslide<+->{也可以从 $\liml_{z\to0}zf(z)=\ee^0-1=0$ 看出.
		}
	\end{example}
\end{frame}
...
\begin{frame}{典型例题: 函数的极点}
	...
	\begin{example}
		...
		\onslide<+->{由于 $\liml_{z\to 0}z^2f(z)=1$, 因此 $0$ 是 $2$ 阶极点.
...
\begin{frame}{函数在 $\infty$ 的性态}
	...
	\begin{center}
		\arrayrulecolor{second}
		\begin{tabular}{ccc}\toprule
			$\infty$ 类型&洛朗级数特点&$\liml_{z\to\infty}f(z)$\\ \toprule
