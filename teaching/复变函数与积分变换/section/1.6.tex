...
\begin{frame}{数列的极限}
	...
	\end{definition}
	\onslide<+->
	此时称\emph{极限存在}或\emph{数列收敛}.
	\onslide<+->
	若不存在这样的 $z$, 则称\emph{极限不存在}或\emph{数列发散}.
...
\begin{frame}{极限的四则运算}
	\onslide<+->
	由于复数列极限的定义和实数列极限的定义在形式上完全相同,
	\onslide<+->
	因此类似地, 极限的四则运算法则对于复数列也是成立的.
	\onslide<+->
	\begin{theorem}
...
\begin{frame}{数列收敛的等价刻画}
	...
	\begin{proof}[nearprev]
		...
		\onslide<+->{%
			``$\Rightarrow$'': 由三角不等式 $0\le \abs{x_n-x}, \abs{y_n-y}\le\abs{z_n-z}$ 和夹逼准则可得知.
		}
		
		\onslide<+->{%
			``$\Leftarrow$'': 由极限的四则运算法则可知 $\liml_{n\ra\infty}\bigl(\abs{x_n-x}+\abs{y_n-y}\bigr)=0$.
		}\onslide<+->{%
			再由三角不等式 $0\le\abs{z_n-z}\le\abs{x_n-x}+\abs{y_n-y}$ 和夹逼准则可得.\qedhere
		}
	\end{proof}
\end{frame}
...
\begin{frame}{例题: 数列的敛散性}
	...
	\begin{solution}[nearprev]
		...
		\]
		\onslide<+->{%
			因此 $\{z_n\}$ 收敛且 $\liml_{n\to\infty}z_n=1$.
		}
	\end{solution}
\end{frame}
...
\begin{frame}{数列极限的等价定义\noexer}
	...
	\end{definition}
	\onslide<+->
	若 $\liml_{n\ra\infty}\abs{z_n}=+\infty$, 我们将其记为 \emph{$\liml_{n\ra\infty}z_n=\infty$}.
	\onslide<+->
...
\begin{frame}[b]{复球面}
	...
	取一个球心在 $z=0$ 的单位球面.
	\onslide<+->
	过 $O$ 做垂直于复平面的直线, 并与球面相交于其中一点 $N$, 称之为\emph{北极}.
	\begin{itemize}
	...
	\begin{center}
		\begin{tikzpicture}
			...
			\draw (Z2) node[second,above left,visible on=<4->] {$Z_2$};
		\end{tikzpicture}
	\end{center}
\end{frame}
...
\begin{frame}[b]{复球面: 无穷远点}
	...
	\onslide<+->
	若我们在复平面上添加一个额外的``点''——\emph{无穷远点}, 记作 \emph{$\infty$}.
	\onslide<+->
	那么\emph{扩充复数集合 $\BC^*=\BC\cup\{\infty\}$} 就正好和球面上的点一一对应.
	\onslide<+->
	...
	\begin{center}
		\begin{tikzpicture}
			\begin{scope}[scale=1]
				...
				\draw[cstdash] (0,0)--({-.8*\a},{-.9*\a});
				\draw[cstaxis] ({-.8*\a},{-.9*\a})--(-.8,-.9);
				\coordinate (z1) at (.57,-.15);
				\coordinate (Z1) at ($(N)!1.2!(z1)$);
				\coordinate (z2) at (-1.6,-.2);
				\coordinate (Z2) at ($(N)!.45!(z2)$);
			...
			\draw (N) node[third,above] {$N$};
			\draw (z1) node[fourth,left] {$z_1$};
			\draw (Z1) node[fourth,below left] {$Z_1$};
			\draw (z2) node[second,left] {$z_2$};
			\draw (Z2) node[second,above left] {$Z_2$};
		\end{tikzpicture}
	\end{center}
\end{frame}
...
\begin{frame}{与实函数极限之联系}
	...
	\end{theorem}
	\onslide<+->
	该定理表明: 研究复变函数极限, 只需研究其实部、虚部两个二元实函数的极限.
...
\begin{frame}{例题: 判断函数极限是否存在}
	\onslide<+->
	\begin{example}[nearnext]
		证明: 当 $z\to0$ 时, 函数 $f(z)=\dfrac{\Re z}{\abs{z}}$ 的极限不存在.
	\end{example}
	\onslide<+->
	\begin{proof}[nearprev]
		...
		\onslide<+->{%
			...
			因此 $\liml_{\substack{x\to 0\\y\to0}}u(x,y)$ 不存在, 从而 $\liml_{z\to z_0}f(z)$ 不存在.\qedhere
		}
		\meddel
	\end{proof}
\end{frame}
...
\begin{frame}{例题: 函数连续性的判定}
	...
	\begin{proof}[near]
		设
		\[
			f(z)=u(x,y)+\ii v(x,y),\quad z_0=x_0+\ii y_0.
		\]
		\onslide<+->{%
			...
			\]
			在 $(x_0,y_0)$ 连续.\qedhere
		}
	\end{proof}
	\onslide<+->
	\begin{proof}[nearprev][另证]
