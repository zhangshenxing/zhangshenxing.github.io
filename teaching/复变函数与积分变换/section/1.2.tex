...
\begin{frame}{复数的极坐标形式}
	\begin{itemize}
		\item 由平面的极坐标表示, 我们可以得到复数的另一种表示方式.
		\item 以 $0$ 为极点, 正实轴为极轴, 逆时针为极角方向可以定义出复平面上的极坐标系.
	\end{itemize}
	\onslide<+->
	\begin{center}
...
\begin{frame}{辐角主值}
	\begin{itemize}
		\item 任意非零复数 $z$ 都有无穷多个辐角.
		\item 称其中位于 $(-\pi,\pi]$ 的那个辐角为\emph{辐角主值}或\emph{主辐角}, 记作 $\emphm{\arg z}$.
		\item 那么 $\emphm{\Arg z=\arg z+2k\pi, k\in\BZ}$.
		\item<8-> 注意 \alert{$\arg \ov z=-\arg z$ 未必成立}, 当且仅当 $z$ 不是负实数和 $0$ 时成立.
	\end{itemize}
	\onslide<2->
	\begin{figure}[hbpt]
		\centering
...
\begin{frame}{例: 求复数的三角和指数形式}
	...
	\begin{solution}[near]
		...
		由于 $z$ 在第一象限, 因此
		\[
			\arg z=\arctan\frac{\cos(\pi/5)}{\sin(\pi/5)}
			=\arctan\cot\frac\pi 5=\frac\pi2-\frac\pi5=\frac{3\pi}{10}.
		\]}\onslide<+->{%
		\[
