...
\begin{frame}{三次方程的根\noexer}
	...
	\begin{solution}
		...
		}\onslide<+->{所以 $u=1\pm\sqrt3, v=1\mp\sqrt 3$,
		}\onslide<+->{$x=u+v=2$.}
	\end{solution}
\end{frame}
...
\begin{frame}{三次方程的根\noexer}
	...
	\begin{center}
		\begin{tikzpicture}[framed]
			\filldraw[cstcurve,dcolora,domain=-3.1:4,smooth,fill=white] plot ({(\x)*0.4},{(\x*\x*\x+6*\x-20)*0.04});
		...
		\begin{figure}[h]
			\begin{subfigure}{0.3\textwidth}
				...
				\begin{tikzpicture}[framed,visible on=<4->]
					\draw[cstcurve,dcolora,domain=-2.8:3.5,smooth] plot ({(\x)*0.2},{(\x*\x*\x+6*\x-20)*0.02});
