...
\begin{frame}{复数的引入\noexer}
	...
	\begin{enumerate}
		...
		\onslide<+->
		然而, 如果我们\emph{接受负数开方}的话, 此时仍然有两个根, 形式地计算可以发现它们满足原来的方程.
	\end{enumerate}
\end{frame}
...
\begin{frame}{三次方程的根\noexer}
	...
	\begin{center}
		\begin{tikzpicture}
			\begin{scope}[framed,visible on=<4->]
			...
			\end{scope}
			\begin{scope}[framed,visible on=<5->,xshift=40mm]
				\draw[cstcurve,dcolora,domain=-3.1:2.7,smooth] plot ({(\x)*0.25},{(\x*\x*\x-3*\x+2)*0.06});
				\draw[cstaxis] (-1.5,0)--(1.5,0);
				\draw[cstaxis] (0,-1.2)--(0,1.2);
			\end{scope}
			\begin{scope}[framed,visible on=<6->,xshift=80mm]
