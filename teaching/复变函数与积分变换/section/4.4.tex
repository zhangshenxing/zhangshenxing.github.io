...
\begin{frame}{洛朗级数的形式}
	...
	-\frac1{2\pi i}\alertn{\oint_{K_1}\frac{f(\zeta)}{\zeta-z}\diff\zeta}.\]
	\onslide<2->
	\begin{center}
		\begin{tikzpicture}
...
\begin{frame}{洛朗级数}
	\onslide<+->
	和泰勒级数的推导类似,
	\[\frac1{2\pi i}\emphn{\oint_{K_2}\frac{f(\zeta)}{\zeta-z}\diff\zeta}
	...
	\onslide<+->
	\[-\frac1{2\pi i}\alertn{\oint_{K_1}\frac{f(\zeta)}{\zeta-z}\diff\zeta}=\frac1{2\pi i}\oint_{K_1}f(\zeta)\sum_{n=1}^\infty\frac{(z-z_0)^{-n}}{(\zeta-z_0)^{-n+1}}\diff\zeta.\]
\end{frame}
...
\begin{frame}{洛朗级数的形式}
	...
	\[\alertm{f(z)=\sum_{n=-\infty}^\infty\left[\emphm{\frac1{2\pi i}\oint_C\frac{f(\zeta)\diff\zeta}{(\zeta-z_0)^{n+1}}}\right](z-z_0)^n},\]
	其中 $R_1<|z-z_0|<R_2$.
\end{frame}
...
\begin{frame}{典型例题: 求洛朗级数}
	...
	\onslide<+->
	\begin{solution}
		\[c_n=\frac1{2\pi i}\oint_C\frac{e^z-1}{z^{n+3}}
		=\begin{cases}
			\dfrac{(e^z-1)^{(n+2)}}{(n+2)!}\Big|_{z=0}=\dfrac1{(n+2)!},&n\ge -3;\\
			(e^z-1)(0)=0,&n=-2;\\
			0,&n\le -1.
		\end{cases}\]
		\onslide<+->{%
