...
\begin{frame}{双边幂级数的收敛域}
	...
	\begin{center}
		\begin{tikzpicture}
			\filldraw[cstcurve,draw=main,cstfill] (0,0) circle (1.5);
...
\begin{frame}{洛朗级数的形式}
	...
	\begin{center}
		\begin{tikzpicture}
			\filldraw[cstcurve,second,cstfill] (0,0) circle (1.5);
			\filldraw[cstcurve,main,fill=white] (0,0) circle (1);
			\fill[cstdot,main,visible on=<4->] (-1,0) circle;
			\fill[cstdot,second,visible on=<4->] (-1.48,-0.3) circle;
			\fill[cstdot,black,visible on=<4->] (0,1.25) circle;
			\draw[second,cstra,thick,visible on=<3->] (0,0)--(1.2,0.9);
			\draw[main,cstra,thick,visible on=<3->] (0,0)--(0.6,-0.8);
			\draw
				(-1.25,-0.15) node[visible on=<4->] {$\zeta$}
				(0.5,0.1) node[second,visible on=<3->] {$R$}
				(0.1,-0.5) node[main,visible on=<3->] {$r$}
				(-0.3,0) node {$z_0$}
				(0.8,-1) node[main,visible on=<3->] {$K_1$}
				(1.4,1.2) node[second,visible on=<3->] {$K_2$}
