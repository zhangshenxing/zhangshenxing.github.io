...
\begin{frame}{拉普拉斯变换}
	...
	\onslide<+->
	它的傅里叶变换为
	\[\msf[\varphi(t)u(t)\ee^{-\beta t}]=\int_0^{+\infty}\varphi(t)\ee^{-(\beta+j\omega)t}\diff t=\int_0^{+\infty}\varphi(t)\ee^{-st}\diff t,
	\]
	其中 $s=\beta+j\omega$.
	\onslide<+->
...
\begin{frame}{拉普拉斯变换存在定理}
	...
	\begin{center}
		\begin{tikzpicture}[node distance=118pt]
			...
			\node[cstnode2,align=center, right=of a](b){象函数\\\alert{$F(s)=\displaystyle\int_0^{+\infty}f(t)\ee^{-st}\diff t$}};
...
\begin{frame}{例: 求拉普拉斯变换}
	...
	\begin{solution}
			\begin{align*}
				\msl[\ee^{kt}]&=\int_0^{+\infty}\ee^{kt}\ee^{-st}\diff t\\
				&\visible<+->{=\int_0^{+\infty}\ee^{-(s-k)t}\diff t
				=-\frac1{s-k}\ee^{-(s-k)t}\big|_0^{+\infty}}\\
				&\visible<+->{=\frac1{s-k}, \quad\Re s>\Re k.}
			\end{align*}
			\onslide<+->{即 $\alertm{\msl[\ee^{kt}]=\dfrac1{s-k}}$}.
			\onslide<+->{特别地 $\alertm{\msl[1]=\dfrac1s}$.}
	\end{solution}
\end{frame}
...
\begin{frame}{例: 求拉普拉斯变换}
	...
	\begin{solution}
		由分部积分可知
		\begin{align*}
			\msl[t^m]&=\int_0^{+\infty}t^m\ee^{-st}\diff t\\
			&=-\frac{t^m\ee^{-st}}s\Big|_0^{+\infty}+\int_0^{+\infty}\frac{\ee^{-st}}s\cdot mt^{m-1}\diff t\\
			&=\frac ms\msl[t^{m-1}].
		\end{align*}
		\onslide<+->{%
	...
	\begin{solution}[另解]
		...
		\onslide<+->{则
			\begin{align*}
				...
				b&=\Res[F(s),1]=\Bigl(\frac1s\Bigr)'\Big|_{s=1}=-\frac1{s^2}\Big|_{s=1}=-1,\\
