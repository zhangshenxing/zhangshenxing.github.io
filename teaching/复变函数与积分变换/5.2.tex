...
\begin{tikzpicture}[node distance=25pt]
  ...
  \node[draw,white,fill=heavyb,right=-40pt of integral]  (integral0){定积分 $\displaystyle\int_Cf(z)\diff z$ 的计算};
  ...
  \node[draw,white,fill=heavyr,right=-30pt of end1, visible on=<3->]	(end10){求出 $f(z)$ 的原函数 $F(z)$ 得到 $\displaystyle\int_Cf(z)\diff z=F(b)-F(a)$};
  \node[draw,white,fill=heavyr,below=of issingle,align=center, visible on=<4->]	(end2){设曲线方程为 $z(t)$, 则\\
积分$\displaystyle=\int_a^bf(z)z'(t)\diff t$\\
可能需要分段计算};
  \node[draw,white,fill=heavyr,above=of ismany,align=center, visible on=<8->]	(end3){用闭路内的孤立\\奇点的留数计算};
  \node[draw,white,fill=heavyr,below=of ismany,align=center, visible on=<9->]	(end4){用闭路外的孤立\\奇点的留数计算, \\最后取负号*};
