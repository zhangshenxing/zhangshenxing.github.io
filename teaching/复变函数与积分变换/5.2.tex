...
&\visible<+->{=\frac1{5!}\lim_{z\to0}(-\cos z)=-\frac1{120}.\mqed}
\end{align*}
\end{solutionc}
...
\[\markatt{\Res\left[\frac{P(z)}{Q(z)},z_0\right]=\frac{P(z_0)}{Q'(z_0)}.}\]
\end{conclusion}
\begin{proofs}
...
\visible<+->{=\frac{P(z_0)}{\lim\limits_{z\to z_0}\dfrac{Q(z)-Q(z_0)}{z-z_0}}
=\frac{P(z_0)}{Q'(z_0)}.}&\visible<.->{\mqed}
\end{flalign*}
\end{proofs}
\endgroup
\end{frame}
...
\begin{theorem}
如果 $f(z)$ 只有有限个奇点, 那么 $f(z)$ 在\markatt{扩充复平面内各奇点处的留数之和为 $0$}.
\end{theorem}
\begin{proof}
...
&&&\visible<+->{=-2\pi i\bigl[\Res[f(z),3]+\Res[f(z),\infty]\bigr]=-\frac{\pi i}{(3+i)^{10}}\sin\frac13.}&\visible<.->{\mqed}
\end{flalign*}
\end{solutionc}
\end{frame}
...
可能需要分段计算};
  ...
  \draw[cstnarrow,defcolor,visible on=<2->] (integral) -- (isclosed);
  \draw[cstnarrow,defcolor,visible on=<3->] (isanalytic) -- node[left]{是} (end1);
  \draw[cstnarrow,defcolor,visible on=<5->] (isclosed) -- node[above]{是} (issingle);
  \draw[cstnarrow,defcolor,visible on=<7->] (issingle) -- node[above]{是} (ismany);
  \draw[cstnarrow,defcolor,visible on=<3->] (isclosed) -- node[left]{否} (isanalytic);
  \draw[cstnarrow,defcolor,visible on=<6->] (issingle) -- node[left]{否} (end2);
  \draw[cstnarrow,defcolor,visible on=<4->] (isanalytic) -- node[above]{否} (end2);
  \draw[cstnarrow,defcolor,visible on=<8->] (ismany) -- node[left]{闭路内的少} (end3);
  \draw[cstnarrow,defcolor,visible on=<9->] (ismany) -- node[left]{闭路外的少} (end4);
\end{tikzpicture}
\end{center}
\end{frame}
