...
\onslide<+->{则 $\suml_{n=1}^\infty z_n$ 的部分和为
...
\begin{proof}
必要性由前一定理的证明已经知道,
\onslide<+->{充分性由 $|z_n|\le|x_n|+|y_n|$ 可得.}
\end{proof}
\end{frame}
...
\begin{tikzpicture}[overlay]
	\draw[decorate,decoration={brace,amplitude=8},thick,dcolora] (1.15,-4.5)--(1.15,-1.5);
	\draw[decorate,decoration={brace,amplitude=8},thick,dcolorb] (3.3,-0.37)--(9.5,-0.37);
	\draw (6.4,0.2) node[dcolorb] {实部级数}
	(0.6,-3) node[align=center,dcolora] {虚\\部\\级\\数};
\end{tikzpicture}
\begin{center}
\renewcommand\arraystretch{2}
\defaultrowcolors
\begin{tabular}{|c|c|c|c|}
	&\tht ~~发散~~&\tht 条件收敛&\tht 绝对收敛\\
	\tht 发散&发散&发散&发散\\
	\tht 条件收敛&发散&\cellcolor{lightg} 条件收敛
		&\cellcolor{lightg} 条件收敛\\
	\tht 绝对收敛&发散&\cellcolor{lightg}条件收敛
		&\cellcolor{lightr}绝对收敛\\
\end{tabular}
\end{center}
\end{frame}
...
\onslide<+->{虚部级数 $\displaystyle\sum_{n=1}^\infty\frac1{2^n}$ 绝对收敛,}
\onslide<+->{所以该级数条件收敛.}
\end{solution}
\end{frame}
...
\onslide<+->{\[\sum_{n=1}^\infty y_n=1-\frac13+\frac15-\frac17+\cdots\]
均条件收敛,}
\onslide<+->{所以原级数条件收敛.}
\end{solution}
\end{frame}
