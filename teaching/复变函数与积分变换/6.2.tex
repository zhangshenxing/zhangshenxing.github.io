...
由此可得
\vspace{-3pt}
\[\eqatt{\msf[\delta(t-t_0)]=e^{-j\omega t_0},\quad
...
\onslide<+->
\begin{align*}
\eqatt{\msf[\sin{\omega_0 t}]}&=\frac1{2j}\left[\msf[e^{j\omega_0t}]-\msf[e^{-j\omega_0t}]\right]&\\
&\visible<+->{=\frac1{2j}[2\pi\delta(\omega-\omega_0)-2\pi\delta(\omega+\omega_0)]}&\\
&\visible<+->{\eqatt{=j\pi[\delta(\omega+\omega_0)-\delta(\omega-\omega_0)]}.}
\end{align*}
\end{solution}
\end{frame}
...
\end{exercise}
\begin{answer}
\boxatt{$\msf[\cos{\omega_0 t}]=\pi[\delta(\omega+\omega_0)+\delta(\omega-\omega_0)]$}.
\end{answer}
\end{frame}
