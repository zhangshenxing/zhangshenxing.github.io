...
\draw
	...
	(-1.2,1.2) node[dcolorc,visible on=<5->] {$\arctan\dfrac yx+\pi$}
	(-0.6,-0.2) node[dcolorc,visible on=<5->] {$\pi$}
	(-1.2,-1.1) node[dcolorb,visible on=<6->] {$\arctan\dfrac yx-\pi$}
...
\begin{frame}[<*>]{作业}
  \begin{homeworks}
    \item 判断题: $z$ 是实数当且仅当 $\arg z=0,\pi$.
    \item 判断题: $z$ 是纯虚数当且仅当 $\arg z=\pm\dfrac\pi2$.
    \item 求 $\dfrac{1+2i}{3-4i}-\dfrac{2-i}{5i}$ 的模和主辐角.
    \item 将 $1-\sqrt 3i,\dfrac{2i}{1-i}$ 写成三角形式和指数形式.
    \item 证明当 $|z|=1>|w|$ 时, $\left|\dfrac{z-w}{1-z\ov w}\right|=1$.
    \item(2020年期末A卷) 如果 $(1+2i)\ov z=4+3i$, 求 $z$ 及其主辐角.
    \item(2022年期末A卷) 求 $z=\dfrac{3+i}{i}-\dfrac{10i}{3-i}$ 的模和辐角.
  \end{homeworks}
\end{frame}
}