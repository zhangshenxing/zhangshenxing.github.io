...
\draw
  ...
  (1.2,-0.5) node[notcolor] {复平面}
  (6.7,0.8) node[defcolor,visible on =<4->] {$r$};
\coordinate (A) at (9,0);
\coordinate (B) at (6,0);
\coordinate (C) at (7.6,1.2);
\draw[cstaxis] (B)--(A);
\draw[cstcurve,defcolor,cstarrowto,visible on =<4->] (B)--(C);
\draw[notcolor,thick,visible on =<5->,cstarrowto] pic [draw, "$\theta$", angle eccentricity=1.3, angle radius=0.8cm] {angle};
\end{tikzpicture}
\end{center}
...
\draw[thick,notcolor] pic [draw, "$\varphi$", angle eccentricity=1.4, angle radius=0.7cm] {angle};
\coordinate (A) at (1.8,0.6);
\coordinate (C) at (1,2.32);
\draw[cstcurve,alecolor,cstarrowto] (B)--(A);
\draw[cstcurve,notcolor,cstarrowto] (B)--(C);
\draw[thick,notcolor] pic [draw, "$\frac{\theta-\varphi}2$", angle eccentricity=1.7] {angle};
\draw
  (-0.2,-0.3) node {$0$}
