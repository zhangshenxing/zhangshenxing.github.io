...
\draw
  ...
  (6.7,0.8) node[dcolorc,visible on =<4->] {$r$};
\coordinate (A) at (9,0);
\coordinate (B) at (6,0);
\coordinate (C) at (7.6,1.2);
\draw[cstaxis] (B)--(A);
\draw[cstcurve,dcolorc,cstarrowto,visible on =<4->] (B)--(C);
\draw[dcolorb,thick,visible on =<5->,cstarrowto] pic [draw, "$\theta$", angle eccentricity=1.3, angle radius=0.8cm] {angle};
\end{tikzpicture}
\end{center}
