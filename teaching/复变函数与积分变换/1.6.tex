...
\markatt{如果 $\forall \varepsilon>0,\exists N$ 使得当 $n\ge N$ 时 $|z_n-z|<\varepsilon$}, 则称 $z$ 是\markdef{数列 $\{z_n\}$ 的极限}, 记作 \markdef{$\lim\limits_{n\to\infty}z_n=z$}.
\item 如果 \markatt{$\forall X>0,\exists N$ 使得当 $n\ge N$ 时 $|z_n|>X$}, 则称 $\infty$ 是数列 $\{z_n\}$ 的极限, 记作 $\lim\limits_{n\to\infty}z_n=\infty$.
\end{itemize}
\end{definition}
\end{frame}
...
\filldraw[cstcurve,alecolor,fill=white] (0,0) circle (0.7);
...
\onslide<+->
过 $O$ 做垂直于复平面的直线, 并与球面相交于另一点 $N$, 称之为\markdef{北极}.

\onslide<4->
\begin{center}
\begin{tikzpicture}
\fill[cstfill] (-3.5,-0.67)--(-2,0.67)--(3.5,0.67)--(2,-0.67)--cycle;
\filldraw[cstcurve,fill=black!20,draw=black] (0,1) circle (1);
\draw[cstdash,alecolor] (0,1) circle (1 and 0.3);
\draw[cstdash,black,visible on=<5->] (0,0)--(0,2);
\fill[cstdot,alecolor] (0,1) circle;
\fill[cstdot,defcolor,visible on=<5->] (0,2) circle;
\draw[cstaxis] (0,0)--(2.5,0);
\draw[cstaxis] (0,0)--(-0.8,-0.9);
\draw
  (0,2.3) node[defcolor,visible on=<5->] {$N$};
\end{tikzpicture}
\end{center}
\end{frame}
...
\filldraw[cstcurve,fill=black!20,draw=black] (0,1) circle (1);
\draw[cstdash,alecolor] (0,1) circle (1 and 0.3);
\draw[cstdash,defcolor] (0,0) circle (2 and 0.6);
\draw[cstdash,black] (0,0)--(0,2);
\draw[cstaxis] (0,0)--(2.5,0);
\draw[cstaxis] (0,0)--(-0.8,-0.9);
\draw[cstcurve,alecolor,cstarrowto] (0,2)--(1.65,-0.75);
\fill[cstdot,alecolor] (0.6,1) circle;
\draw[cstcurve,cstarrowto,notcolor,visible on=<2->] (0,2)--(-1,0);
\fill[cstdot,notcolor,visible on=<2->] (-0.7,0.6) circle;
\fill[cstdot,defcolor] (0,2) circle;
\draw
  (0,2.3) node[defcolor] {$N$}
  (-0.3,0.3) node[notcolor,visible on=<2->] {$Z_2$}
  (-1.4,0) node[notcolor,visible on=<2->] {$z_2$}
  (1.3,1) node[alecolor] {$Z_1$}
  (1.9,-0.75) node[alecolor] {$z_1$};
\end{tikzpicture}
\end{center}
\end{frame}
...
\onslide<+->
如果我们在复平面上添加一个额外的"点"——\markdef{无穷远点}, 记作 \markdef{$\infty$}.
\onslide<+->
那么\markdef{扩充复数集合 $\BC^*=\BC\cup\set\infty$} 就正好和球面上的点一一对应.
\onslide<+->
称这样的球面为\markdef{复球面}, 称包含无穷远点的复平面为\markdef{扩充复平面}(\markdef{闭复平面}).
\onslide<1->
\begin{center}
\begin{tikzpicture}
\fill[cstfill] (-3.5,-0.67)--(-2,0.67)--(3.5,0.67)--(2,-0.67)--cycle;
...
\draw[cstcurve,alecolor,cstarrowto] (0,2)--(1.65,-0.75);
\fill[cstdot,alecolor] (0.6,1) circle;
\draw[cstcurve,cstarrowto,notcolor] (0,2)--(-1,0);
\fill[cstdot,notcolor] (-0.7,0.6) circle;
\fill[cstdot,defcolor] (0,2) circle;
\draw
  (0,2.3) node[defcolor] {$N$}
  (-0.3,0.3) node[notcolor] {$Z_2$}
  (-1.4,0) node[notcolor] {$z_2$}
  (1.5,1) node[alecolor] {$Z_1$}
  (1.9,-0.75) node[alecolor] {$z_1$};
\end{tikzpicture}
\end{center}
\end{frame}
...
\draw[cstcurve,cstarrowto] (0,2)--(2.2,0);
\fill[cstdot,alecolor] (1,1.1) circle;
\draw[cstcurve,alecolor,cstarrowto,notcolor] (0,2)--(-1,0);
\fill[cstdot,notcolor] (-0.8,0.4) circle;
\fill[cstdot,defcolor] (0,2) circle;
\draw
  (0,2.3) node[defcolor] {$N$}
  (-0.4,0.5) node[notcolor] {$X_2$}
  (-1,-0.2) node[notcolor] {$x_2$}
  (1.4,1.2) node[alecolor] {$X_1$}
  (2.1,-0.2) node[alecolor] {$x_1$};
\end{tikzpicture}
\end{center}
\end{frame}
...
\onslide<+->
如果存在复数 $A$ 使得对 $A$ 的任意邻域 $U,\exists\delta>0$ 使得当 $z\in\stackrel\circ U(z_0,\delta)$ 时, 有 $f(z)\in U$, 则称 $A$ 为 \markdef{$f(z)$ 当 $z\to z_0$ 时的极限}, 记为 \markdef{$\lim\limits_{z\to z_0}f(z)=A$} 或 \markdef{$f(z)\to A (z\to z_0)$}.
\end{definition}
...
\begin{definition}
\begin{itemize}
\item 如果 $\lim\limits_{z\to z_0}f(z)=f(z_0)$, 则称 $f(z)$ 在 \markdef{$z_0$ 处连续}.
\item 如果 $f(z)$ 在区域 $D$ 内处处连续, 则称 $f(z)$ 在 \markdef{$D$ 内连续}.
\end{itemize}
\end{definition}
...
\onslide<+->
这就导致了复变函数有\markatt{导数}, 而不是像多变量实函数只有偏导数.
\onslide<+->
