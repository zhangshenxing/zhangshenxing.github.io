...
\begin{frame}[<*>]{作业}
  \begin{homeworks}
		\item(2020年A卷) 设 $C$ 为从原点到 $2$ 再到 $2+i$ 的折线段, 计算积分 $\int_Cz^2\diff z$.
		\item(2021年A卷) 设 $f(z)$ 在单连通域 $D$ 内处处解析且不为零, $C$ 为 $D$ 内任何一条简单闭曲线, 则 $\displaystyle\oint_C\frac{f''(z)+2f'(z)+f(z)}{f(z)}\diff z=$\fillblank{}.
		\item(2021年A卷) 设 $\displaystyle f(z)=\oint\frac{\sin \zeta}{\zeta-z}\diff z$, 其中 $C$ 为正向圆周 $|z|=2$, 则 $f\left(\dfrac\pi6\right)=$\fillbrace
		\xx{$\pi i$}{$-\pi i$}{$0$}{$2\pi i$}
    \item(2022年A卷) 设 $C$ 为正向圆周 $|z|=2$, 则 $\displaystyle\oint_C\left(\frac{\overline z}{z}\right)\diff z=$\fillblank{}.
    \item(2020年A卷) 设 $C$ 为正向圆周 $|z|=2$, 求 $\displaystyle\oint_C\frac{ze^z}{z^2-1}\diff z$.
    \item(2020年B卷) 设 $C$ 为正向圆周 $|z|=\dfrac32$, 求 $\displaystyle\oint_C\frac1{(z+1)^2(z-2)}\diff z$.
    \item(2021年B卷) 设 $C$ 为正向圆周 $|z|=2$, 求 $\displaystyle\oint_C\frac1{(z^2+1)(z^2+9)}\diff z$.
