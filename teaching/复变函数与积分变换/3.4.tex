...
\draw[cstcurve,notcolor] (0,0) circle (1);
\draw[cstcurve,alecolor,visible on=<3->] (0.5,0) circle(0.2);
\draw[cstcurve,alecolor,visible on=<3->] (-0.5,0) circle(0.2);
\draw[cstcurve,alecolor,visible on=<3->] (0,0) circle(0.2);
\fill[cstdot,notcolor,visible on=<2->] (0,0) circle;
\fill[cstdot,notcolor,visible on=<2->] (0.5,0) circle;
\fill[cstdot,notcolor,visible on=<2->] (-0.5,0) circle;
\draw
  (1,0.8) node[notcolor] {$C$}
  (-0.3,0.45) node[alecolor,visible on=<3->] {$C_1$}
  (0.5,-0.45) node[alecolor,visible on=<3->] {$C_2$}
  (-0.5,-0.45) node[alecolor,visible on=<3->] {$C_3$};
\end{tikzpicture}
\end{center}
\end{solutions}
\end{frame}
...
&&&\visible<+->{=2\pi i\left(-1+\frac e2+\frac{e^{-1}}2\right)=\pi i(e+e^{-1}-2).}\mqed
\end{flalign*}
\end{solutionc}
\end{frame}
...
\begin{columns}
	...
	\column{0.48\textwidth}
		...
		\filldraw[cstcurve,alecolor,rounded corners=0.3cm,fill=white] (-0.6,0.6) rectangle (-1.3,-0.6);
		\draw[cstcurve,visible on=<4->,defcolor] (0,0) circle (0.4);
		\draw[cstcurve,visible on=<4->,notcolor] (0,0) circle (1.5);
		\fill[cstdot,alecolor] (0,0) circle;
		\draw
		  (0,0.2) node[alecolor] {$z_0$}
		  (-0.6,-0.8) node[alecolor] {$C$}
		  (0.7,-0.2) node[defcolor,visible on=<4->] {$C_1$}
		  (0.9,0.8) node[notcolor,visible on=<4->] {$C_2$};
		\end{tikzpicture}
\end{columns}
\onslide<+->
\begin{answer}
...
解析函数可以由它的积分所表示.
\onslide<+->
不仅如此, 通过积分表示, 还可以说明\markatt{解析函数存在任意阶解析的导数}.

\begin{theorem}[柯西积分公式]
设函数 $f(z)$ 在闭路或复合闭路 $C$ 及其内部 $D$ 解析, 则对任意 $z_0\in D$,
\[\markatt{f^{(n)}(z_0)=\frac{n!}{2\pi i}\oint_C\frac{f(z)}{(z-z_0)^{n+1}}\diff z}.\]
\end{theorem}
\onslide<+->
其中右侧被积函数可以记忆成公式
...
\onslide<+->
\markatt{柯西积分公式的作用不在于计算高阶导数, 而是用高阶导数来计算积分.}
\begin{example}
...
&&&\visible<+->{=\frac{2\pi i}{2!}\left[\frac1{(z-2)^2}\right]''\Big|_{z=0}+\frac{2\pi i}{1!}\left(\frac1{z^3}\right)'\Big|_{z=2}=0.}\mqed
\end{flalign*}
\end{solutionc}
\onslide<+->
\begin{columns}
