...
\end{align*}}
\onslide<+->{由 $\varepsilon$ 的任意性可知 $\displaystyle\oint_C\frac{f(z)}{z-z_0}\diff z=2\pi i f(z_0)$.\qedhere}
\end{proof}
\end{frame}
...
\onslide<+->{因此我们只需要证明右边的极限等于 $\displaystyle\frac1{2\pi i}\emph{\oint_C\frac{f(z)}{(z-z_0)^2}\diff z}$.}
\end{proofs*}
\end{frame}
...
\onslide<+->{然后利用长大不等式证明 $h\to 0$ 时, $\dfrac{f^{(n)}(z_0+h)-f^{(n)}(z_0)}h$ 趋于积分公式右侧.}
\onslide<+->{具体过程省略.}
\end{proofe*}
\end{frame}
...
\begin{proof}
由题设可知 $f(z)$ 的积分与路径无关.
\onslide<+->{固定的 $z_0\in D$, 则
\[F(z)=\int_{z_0}^zf(z)\diff z\]
定义了 $D$ 内一个单值函数.}
\onslide<+->{类似于原函数的证明可知 $F'(z)=f(z)$.}
\onslide<+->{故 $f(z)$ 作为解析函数 $F(z)$ 的导数也是解析的.}
\end{proof}
\end{frame}
