...
\begin{center}
\begin{tikzpicture}
\filldraw[cstcurve,draw=alecolor,fill=cyan!50!white] (0,0) circle (1);
...
\draw
  (0.6,0.1) node {$R$}
  (0,-0.4) node[notcolor] {绝对收敛}
  (2.5,-0.4) node[notcolor] {发散}
  (-3,-0.4) node[notcolor] {都有可能};
\end{tikzpicture}
\end{center}
\onslide<+->
\begin{proof}
...
\begin{enumerate}
\item \marknot{达朗贝尔公式(比值法)}: $r=\displaystyle\lim_{n\to\infty}\abs{\frac{c_{n+1}}{c_n}}$ (假设存在);
...
&&&\visible<+->{=\frac2{1-2z}-\frac1{1-z}=\frac1{(1-2z)(1-z)}.}\mqed
\end{flalign*}
\end{solutions}
\end{frame}
