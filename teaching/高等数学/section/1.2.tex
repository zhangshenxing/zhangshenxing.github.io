...
\begin{frame}{有界和无界}
	...
	\begin{definition}
		设函数 $f(x)$ 的定义域为 $D$.
		如果 $f(x)$ 的值域包含在一个有限区间 $[m,M]$ 内, 则称 $f(x)$ \emph{有界}. 否则称 $f(x)$ \emph{无界}.
	\end{definition}
	\onslide<+->
	$f(x)$ 有界等价于: $\exists M>0$ 使得 $\forall x\in D, |f(x)|\le M$.
	\onslide<+->
	\begin{center}
		\begin{tikzpicture}
			\draw[cstaxis] (-3,0)--(3,0);
			\draw[cstaxis] (0,-2)--(0,2);
			\draw[cstcurve,main,smooth,domain=-1:2] plot coordinates {(-2.8,-0.8) (-2.1,-0.9) (-1.4,-0.9) (-0.7, 0.5) (0,1) (0.7,0.5) (1.4,1) (2.1,0.9) (2.8,0.6)};
			\draw[cstdash,fourth] (-3,-1.1)--(3,-1.1);
			\draw[cstdash,fourth] (-3,1.1)--(3,1.1);
			\draw
				(-1.7,0.5) node[main] {$y=f(x)$}
				(2.3,1.4) node[fourth] {$y=M$}
				(2.3,-1.4) node[fourth] {$y=m$}
				(-0.2,-0.2) node {$O$}
				(2.8,-0.2) node {$x$}
				(-0.2,1.8) node {$y$};
	...
	\begin{definition}
		...
		\begin{enumerate}
			...
			\onslide<+->{\begin{tikzpicture}[overlay,xshift=-0.1cm,yshift=0.4cm]
				\draw[decorate,thick,decoration={brace,amplitude=5},second] (0,0.5)--(0,-0.5);
				\draw (0.6,0) node[second,align=center] {\emph{单调}\\\emph{函数}};
			\end{tikzpicture}}
		\end{enumerate}
	\end{definition}
	\onslide<+->
...
\begin{frame}{函数可分解为奇函数与偶函数之和\noexer}
	...
	\begin{proof*}
		...
		\onslide<+->{那么 $\forall x\in D$,
			\[f(-x)=f_1(-x)+f_2(-x)=f_1(x)-f_2(x).\]
		}\onslide<+->{从而
			\[f_1(x)=\dfrac{f(x)+f(-x)}2,\qquad
			f_2(x)=\dfrac{f(x)-f(-x)}2.\]
		}\onslide<+->{唯一性得证.}
		
		\onslide<+->{反过来, 这样的 $f_1(x),f_2(x)$ 的定义域和 $f(x)$ 的定义域相同, 且分别是偶函数和奇函数, 于是存在性得证.\qedhere}
	\end{proof*}
	\endgroup
\end{frame}
...
\begin{frame}{例: 函数的特征}
	...
	\onslide<+->
	\begin{example}
		函数 $f(x)=|x\cos x| e^{\cos x}$ 在 $\BR$ 上是\fillbraceframe{D}.
		\begin{exchoice}(4)
			() 有界函数
			() 单调函数
			() 周期函数
	...
	\begin{solution*}
		...
		\onslide<+->\onslide<+->{对于任意正整数 $M$,
			\[f(2M\pi)=2M\pi e>M,\]
		所以 $f(x)$ 无界.}

		\onslide<+->{由于
			\[f(0)=0,\quad f(\pi)=\frac\pi e,\quad f\left(\frac{3\pi}2\right)=0,\]
		因此 $f(x)$ 不单调.}

		\onslide<+->{若 $T>0$ 是 $f(x)$ 的周期, 则由 $x \in[0,T)$ 时, $f(x)\le Te$ 可知 $f(x)$ 有界, 矛盾!}
	\end{solution*}
	\endgroup
\end{frame}