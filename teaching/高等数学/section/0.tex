...
\begin{frame}{课程内容I}
	\begin{center}
		\begin{tikzpicture}[
			...
			child [grow=180, concept color=main] { node {函数}
				child[grow=-130] { node {定义域和值域} }
				child[grow=-90] { node {有界、单调、奇偶、周期} }
				child[grow=-50] { node {初等函数} }
			}
			child [grow=-90, level distance=3cm, concept color=second] { node {极限}
			...
			child [grow=0, concept color=third] { node {连续}
...
\begin{frame}{课程内容II}
	\begin{center}
		\begin{tikzpicture}[
			...
			child [grow=225, concept color=fourth] { node {导数}
			...
			child [grow=-45, concept color=fifth] { node {一元函数微分学}
...
\begin{frame}{课程学习方法}
	\begin{center}
		\begin{tikzpicture}[node distance=25pt]
			\node[cstnode2,align=center] (1) at (0,2)  {\emph{课前}\\预习课本};
			\node[cstnode2,align=center] (2) at (3,0)  {\emph{课上}\\认真听课\\记好笔记};
			\node[cstnode2,align=center] (3) at (0,-2) {\emph{课后}\\过一遍教材\\与课上知识点};
			\node[cstnode2,align=center] (4) at (-3,0) {\emph{作业}\\检测学\\习效果};
