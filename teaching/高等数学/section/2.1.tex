...
\begin{frame}{极限的引入}\small
	\onslide<+->
	\begin{example}
		\begin{tikzpicture}[overlay]\begin{tikzpicture}[framed,xshift=-93mm,yshift=47mm,scale=1.3]
			\foreach \j in {1,2,...,6}{
			...
			\begin{scope}[visible on=<2>]
				\draw[cstcurve,dcolora] (0,0)--(1,{(sqrt 3)/3});
				\draw
					(0,-0.3) node[dcolorb] {$O$}
					(1.1,0) node {$A$}
					(0.6,1) node {$B$}
					(1.1,0.7) node {$C$};
					\coordinate (A) at (1,0);
					...
					\draw[dcolorb,thick] pic [draw, "$\theta$",angle eccentricity=1.5,angle radius=5mm] {angle};
			\end{scope}
	...
	\begin{example}
		...
		\begin{center}
			\begin{tikzpicture}[framed]
				...
				\foreach \x in {1,2,3,...,12}{
					\fill[cstdot,dcolora] (\x,{8/(2^\x)}) circle;
				}
			\end{tikzpicture}
		\end{center}
	\end{example}
	...
	\begin{example}		
		...
		\begin{center}
			\begin{tikzpicture}[framed]
				...
				\foreach \x in {1,2,3,...,9}{
					\fill[cstdot,dcolora] ({\x/2},{\x/2}) circle;
				}
			\end{tikzpicture}
		\end{center}
	\end{example}
	...
	\begin{example}
		...
		\begin{center}
			\begin{tikzpicture}[framed]
				...
				\draw[cstdash,dcolord] (0,3.14)--(13,3.14);
				...
				\foreach \x in {1,2,3,...,20}{
					\fill[cstdot,dcolora] ({0.6*\x},{\x*sin(180/\x)}) circle;
				}
			\end{tikzpicture}
		\end{center}
	\end{example}
	...
	\begin{example}
		...
		\begin{center}
			\begin{tikzpicture}[framed]
				...
				\draw[cstdash,dcolord] (0,1)--(12.5,1);
				\draw[cstdash,dcolord] (0,-1)--(12.5,-1);
				\draw (-0.2,1) node {$1$};
				\draw (-0.35,-1) node {$-1$};
				...
				\foreach \x in {1,3,5,...,19}{
					\fill[cstdot,dcolora] ({0.6*\x},{-1+1/\x}) circle;
				}
	...
	\begin{example}
		...
		\begin{center}
			\begin{tikzpicture}[framed]
				...
				\draw[cstdash,dcolord,visible on=<1-2>] (0,1)--(12,1);
				...
				\begin{scope}[visible on=<3->]
					\draw[cstdash,dcolore] (0,1.2)--(12,1.2);
					\draw[cstdash,dcolore] (0,0.8)--(12,0.8);
					\draw (-0.6,1.2) node {$1+\varepsilon$};
					\draw (-0.6,0.8) node {$1-\varepsilon$};
				\end{scope}
				\begin{scope}[visible on=<4->]
					\draw[cstdash,dcolorc] (3.9,0)--(3.9,1.2);
...
\begin{frame}{数列极限的定义}
	...
	\begin{definition*}
		...
		\onslide<+->{\begin{tikzpicture}[overlay]
			\draw[semithick,dcolorc] (-9.3,2.5) rectangle (-0.6,3.2);
			\draw (0.5,2.85) node[dcolorc] {$\varepsilon$-$N$ 语言};
		\end{tikzpicture}}
	\end{definition*}
	\onslide<+->
...
\begin{frame}{数列与子数列的极限关系\noexer}
	...
	\onslide<+->
	这是因为 $\forall\varepsilon>0, \exists N_i$ 使得当 $n>N_i$ 时, $|a_n-a|<\varepsilon$, 
	\onslide<+->
	从而当
		\[n>\max\set{N_1,N_2,\dots,N_m}\]
	时, $|a_n-a|<\varepsilon$.
\end{frame}
...
\begin{frame}{数列与子数列的极限关系\noexer}	
	...
	但是 $\set{a_n}$ 本身却不收敛.}
	\begin{center}
		\begin{tikzpicture}[framed,scale=0.8,visible on=<2->]
			...
			\foreach \x in {1,2,3,4,5}{
				...
				\draw[dcolorb,visible on=<3->] (\x,5.8) node {$0$};
			}
