...
\begin{frame}{极限的引入}\small
	...
	\begin{example}
		...
		\begin{tikzpicture}[overlay,xshift=-3mm,yshift=-42mm,scale=1.5]
			\foreach \j in {1,2,...,6}{
				\filldraw[cstcurve,dcolorb,cstfill,visible on=<2-5>] (0,0)--({cos(60*\j-30)/cos(30)},{sin(60*\j-30)/cos(30)})--({cos(60*\j+30)/cos(30)},{sin(60*\j+30)/cos(30)})--cycle;
			}
			\foreach \j in {1,2,3,...,12}{
				\filldraw[thick,dcolorb,cstfill,visible on=<6>] (0,0)--({cos(30*\j-15)/cos(15)},{sin(30*\j-15)/cos(15)})--({cos(30*\j+15)/cos(15)},{sin(30*\j+15)/cos(15)})--cycle;
			}
			\foreach \j in {1,2,3,...,24}{
				\filldraw[semithick,dcolorb,cstfill,visible on=<7>] (0,0)--({cos(15*\j-7.5)/cos(7.5)},{sin(15*\j-7.5)/cos(7.5)})--({cos(15*\j+7.5)/cos(7.5)},{sin(15*\j+7.5)/cos(7.5)})--cycle;
			}
			\foreach \j in {1,2,3,...,48}{
				\filldraw[thin,dcolorb,cstfill,visible on=<8->] (0,0)--({cos(7.5*\j-3.75)/cos(3.75)},{sin(7.5*\j-3.75)/cos(3.75)})--({cos(7.5*\j+3.75)/cos(3.75)},{sin(7.5*\j+3.75)/cos(3.75)})--cycle;
			}
			\fill[dcolord] (0,0) circle (1);
			\foreach \j in {1,2,...,6}{
				\filldraw[cstcurve,dcolorc,cstfill,fill=dcolore,visible on=<2-5>] (0,0)--({cos(60*\j)},{sin(60*\j)})--({cos(60*\j+60)},{sin(60*\j+60)})--cycle;
			}
			\foreach \j in {1,2,3,...,12}{
				\filldraw[thick,dcolorc,cstfill,fill=dcolore,visible on=<6>] (0,0)--({cos (30*\j)},{sin (30*\j)})--({cos(30*\j+30)},{sin(30*\j+30)})--cycle;
			}
			\foreach \j in {1,2,3,...,24}{
				\filldraw[semithick,dcolorc,cstfill,fill=dcolore,visible on=<7>] (0,0)--({cos (15*\j)},{sin (15*\j)})--({cos(15*\j+15)},{sin(15*\j+15)})--cycle;
			}
			\foreach \j in {1,2,3,...,48}{
				\filldraw[thin,dcolorc,cstfill,fill=dcolore,visible on=<8->] (0,0)--({cos (7.5*\j)},{sin (7.5*\j)})--({cos(7.5*\j+7.5)},{sin(7.5*\j+7.5)})--cycle;
			}
		\end{tikzpicture}
		\onslide<+->{
			依次计算单位圆的内接和外切正 $3\cdot 2^n=6,12,24,48,\dots$ 边形的面积
			\[A_n=3\cdot 2^{n-1}\sin\frac\pi{3\cdot 2^{n-1}},\qquad
			B_n=3\cdot 2^n\tan\frac\pi{3\cdot 2^n},\]
		...
		\begin{center}
			\begin{tabular}{c|l|l}\hline
				...
				\onslide<+->{$2$}&\onslide<.->{$\alert{3.}00000000$}&\onslide<.->{$\alert{3.}21539031$}\\\hline
				\onslide<+->{$3$}&\onslide<.->{$\alert{3.1}0582854$}&\onslide<.->{$\alert{3.1}5965994$}\\\hline
				\onslide<+->{$4$}&\onslide<.->{$\alert{3.1}3262861$}&\onslide<.->{$\alert{3.14}608622$}\\\hline
				\onslide<+->{$12$}&$\onslide<.->{\alert{3.141592}51$}&\onslide<.->{$\alert{3.141592}72$}\\\hline
				\onslide<+->{$13$}&\onslide<.->{$\alert{3.1415926}2$}&\onslide<.->{$\alert{3.1415926}7$}\\\hline
			\end{tabular}
		\end{center}
	...
	\begin{alertblock}{极限的朴素定义}
		...
		\begin{tikzpicture}[overlay,xshift=-121mm,yshift=-7mm]
			\begin{scope}[visible on=<2->]
				\draw[dcolorb,thick] (-2.25,1.1) rectangle (4.3,1.6);
				\draw (6,2.8) node {极限过程: $x\ra$ 某个状态};
				\draw[cstaxis,semithick,dcolorb] (2.5,1.6)--(6,2.5);
			\end{scope}
			\begin{scope}[visible on=<3->]
				\draw[dcolorb,thick] (4.45,1.1) rectangle (8.3,1.6);
				\draw (10.3,2.8) node {记为 $y\ra A$};
				\draw[cstaxis,semithick,dcolorb] (7.9,1.6)--(10.3,2.5);
			\end{scope}
		\end{tikzpicture}
	\end{alertblock}
...
\begin{frame}{例题: 数列的变化趋势}
	\onslide<+->
	\begin{example}
		\begin{itemize}
			\item $\set{\dfrac1{2^n}}: \dfrac12,\dfrac14,\dfrac18,\dfrac1{16},\dfrac1{32},\cdots$ 递减地 $\ra 0$;
			\item $\set{n}: 1,2,3,4,5,\cdots$ 无限增大;
			\item $\set{A_n=3\cdot 2^{n-1}\sin\dfrac\pi{3\cdot2^{n-1}}}$ 递增地 $\ra \pi$;
			\item $\set{(-1)^n+\dfrac1n}$ 奇数项和偶数项分别交错地越来越接近 $1$ 和 $-1$;
			\item $\set{1+\dfrac{(-1)^n}n}$ 交错地 $\ra 1$.
		\end{itemize}
		\onslide<+->{
		\begin{center}
			\begin{tikzpicture}[scale=0.95]
				\draw[cstaxis] (-0.3,0)--(13,0);
				\draw[cstaxis] (0,-0.3)--(0,1.5);
				\draw[cstdash,dcolord] (0,1)--(13,1);
				\draw (-0.3,1) node {$1$};
				\draw (13,-0.3) node {$n$};
				...
				\foreach \x in {1,3,5,7,9,11}{
					\fill[cstdot,dcolora] (\x,{1-1/\x}) circle;
				}
...
\begin{frame}{数列极限的定义}
	...
	\begin{definition*}
		...
		\onslide<+->{\begin{tikzpicture}[overlay]
			\draw[semithick,dcolorc] (-9.3,3.2) rectangle (-0.6,3.9);
			\draw (0.5,3.55) node[dcolorc] {$\varepsilon$-$N$ 语言};
		\end{tikzpicture}}
	\end{definition*}
\end{frame}
...
\begin{frame}{数列极限的定义}
	...
	\onslide<+->
	\begin{example}
		单选题: “极限 $\lim\limits_{n\ra\infty}a_n=a$ 存在”的充要条件是“$\forall\varepsilon>0$,\fillbrace{\visible<7->{C}}”.
...
\begin{frame}{数列极限的有界性}
	...
	\begin{proof}
		...
		\[|a_n-a|<\varepsilon=1,\qquad |a_n|\le|a|+|a_n-a|<|a|+1.\]
		\onslide<+->{因此对于
		\[M=\max\set{|a_1|,\dots,|a_N|,|a|+1},\]
		有 $|a_n|\le M$.
...
\begin{frame}{数列极限的保号性}
	...
	\onslide<+->
	\begin{proof}
		\enumnum1 对于 $\varepsilon=\dfrac a2>0$, $\exists N$ 使得当 $n>N$ 时, 有 $|a_n-a|<\varepsilon=\dfrac a2$,
		\onslide<+->{从而
		\[a_n>a-\dfrac a2=\dfrac a2>0.\]
		}\onslide<+->{\enumnum2 同理可得.\qedhere}
	\end{proof}
	\onslide<+->
	注意这里 $>0$ \alert{不能换成} $\ge 0$,	$<0$ 也\alert{不能换成} $\le 0$.
	\onslide<+->
	因为极限为 $0$ 的数列符号可以无限次变化, 例如$\lim\limits_{n\ra\infty}\dfrac{(-1)^n}n=0$.
\end{frame}
...
\begin{frame}{数列极限的保号性}
	...
	\end{corollary}
	\onslide<+->
	同理, 这里 $\ge$ \alert{也不能换成} $>$ (这很容易记错!), 例如 $\lim\limits_{n\ra\infty}\dfrac1n=0$.
	\onslide<+->
	\begin{corollary}
...
\begin{frame}{数列与子数列的极限关系}
	...
	\begin{proof*}
		...
		\begin{center}
			当 $n>N$ 时, $|a_{2n-1}-a|<\varepsilon$; 当 $n>M$ 时, $|a_{2n}-a|<\varepsilon$.
		\end{center}
...
\begin{frame}{数列与子数列的极限关系\noexer}
	...
	\onslide<+->
	这是因为 $\forall\varepsilon>0$, 每个 $S_i$ 中至多只有有限多项满足 $|a_n-a|\ge\varepsilon$, 从而一共也只有有限多项满足这个不等式.
	于是对充分大的 $n$, $|a_n-a|<\varepsilon$.
\end{frame}
...
\begin{frame}{数列与子数列的极限关系\noexer}	
	...
	\begin{center}
		\begin{tikzpicture}[scale=0.8,visible on=<2->]
			...
			\draw[cstaxis] (0,0)--(0,6.5);
			\draw[cstaxis,dcolora,visible on=<2>]
				(1,1)--(1,2)--(2,1)--(1,3)--(2,2)--(3,1)--(1,4)--(2,3)--(3,2)--(4,1)--(1,5)--(2,4)--(3,3)--(4,2)--(5,1)--(1,6);
			\foreach \x in {1,2,3,4,5}{
				\draw[cstaxis,dcolorb,visible on=<3->] (\x,1)--(\x,5.5);
				\draw[dcolorb,visible on=<3->] (\x,5.7) node {$0$};
			}
			\draw[cstaxis,dcolorc,visible on=<4->] (1,1)--(5.5,5.5);
			\draw[dcolorc,visible on=<4->] (5.7,5.7) node {$1$};
			\foreach \x in {1,2,3,4,5}{
				\draw (\x,0)--(\x,0.2);
				\draw (0,\x)--(0.2,\x);
