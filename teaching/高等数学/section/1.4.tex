	...
	\begin{proof}
		\begin{minipage}{.24\textwidth}
			...
			\begin{tikzpicture}
				...
				\draw[second,thick] (A)--(B)--(C) pic [draw, "$x$",angle eccentricity=2,angle radius=2mm] {angle};
				\draw[cstcurve,main] (A)--(B)--(C)--cycle (A)--(0.5,{0.5*sqrt 3})--cycle;
				\draw[cstcurve,second] (B) circle (1);
				\draw
					(1.2,0) node {$A$}
					(-0.2,-0.2) node {$O$}
					(0.45,1.2) node {$B$}
		...
		\begin{minipage}{.72\textwidth}
			...
			\onslide<+->{由
			\begin{center}
				$\triangle OAB$ 的面积 $<$ 扇形 $OAB$ 的面积 $<$ 直角 $\triangle OAC$ 的面积
			\end{center}
			}\onslide<+->{可知
...
\begin{frame}{均值不等式}
	...
	\onslide<+->
	\begin{proof*}
		设 $b_i={a_i}/{\sqrt[n]{a_1\cdots a_n}}$, 则 $b_1b_2\cdots b_n=1$ 且我们只需证明 $b_1+b_2+\cdots+b_n\ge n$.

		\onslide<+->{%
			利用数学归纳法. $n=1$ 时显然成立.
		}\onslide<+->{%
			假设对于 $n-1\ge1$ 个数成立, 且等号成立当且仅当 $b_i$ 均为 $1$.
		}\onslide<+->{%
			我们不妨设 $b_1\ge b_2\ge\cdots\ge b_n$.
		}\onslide<+->{%
			对 $b_1b_n,b_2,\dots,b_{n-1}$ 应用归纳假设, 则
			$b_1b_n+b_2+\cdots+b_{n-1}\ge n-1$.
		}\onslide<+->{%
			注意到 $b_1\ge 1\ge b_n$, 因此
			\[0\ge (b_1-1)(1-b_n)=b_1+b_n-1-b_1b_n.\]
		}\onslide<+->{%
			故 $b_1+\cdots+b_{n-1}\ge n$.
		}\onslide<+->{%
			不难看出等式成立当且仅当 $b_i$ 全为 $1$.\qedhere
		}
	\end{proof*}
\end{frame}
...
\begin{frame}{柯西不等式}
	...
	\begin{corollary}
		...
		\vspace{0.5cm}
	\begin{tikzpicture}[overlay,yshift=4mm,xshift=26mm]
		\begin{scope}[thick,decoration={brace,amplitude=5},second]
			\draw[decorate,visible on=<4->] (2.8,0.1)--(0,0.1);
			\draw[decorate,visible on=<5->] (8.8,0)--(6.2,0);
		\end{scope}
		\draw[second,visible on=<4->] (1.4,-0.4) node {平方平均数};
		\draw[second,visible on=<5->] (7.5,-0.5) node {调和平均数};
	\end{tikzpicture}
	\end{corollary}
	\onslide<+->
	\begin{proof}
