...
\begin{frame}{双边幂级数}
	\onslide<+->
	若解析函数 $f(z)$ 在 $z_0$ 处解析, 则在 $z_0$ 处可以展开成泰勒级数.
	\onslide<+->
	若 $f(z)$ 在 $z_0$ 处不解析呢?
	\onslide<+->
...
\begin{frame}{双边幂级数的收敛域}
	...
	\onslide<+->
	对于负幂次部分, 令 $\zeta=\frac1{z-z_0}$, 则负幂次部分是 $\zeta$ 的一个幂级数 $\suml_{n=1}^\infty c_{-n}\zeta^n$.
	\onslide<+->
	...
	\begin{enumerate}
		...
		\onslide<+->
		此时没有收敛域.
		\item 若 $R_1<R_2$, 则该双边幂级数在 $R_1<|z-z_0|<R_2$ 内收敛, 在 $|z-z_0|<R_1$ 或 $>R_2$ 内发散, 在圆周 $|z-z_0|=R_1$ 或 $R_2$ 上既可能发散也可能收敛.
	\end{enumerate}
\end{frame}
...
\begin{frame}{洛朗级数的特点\noexer}
	\onslide<+->
	若有理函数 $f(z)$ 在圆环域 $r<|z|<R$ 内解析, 则它的洛朗展开一定形如
...
\begin{frame}{洛朗级数的特点\noexer}
	...
	\onslide<+->
	则在 $2<|z|<3$ 内洛朗展开的系数就是上面的每个系数(不论正负 $n$)减去一个共同形式的项, 使得其非负幂次通项对应那些奇点 $|\lambda|\ge 3$, 而负幂次通项对应那些奇点 $|\lambda|\le2$.
	\onslide<+->
	故
