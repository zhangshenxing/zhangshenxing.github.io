...
\begin{frame}{函数在 $\infty$ 的性态}
	\onslide<+->
	\begin{definition}
		若函数 $f(z)$ 在 $\infty$ 的去心邻域 $R<|z|<+\infty$ 内没有奇点, 则称 $\infty$ 是 $f(z)$ 的\emph{孤立奇点}.
	\end{definition}
...
\begin{frame}{例题: $\infty$ 的奇点类型}
	...
	\begin{example*}
		...
		\onslide<+->{事实上, 若函数 $f(z)$ 在复平面上处处解析, 且 $f(z)$ 不是多项式, 则 $\infty$ 是它的本性奇点.
		}
	\end{example*}
\end{frame}
...
\begin{frame}{例题: 证明复数域是代数封闭的\noexer}
	...
	\onslide<+->
	\begin{proof*}
		假设多项式 $p(z)$ 没有复零点, 则 $f(z)=\dfrac1{p(z)}$ 在复平面上处处解析, 
	\onslide<+->{%
