...
\begin{frame}{夹角}
	\onslide<+->
	\begin{definition}
		设 $\bma=(a_1,\dots,a_n)^\rmT,\bmb=(b_1,\dots,b_n)^\rmT\in\BR^n$, 定义 $\bma,\bmb$ 的\emph{夹角}为
		\[\theta=\arccos\dfrac{[\bma,\bmb]}{\|\bma\|\cdot\|\bmb\|}\in[0,\pi].\]
		\onslide<+->{%
			若 $\bma,\bmb$ 夹角为 $\dfrac\pi2$, 即 $[\bma,\bmb]=0$, 称 $\bma,\bmb$ \emph{正交}(垂直).
		}
	\end{definition}
\end{frame}
...
\begin{frame}{格拉姆-施密特正交化}
	\onslide<+->
	由此得到\alert{\cnen{格拉姆}{Gram}-\cnen{施密特}{Schmidt}正交单位化方法:} 取
		\begin{align*}
			\bmb_1&=\bma_1\\
