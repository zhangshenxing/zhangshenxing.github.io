...
\begin{frame}{基}
	\onslide<+->
	我们知道, $\BC^n$ 中任一向量可以\alert{唯一}表为 $\bfe_1,\dots,\bfe_n$ 的线性组合.
	\onslide<+->
	由此引出基的概念:
	\begin{definition}
		如果 $\bma_1,\dots,\bma_m\in V$ 满足: 对任意 $\bfv\in V$, 存在唯一的一组数 $\lambda_1,\dots,\lambda_m$ 使得
...
\begin{frame}{秩}
	...
	\begin{definition}
		...
		称 $V$ 的维数为该向量组的\emph{秩}(\emph{R}ank), 记作 $R(\bma_1,\dots,\bma_m)$.
	\end{definition}
	\onslide<+->
	由于该向量组线性无关当且仅当它们构成一组基, 因此:
	\begin{theorem}
		\begin{enumerate}
...
\begin{frame}{例: 向量组的秩}
	...
	\begin{exercise}
		...
		\bma_3=(3,2,1)^\rmT,\]
		线性表示, 则 $a\neq$\fillblank{\visible<+->{$5$}}.
	\end{exercise}
	\onslide<+->
	\begin{exercise}
		判断题: 设 $S$ 和 $T$ 为两个 $n$ 维向量组, 且 $R(S)=R(T)$, 则 $S$ 和 $T$ 等价.\visible<+->{\alert{\text{\huge$\times$}}}.
	\end{exercise}
\end{frame}
...
\begin{frame}{例: 向量组的秩}
	\onslide<+->
	\begin{example}
		如果 $S_1=\{\bma_1,\bma_2,\bma_3\}$, $S_2=\{\bma_1,\bma_2,\bma_3,\bma_4\}$, $S_3=\{\bma_1,\bma_2,\bma_3,\bma_4,\bma_5\}$ 满足 $R(S_1)=R(S_2)=3, R(S_3)=4$.
...
\begin{frame}{极大线性无关组的性质}
	...
	\begin{enumerate}
		...
		\onslide<+->{
		那么 $\bma_1,\bma_2$ 是一个极大线性无关组, $\bma_1,\bma_3$ 也是一个极大线性无关组.}
