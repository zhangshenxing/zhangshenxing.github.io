...
\begin{frame}{矩阵秩的性质}
	...
	\begin{proposition}
		\begin{enumerate}
			...
			\item $R(a\bfA+b\bfB)\le R(\bfA,\bfB)\le R(\bfA)+R(\bfB)$.
			
			特别地, $\max\bigl(R(\bfA),R(\bfB)\bigr)\le R(\bfA,\bfB)$.
		\end{enumerate}
	\end{proposition}
\end{frame}
...
\begin{frame}{矩阵秩的性质}
	...
	\begin{proof}
		...
		\onslide<+->{%
		则 $S$, $T$ 的大小分别是 $R(\bfA)$, $R(\bfB)$.
		}\onslide<+->{%
		由于 $(\bfA,\bfB)$ 的列向量组和 $S\cup T$ 向量组等价, 因此
		\[R(\bfA,\bfB)=R(S\cup T)\le R(\bfA)+R(\bfB).\]
		}\onslide<+->{%
		由于 $a\bfA+b\bfB$ 的列向量组可以由 $(\bfA,\bfB)$ 的列向量组线性表示, 因此
		\[R(a\bfA+b\bfB)\le R(\bfA,\bfB).\qedhere\]
		}
		\vspace{-\baselineskip}
	\end{proof}
\end{frame}
...
\begin{frame}{矩阵秩的性质}
	...
	\begin{proof}[另证]
		...
		因此 $R(a\bfA+b\bfB)\le R(\bfA,\bfB)\le R(\bfA)+R(\bfB)$.
		}
	\end{proof}
\end{frame}
	...
	\begin{exercise}
		\begin{enumerate}
			\item 设 $R(\bfA)=2,\bfB=\begin{pmatrix}
				...
				0&2&0\\
				-1&0&3
			\end{pmatrix}$, 则 $R(\bfA\bfB)=$\fillblankframe{$2$}.
			\item 若 $\bfA$ 是 $n$ 阶方阵且 $R(\bfA\bfB)<R(\bfB)$, 则 $|\bfA|=$\fillblankframe{$0$}.
			...
			\end{pmatrix},\bfB=\begin{pmatrix}
				...
				1&2&1\\
				4&6&t
			\end{pmatrix},\bfA\bfX=\bfB$ 且 $R(\bfX)=2$, 则 $t=$\fillblankframe{$2$}.
			\item 若 $\bfA=\begin{pmatrix}
				t&2&3\\
				2&1&-1\\
				0&0&5
...
\begin{frame}{例: 矩阵秩性质的应用}
	...
	\begin{exercise}
		\begin{enumerate}
			\item 设 $\bma=(1,0,-1,2)^\rmT,\bmb=(0,1,0,2)^\rmT$, 则 $R(\bma\bmb^\rmT)=$\fillblankframe{$1$}.
			\item 若 $\bfA=\begin{pmatrix}
				a&b&b\\
				b&a&b\\
				b&b&a
	...
	\begin{exercise}
		\begin{enumerate}
			...
			\begin{taskschoice}(2)
				...
				() 当 $m<n$ 时, 必有 $|\bfA\bfB|\neq0$
			\end{taskschoice}
			\item 设 $\bfA\in M_{n\times m},\bfB\in M_{m\times n},n<m$. 若 $\bfA\bfB=\bfE$, 则 $R(\bfB)=$\fillblankframe{$n$}.
			...
			\begin{taskschoice}(4)
				...
				() $a=1$
			\end{taskschoice}
			\item 设四阶方阵 $\bfA=(\bma_1,\bma_2,\bma_3,\bma_4)$ 满足 $\bma_1+\bma_2-2\bma_3={\bf0},\bma_2+5\bma_4={\bf0}$, 则 $R(\bfA^*)=$\fillblankframe{$0$}.
		\end{enumerate}
	\end{exercise}
\end{frame}
