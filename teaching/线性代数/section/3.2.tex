...
\begin{frame}{实二次型的定义}
	...
	\begin{definition}
		...
		它可以写成
		\[f(x)=a_{11}x_1^2+a_{22}x_2^2+\cdots+a_{nn}x_n^2+2a_{12}x_1x_2+\cdots+2a_{n-1,n}x_{n-1}x_n.\]
	\end{definition}
	\onslide<+->
	\alert{本课程仅讨论实二次型}.
	\onslide<+->
...
\begin{frame}{合同、等价、相似的关系}
	...
	合同、等价、相似有如下关系:
	\begin{enumerate}
		\item 若 $\bfA,\bfB$ 合同, 则 $\bfA,\bfB$ 等价, $\rank(\bfA)=\rank(\bfB)$. 反之未必.
...
\begin{frame}{对阵矩阵的性质}
	...
	\begin{exercise}
		\begin{enumerate}
			\item 设 $\bma_1=(1,-3,1)^\rmT,\bma_2=(1,a,2)^\rmT$ 是实对称阵 $\bfA$ 对应特征值 $\lambda_1=1$ 和 $\lambda_2=2$ 的特征向量, 则$a=$\fillblankframe{$1$}.
			\item 若 $3$ 阶实对称阵 $\bfA$ 满足 $\bfA^2=\bfA, \rank(\bfA)=1$, 则 $\bfA$ 的特征值为\fillblankframe[2cm]{$0,0,1$}.
		\end{enumerate}
	\end{exercise}
\end{frame}
...
\beqskip{5pt}
	...
	\onslide<+->
	\begin{solution}
		由于 $\bfA$ 有两个对应特征值 $3$ 的线性无关特征向量, 因此与 $\bma_1$ 正交的向量都是与特征值 $3$ 对应的特征向量.
		\onslide<+->{%
		由 $\bma_1^\rmT\bfx=0$ 得 $\bma_2=(-1,1,0)^\rmT,\bma_3=(-1,0,1)^\rmT$.
		}\onslide<+->{%
		故
		\[
	...
	\onslide<+->
	\begin{solution}[另解]
		根据 $\bfA$ 的行和、迹和对称性可设 $\bfA=\begin{pmatrix}
			a&b&6-a-b\\
			b&9-a-b&a\\
			6-a-b&a&3+b
		\end{pmatrix}$.
		\onslide<+->{%
		再由 $\rank(\bfA-3\bfE)=1$ 可知 $a=4,b=1$.
		}
	\end{solution}
\endgroup
\end{frame}
...
\begin{frame}{实二次型对角化的步骤}
	\onslide<+->
	对称阵正交合同对角化, 或求正交变换 $\bfx=\bfP\bfy$ 将实二次型 $f$ 化为标准形的步骤:
	\begin{enumerate}
		\item 写出 $f$ 对应的对称阵 $\bfA$.
		\item 求出 $\bfA$ 的特征值.
		\item \alert{若特征值是 $k$ 重的, 求出 $k$ 个特征向量后, 用格拉姆-施密特方法将其正交单位化.}
		\item 这些特征向量构成正交阵 $\bfP$, $\bfP^\rmT\bfA\bfP=\diag(\lambda_1,\lambda_2,\dots,\lambda_n)$.
...
\begin{frame}{典型例题: 实二次型的对角化}
	\onslide<+->
	\begin{example}
		设实二次型 $f=a(x_1^2+x_2^2+x_3^2)+4x_1x_2+4x_2x_3+4x_3x_1$ 经正交变换化成标准形 $f(y_1,y_2,y_3)=6y_1^2$, 则 $a=$\fillblankframe{$2$}.
	\end{example}
	\onslide<+->
	\begin{exercise}
