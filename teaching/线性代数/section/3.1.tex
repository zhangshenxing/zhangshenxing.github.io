...
\begin{frame}{例: 相似矩阵的性质}
	\onslide<+->
	\begin{example}
		若 $4$ 阶方阵 $\bfA$ 与 $\bfB$ 相似, $\bfA$ 的特征值为 $1,2,3,4$, 则 $|\bfB+\bfE|=$\fillblankframe{$120$}.
	\end{example}
	\onslide<+->
	\begin{example}
		设$3$ 阶可逆阵 $\bfA,\bfB$ 相似, $\bfA^{-1}$ 的特征值为 $1/2,1/3,1/4$, 则 $|\bfE-\bfB|=$\fillblankframe{$-6$}.
	\end{example}
	\onslide<+->
	\begin{example}
...
\begin{frame}{例: 相似矩阵的性质}
	...
	\begin{exercise}
		\begin{enumerate}
			\item 若 $3$ 阶矩阵 $\bfA$ 与 $\bfB$ 相似, 且 $|\bfA-\bfE|=0, |\bfA+2\bfE|=0, |2\bfA-\bfE|=0$, 则 $|\bfB^{-1}-\bfE|=$\fillblankframe{\visible<4->{$3/4$}}.
			\item 若 $3$ 阶矩阵 $\bfA$ 与 $\bfB$ 相似, 且存在非零矩阵 $\bfC$ 使得 $\bfA\bfC=2\bfC, |\bfA+2\bfE|=0, |2\bfA-\bfE|=0$, 则 $|\bfB^{-1}-\bfE|=$\fillblankframe{\visible<5->{$3/4$}}.
		\end{enumerate}
	\end{exercise}
	\onslide<+->
...
\beqskip{4pt}
	...
	\begin{exercise}
		已知矩阵 $\bfA=\begin{pmatrix}
			...
			a&1&1\\
			1&0&0
		\end{pmatrix}$ 可对角化, 则\fillblankframe{$-1$}.
	\end{exercise}
	\onslide<+->
	\begin{exercise}
