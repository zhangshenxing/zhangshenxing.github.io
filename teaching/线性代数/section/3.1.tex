...
\begin{frame}{特征值和特征向量的性质}
	...
	\begin{example}
		$\begin{pmatrix}
			...
			1&0&1\\
			0&1&1
		\end{pmatrix}$ 的特征值为\fillbraceframe{C}.
		\begin{exchoice}(4)
			() $1,0,1$
			() $1,1,2$
			() $-1,1,2$
...
\begin{frame}{例: 特征值和特征向量的性质}
	...
	\begin{example}
		...
		\begin{enumerate}
			\item $\bfA^2$ 的特征值为\fillblankframe[2cm]{$4,4,9$};
			\item $\bfA^2-2\bfA+\bfE$ 的特征值为\fillblankframe[2cm]{$1,1,4$};
			\item $|\bfA^2-2\bfA+\bfE|=$\fillblankframe{$4$};
			\item $\bfA^{-1}$ 的特征值为\fillblankframe[3cm]{$1/2,1/2,1/3$};
			\item $\bfA^*$ 的特征值为\fillblankframe[2cm]{$4,6,6$};
			\item $A_{11}+A_{22}+A_{33}=$\fillblankframe{$16$}, 其中 $A_{ij}$ 表示 $\bfA$ 的代数余子式.
		\end{enumerate}
	\end{example}
\end{frame}
...
\begin{frame}{例: 特征值和特征向量的性质}
	...
	\onslide<+->
	\begin{exercise}
		若 $4$ 阶方阵 $\bfA$ 的特征值为 $1,2,-2,0$, 则下列矩阵可逆的是\fillbraceframe{C}.
		\begin{exchoice}(4)
			() $\bfA$
			() $\bfA-2\bfE$
			() $\bfA+\bfE$
...
\begin{frame}{例: 特征值的应用\noexer}
	...
	\begin{solution}
		...
		由于 $\Tr(\bfB)=nb$, 因此 $\bfB$ 的所有特征值为 $nb,0,\dots,0$,
		}\onslide<+->{%
		$\bfA$ 的所有特征值为 $nb+a-b,a-b,\dots,a-b$,
		\[
			|\bfA|=(a-b)^{n-1}(nb-b+a).
		\]
		\vspace{-\baselineskip}
...
\begin{frame}{应用: 天气预报}
	\onslide<+->
	将天气简化为晴雨雪三种，其它天气由它们组合得到.
	\onslide<+->
	根据该地历史的天气信息，得到当天与第二天天气的关系:
	\begin{center}
		\begin{tikzpicture}
			\fill[main!50,rounded corners=2mm] (-.75,-.55) rectangle (.75,.55);
			\fill[white] (0,0) circle(.2);
			\foreach \x in {0,45,...,315} {
				\draw[white,line width=.5mm] ({.27*cos(\x)},{.27*sin(\x)}) -- ({.5*cos(\x)},{.5*sin(\x)});
			}
			\begin{scope}[xshift=60mm]
				\fill[second!50,rounded corners=2mm] (-.75,-.55) rectangle (.75,.55);
				\fill[white] (-.45,0) circle (.15);
				\fill[white] (0,0) arc (0:180:.25);
				\fill[white] (.43,0.07) arc (0:180:.35);
				\fill[white] (.45,0) circle (.15);
				\fill[white] (-.45,.15) rectangle (.45,-.15);
				\begin{scope}[yshift=-4mm,scale=.06]
					\fill[white] ({-cos(15)},{sin(15)})--(0,{cos(15)/sin(15)})--({cos(15)},{sin(15)})--cycle;
					\fill[white] ({-cos(15)},{sin(15)+0.1}) arc (165:365:1);
				\end{scope}
				\begin{scope}[xshift=3mm,yshift=-4mm,scale=.06]
					\fill[white] ({-cos(15)},{sin(15)})--(0,{cos(15)/sin(15)})--({cos(15)},{sin(15)})--cycle;
					\fill[white] ({-cos(15)},{sin(15)+0.1}) arc (165:365:1);
				\end{scope}
				\begin{scope}[xshift=-3mm,yshift=-4mm,scale=.06]
				...
				\end{scope}
			\end{scope}
			\begin{scope}[xshift=30mm,yshift=-30mm]
				\fill[fourth!50,rounded corners=2mm] (-.75,-.55) rectangle (.75,.55);
				\fill[white] (-.45,0) circle (.15);
				\fill[white] (0,0) arc (0:180:.25);
				\fill[white] (.43,0.07) arc (0:180:.35);
				\fill[white] (.45,0) circle (.15);
				\fill[white] (-.45,.15) rectangle (.45,-.15);
				\begin{scope}[yshift=-3.5mm,scale=.15]
					\draw[white,line width=.5mm] (0,-1)--(0,1);
					\draw[white,line width=.5mm] (-.866,-.5)--(.866,.5);
					\draw[white,line width=.5mm] (.866,-.5)--(-.866,.5);
				\end{scope}
				\begin{scope}[xshift=3mm,yshift=-3.5mm,scale=.15]
					\draw[white,line width=.5mm] (0,-1)--(0,1);
					\draw[white,line width=.5mm] (-.866,-.5)--(.866,.5);
					\draw[white,line width=.5mm] (.866,-.5)--(-.866,.5);
				\end{scope}
				...
				\end{scope}
			\end{scope}
			\draw[cstra,cstcurve,main] (0,.55) |- (-1.2,.8) |- node [above left] {0.75} (-.75,0);
			\draw[cstra,cstcurve,main] (5.25,-.2) -- node [below] {0.1} (.75,-.2);
			\draw[cstra,cstcurve,main] (2.25,-3.1) -- node [below left] {0.05} (-.2,-.55);

			\draw[cstra,cstcurve,second] (.75,.2) -- node [above] {0.15} (5.25,.2);
			\draw[cstra,cstcurve,second] (6,.55) |- node [above right] {0.8} (7.2,.8) |- (6.75,0) ;
			\draw[cstra,cstcurve,second] (3.75,-3.1) -- node [below right] {0.1} (6.2,-.55);
			
			\draw[cstra,cstcurve,fourth] (.2,-.55) -- node [above right] {0.1} (2.25,-2.7);
			\draw[cstra,cstcurve,fourth] (5.8,-.55) -- node [above left] {0.1} (3.75,-2.7);
			\draw[cstra,cstcurve,fourth] (2.6,-3.55) |- node [below right] {0.85} (3,-3.9) -| (3.4,-3.55) ;
		\end{tikzpicture}
	\end{center}
\end{frame}
...
\begin{frame}{应用: 天气预报}
	\onslide<+->
	\begin{example}
		某地某季节天气仅受前一天天气状态影响: 设 $k$ 天后天气为晴天、雨天、雪天概率分别为 $a_k,b_k,c_k$, 则
		\[\begin{pmatrix}
			a_k\\b_k\\c_k
		\end{pmatrix}=\bfA\begin{pmatrix}
			a_{k-1}\\b_{k-1}\\c_{k-1}
		\end{pmatrix},\qquad \bfA=\begin{pmatrix}
			0.75&0.1&0.05\\
			0.15&0.8&0.1\\
			0.1&0.1&0.85
		\end{pmatrix}.\]
		如果今天天气为晴天，请问未来七天的各种天气概率分别是多少?
	\end{example}
\end{frame}
...
\begin{frame}{应用: 天气预报}
	...
	\onslide<+->
	\begin{solution}
		设 $\bfx_k=(a_k,b_k,c_k)^\rmT$, 则 $\bfx_k=\bfA \bfx_{k-1}$.
		\onslide<+->{%
			因此 $x_k=\bfA x_{k-1}=\bfA^2 x_{k-2}=\cdots=\bfA^k \bfx_0$.
		}\onslide<+->{%
			解特征多项式
			\[|\bfA-\lambda\bfE|=\begin{vmatrix}
				0.75-\lambda&0.1&0.05\\
				0.15&0.8-\lambda&0.1\\
				0.1&0.1&0.85-\lambda
			\end{vmatrix}=0\]
			得到特征值 $\lambda_1=1,\lambda_2=0.75,\lambda_3=0.65$.
		}\onslide<+->{%
			解方程 $(\bfA-\lambda_i\bfE)\bfx={\bf0}$ 得到特征向量
			\[\bfv_1=(8,13,14)^\rmT,\quad \bfv_2=(1,1,-2)^\rmT,\quad \bfv_3=(1,-1,0)^\rmT.\]
		}\onslide<+->{%
			由于今天天气是晴天, 所以 $\bfx_0=(1,0,0)^\rmT$.
		}\onslide<+->{%
			注意到 $\bfv_1,\bfv_2,\bfv_3$ 线性无关, 因此 $\bfx_0$ 可以由它们线性表示.
		}\onslide<+->{%
			通过计算得到 $\bfx_0=\dfrac1{35} \bfv_1+\dfrac15 \bfv_2+\dfrac47 \bfv_3$,
			因此
			\[\bfx_k=\bfA^k\bfx_0=\frac1{35} \lambda_1^k \bfv_1+\frac15 \lambda_2^k \bfv_2+\frac47 \lambda_3^k \bfv_3.\]
		}
...
\begin{frame}{应用: 天气预报}
	...
	\begin{solution}[续解]
			\onslide<+->{%
			代入计算可得 $\bfx_k$, 从而得到未来七天晴雨雪的概率:
			\begin{center}
				\begin{tabular}{cccccccc} \toprule
		&明天& 后天& $3$ 天后& $4$ 天后& $5$ 天后& $6$ 天后& $7$ 天后\\ \midrule
		晴&$0.75$&$0.58$&$0.47$&$0.39$&$0.34$&$0.31$&$0.28$\\
		雨&$0.15$&$0.24$&$0.30$&$0.33$&$0.35$&$0.36$&$0.37$\\
		雪&$0.10$&$0.18$&$0.23$&$0.27$&$0.31$&$0.33$&$0.35$\\
		\bottomrule
				\end{tabular}
				\begin{tikzpicture}[overlay,yshift=12.55mm,main]
					\draw[cstcurve] (-10.88,0)--(-.12,0);
					\draw[cstcurve] (-10.88,-2.32)--(-.12,-2.32);
					\draw[thick] (-10.88,-.68)--(-.12,-.68);
				\end{tikzpicture}
			\end{center}
		}
	\end{solution}
...
\begin{frame}{相似与等价}
	...
	\onslide<+->
	\begin{example}
		若 $3$ 阶方阵 $\bfA\wsim{r_1\swap r_3}{}\bfB\wsim{c_1\swap c_3}{}\bfC$, 则 $\bfA$ 与 $\bfC$ \fillbraceframe{C}.
		\begin{exchoice}(2)
			() 等价但不相似
			() 相似但不等价
			() 等价而且相似
...
\begin{frame}{例: 相似矩阵的性质}
	...
	\begin{example}
		...
		\end{pmatrix}$ 与 $\bmL=\begin{pmatrix}
			...
			0&2&0\\
			0&0&b
		\end{pmatrix}$ 相似, 则 $a=$\fillblankframe{$1$}, $b=$\fillblankframe{$3$}．
	\end{example}
	\onslide<+->
	\begin{example}
...
\begin{frame}{可对角化的刻画}
	...
	\begin{solution}
		...
		\onslide<+->{%
		于是
		\[\bfP^{-1}\bfA\bfP=\bmL^3-4\bmL+\bfE=\diag(-2,1,1).\]
		}\vspace{-\baselineskip}
	\end{solution}
\end{frame}
...
\begin{frame}{例: 可对角化的刻画}
	...
	\begin{solution}
		\begin{enuminline}
			...
			\item 特征值一正一负, 能对角化;}
			\onslide<+->{%
			\item 特征值为 $1,3,0$, 能对角化;\par
			}
			\onslide<+->{%
			\item	$(2-\lambda)(\lambda^2-\lambda-2)\implies\lambda=-1,2,2$.
			}\onslide<+->{%
			$\bfA-2\bfE=\begin{pmatrix}
				-4&1&1\\
				0&0&0\\
				-4&1&1
			\end{pmatrix}$ 秩 $1$, 能对角化;\par
			}
			\onslide<+->{%
			\item $(2-\lambda)(\lambda^2-2\lambda+2)\implies\lambda=2,1,1$.
		}\onslide<+->{%
			$\bfA-\bfE=\begin{pmatrix}
				-2&1&0\\
				-4&2&0\\
	...
	\begin{exercise}
		\begin{enumerate}
			\item 若 $\bfA=\begin{pmatrix}
				...
				a&1&1\\
				1&0&0
			\end{pmatrix}$ 可对角化, 则 $a=$\fillblankframe{$-1$}.
			\item 设 $\bfP=(\bma_1,\bma_2,\bma_3),\bfQ=(\bma_1,\bma_3,\bma_2)$.
			若 $\bfP^{-1}\bfA\bfP=\diag(1,2,3)$, 则 $\bfQ^{-1}\bfA\bfQ=$\fillblankframe[2cm]{$\diag(1,3,2)$}.
			\item 若 $3$ 阶方阵 $\bfA$ 的特征值互不相同且 $|\bfA|=0$, 则 $\rank(\bfA)=$\fillblankframe{$2$}.
			\item 若 $\bfA=\begin{pmatrix}
				2&0&0\\
				0&2&1\\
				0&0&1
...
\begin{frame}{例: 对角化的计算}
	\onslide<+->
	\begin{example}
		若 $\bfA$ 的各行元素之和为 $2$, $\bfA\begin{pmatrix}
			1&2\\
			0&1\\
			-1&1
		\end{pmatrix}=\begin{pmatrix}
			...
			0&1\\
			1&1
		\end{pmatrix}$, 则 $\bfA$ 相似于\fillbraceframe{C}.
...
\begin{frame}{例: 对角化的计算}
	...
	\onslide<+->
	\begin{example}
		设 $\bfA$ 为三阶方阵, $\bma_1,\bma_2,\bma_3$ 是线性无关的三维列向量且
		\[
			\bfA\bma_1=2\bma_1,\bfA\bma_2=3\bma_2+2\bma_3,\bfA\bma_3=2\bma_2+3\bma_3.
		\]
		证明 $\bfA$ 可对角化.
	\end{example}
	\onslide<+->
	\begin{solution}
		设 $\bfP=(\bma_1,\bma_2,\bma_3)$, 则
		\[
			\bfA\bfP=\bfA(\bma_1,\bma_2,\bma_3)=(\bma_1,\bma_2,\bma_3)\bfB=\bfP\bfB,\quad\text{其中}\ \bfB=\begin{pmatrix}
				2&0&0\\
				0&3&2\\
				0&2&3
			\end{pmatrix}.
		\]
		\onslide<+->{%
		由于 $\bfB$ 的特征值是 $2,1,5$, 因此 $\bfB$ 能对角化, 从而 $\bfA=\bfP\bfB\bfP^{-1}$ 也可以.}
	\end{solution}
	\onslide<+->
	一般地, 实对称矩阵一定能对角化. 我们将在下一节中解释这为何成立.
	\endgroup
\end{frame}
...
\begin{frame}{伴随矩阵的特征值}
	...
	\onslide<+->
	\begin{exercise}
		若 $4$ 阶实矩阵 $\bfA^*$ 的特征值为 $-1,1,2,4$, 则下列矩阵可逆的是\fillbraceframe{D}.
		\begin{exchoice}(4)
			() $\bfA+2\bfE$
			() $\bfA-2\bfE$
			() $2\bfA+\bfE$
