...
\beqskip{1pt}
	...
	$a_n=\frac1{\sqrt5}\biggl(\Bigl(\frac{1+\sqrt5}2\Bigr)^n-\Bigl(\frac{1-\sqrt5}2\Bigr)^n\biggr)$,
	\onslide<+->
...
\begin{frame}{相似的应用: 求线性递推数列通项}
	...
	$\bfx_n=(a_{n+k-1},\dots,a_{n+1},a_n)^\rmT$ 满足 $\bfx_{n+1}=\bfA\bfx_n$, 其中 $\bfA=\begin{pmatrix}
		...
		&\ddots&&\\
		&&1&
	\end{pmatrix}$ 的特征多项式为(可能相差 $-1$ 倍)
	\[\lambda^k-c_{k-1}\lambda^{k-1}-\cdots-c_1\lambda-c_0=0.\]
	\onslide<+->
...
\begin{frame}{相似的应用: 求常系数微分方程的解}
	...
	\onslide<+->
	从而 $y$ 是 $e^{\lambda_1},\dots,e^{\lambda_k}$ 的线性组合.
	\onslide<+->
