...
\begin{frame}{维数和秩}
	...
	\begin{theorem}
		\begin{enumerate}
			\item 设向量组 $S$ 可由向量组 $T$ 线性表示, 则 $\rank(S)\le \rank(T)$.
			\item 若线性空间 $V\subseteq W$, 则 $\dim V\le\dim W$.
			\item 设向量组 $S$ 可由向量组 $T$ 线性表示, 且 $\rank(S)=\rank(T)$, 则 $S,T$ 向量组等价.
			\item 若线性空间 $V\subseteq W$ 且 $\dim V=\dim W$, 则 $V=W$.\label{enum:same-dim-subspace-same}
		\end{enumerate}
	\end{theorem}
	\onslide<+->
	我们来证明\enumref{enum:same-dim-subspace-same}.
	\onslide<+->
	设 $S,T$ 是 $V,W$ 的一组基.
	\onslide<+->
...
\begin{frame}{例: 向量组的秩}
	...
	\begin{example}
		...
		满足 $\rank(S_1)=\rank(S_2)=3, \rank(S_3)=4$.
		证明向量组 $S=\{\bma_1,\bma_2,\bma_3,\bma_5-\bma_4\}$ 线性无关.
	\end{example}
	\onslide<+->
	\begin{proof}
		由 $\rank(S_1)=3$ 可知 $S_1$ 线性无关.
		\onslide<+->{%
			由 $\rank(S_2)=3$ 可知 $S_2$ 线性相关.
		}\onslide<+->{%
			从而 $\bma_4$ 可由 $S_1$ 线性表示.
		}\onslide<+->{%
			于是 $S_3$ 可由 $S$ 线性表示.
		}\onslide<+->{%
			显然 $S$ 可由 $S_3$ 线性表示, 因此二者等价, $\rank(S)=\rank(S_3)=4$.\qedhere
		}
	\end{proof}
	\onslide<+->
	\begin{exercise}
		判断题: 设 $S$ 和 $T$ 为两个 $n$ 维向量组, 且 $\rank(S)=\rank(T)$, 则 $S$ 和 $T$ 等价.\onslide<+->{\alert{\text{\huge$\times$}}}
	\end{exercise}
\end{frame}
...
\begin{frame}{极大线性无关组的定义}
	...
	\onslide<+->
	换言之, $S_0$ 和 $S$ 等价, 它们生成相同的子空间 $V$, $m=\rank(S)$, $S_0$ 是 $V$ 的一组基.
\end{frame}
...
\begin{frame}{极大线性无关组的性质}
	\begin{enumerate}
		\item 若 $\rank(S)=r$, 则 $S$ 中任意 $r$ 个线性无关的向量构成 $S$ 的一个极大线性无关组.
