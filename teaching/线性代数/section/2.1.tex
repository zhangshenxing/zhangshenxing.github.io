...
\begin{frame}{例: 判断线性无关}
	...
	\begin{proof}
		...
		\onslide<+->{则 $|\bfA|=2$, $\bfA$ 可逆,
			...
			如果 $(\bma_1+\bma_2,\bma_2+\bma_3,\bma_3+\bma_1)\bfx={\bf0}$, 则
...
\begin{frame}{例: 判断线性无关}
	...
	\begin{exercise}
		...
		\end{pmatrix},\bma=\begin{pmatrix}
			k\\1\\1
		\end{pmatrix}$.
		如果 $\bfA\bma$ 和 $\bma$ 线性相关, 则 $k=$\fillblank{\visible<+->{$-1$}}.
	\end{exercise}
\end{frame}
...
\begin{frame}{线性相关和线性无关的等价刻画}
	...
	\onslide<+->
	\begin{proof*}
		如果该向量组线性相关, 则存在不全为零的数 $\lambda_1,\dots,\lambda_m$ 使得
		\[\lambda_1\bma_1+\cdots+\lambda_m\bma_m={\bf0}.\]
		\onslide<+->{%
			设 $\lambda_i\neq 0$, 则
			$\bma_i=-\displaystyle\frac1{\lambda_i}\sum_{j=1\atop j\neq i}^m \lambda_j \bma_j$
			可由其它向量线性表示.
		}

		\onslide<+->{
			反之, 若 $\bma_i=\displaystyle\sum_{j=1\atop j\neq i}^m \lambda_j \bma_j$ 
			可由其它向量线性表示.
		}\onslide<+->{%
			则 $\displaystyle -\bma_i+\sum_{j=1\atop j\neq i}^m \lambda_j \bma_j={\bf0}$, 
...
\begin{frame}{线性相关和线性无关的等价刻画}
	...
	\onslide<+->
	\begin{exercise}
		设 $\bma_1,\dots,\bma_m$ 是 $m$ 个 $n$ 维向量, 则下列结论是否正确的有\fillblank{\visible<+->{1}}个.
		\begin{enumerate}
...
\begin{frame}{例: 线性相关和线性无关}
	...
	\begin{proof}
		\begin{enumerate}
			...
			\item 如果 $\bma_4$ 能由 $\bma_1,\bma_2,\bma_3$ 线性表示, 由于 $\bma_1$ 能由 $\bma_2,\bma_3$ 线性表示, 于是 $\bma_4$ 也能由 $\bma_2,\bma_3$ 线性表示.
