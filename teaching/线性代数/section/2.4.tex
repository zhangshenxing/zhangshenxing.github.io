...
\begin{frame}{例: 计算矩阵的秩}
	...
	\begin{solution}
		...
		\end{pmatrix}\visible<+->{\wsim{r_3-ar_4}{-\frac12r_3}\begin{pmatrix}
			...
			0&0&a\\
			0&0&a+1
		\end{pmatrix}}\visible<+->{\sim\begin{pmatrix}
			1&1&a\\
			0&a&1\\
			0&0&1\\
			0&0&0
...
\begin{frame}{矩阵秩的性质}
	...
	\begin{proof}
		...
		\onslide<+->{%
		由于\[\bfA+\bfB=(\bfE,\bfE)\begin{pmatrix}
			\bfA&\\&\bfB
		\end{pmatrix}\begin{pmatrix}
			\bfE\\\bfE
		\end{pmatrix},\]
...
\begin{frame}{矩阵秩的性质}
	...
	\begin{proof}
		...
		因此 $R(\bfA)\le R(\bfA,\bfB)\le R\begin{pmatrix}
			\bfA&\\&\bfB
		\end{pmatrix}=R(\bfA)+R(\bfB)$.
		}
	\end{proof}
	...
	\begin{exercise}
		\begin{enumerate}
			\item 设 $R(\bfA)=2,\bfB=\begin{pmatrix}
				...
				0&2&0\\
				-1&0&3
			\end{pmatrix}$, 则 $R(\bfA\bfB)=$\fillblank{\visible<6->{$2$}}.
			\item 若 $\bfA$ 是 $n$ 阶方阵且 $R(\bfA\bfB)<R(\bfB)$, 则 $|\bfA|=$\fillblank{\visible<7->{$0$}}.
			...
			\end{pmatrix},\bfB=\begin{pmatrix}
				...
				1&2&1\\
				4&6&t
			\end{pmatrix},\bfA\bfX=\bfB$ 且 $R(\bfX)=2$, 则 $t=$\fillblank{\visible<8->{$2$}}.
			\item 若 $\bfA=\begin{pmatrix}
				t&2&3\\
				2&1&-1\\
				0&0&5
			\end{pmatrix}$ 且存在非零矩阵 $\bfB$ 使得 $\bfA\bfB=\bfO$, 则 $t=$\fillblank{\visible<9->{$4$}}.
		\end{enumerate}
	\end{exercise}
\end{frame}
...
\begin{frame}{例: 矩阵秩性质的应用}
	...
	\begin{exercise}
		\begin{enumerate}
			\item 设 $\bma=(1,0,-1,2)^\rmT,\bmb=(0,1,0,2)^\rmT$, 则 $R(\bma\bmb^\rmT)=$\fillblank{\visible<5->{$1$}}.
			\item 若 $\bfA=\begin{pmatrix}
				a&b&b\\
				b&a&b\\
				b&b&a
			\end{pmatrix}$ 且 $R(\bfA^*)=1$, 则\fillbrace{\visible<6->{B}}.
			\begin{taskschoice}(2)
				() $a\neq b,a+2b\neq 0$
				() $a\neq b,a+2b=0$
				() $a=b,a\neq 0$
				() $a=b=0$
			\end{taskschoice}
			\item 设 $\bfA,\bfB$ 均为 $n$ 阶非零矩阵, 且 $\bfA\bfB=\bfO$, 则 $R(\bfA)$ 与 $R(\bfB)$\fillbrace{\visible<7->{B}}.
			\begin{taskschoice}(2)
				() 必有一个等于 $0$
				() 都小于 $n$
				() 都等于 $n$
	...
	\begin{exercise}
		\begin{enumerate}
			...
			\item 设 $\bfP$ 为 $3$ 阶非零矩阵, $\bfQ=\begin{pmatrix}
				...
				2&4&t\\
				3&6&9
			\end{pmatrix}$ 且 $\bfP\bfQ=\bfO$, 则\fillbrace{\visible<4->{A}}.
			\begin{taskschoice}(2)
				() $t\neq 6$ 时, $R(\bfP)=1$
				() $t\neq 6$ 时, $R(\bfP)=2$
				() $t=6$ 时, $R(\bfP)=1$
				() $t=6$ 时, $R(\bfP)=2$
			\end{taskschoice}
			\item 设 $\bfA,\bfB$ 为 $n$ 阶方阵, 则\fillbrace{\visible<5->{A}}.
			\begin{taskschoice}(2)
				() $R(\bfA,\bfA\bfB)=R(\bfA)$
				() $R(\bfA,\bfB\bfA)=R(\bfA)$
...
\begin{frame}{例: 矩阵秩性质的应用}
	\begin{exercise}
		\begin{enumerate}[<*>]
			\setcounter{enumi}{5}
			\item 设 $\bfA\in M_{m\times n},\bfB\in M_{n\times m}$, 则\fillbrace{\visible<2->{A}}.
			\begin{taskschoice}(2)
				...
				() 当 $m<n$ 时, 必有 $|\bfA\bfB|\neq0$
			\end{taskschoice}
			\item 设 $\bfA\in M_{n\times m},\bfB\in M_{m\times n},n<m$. 若 $\bfA\bfB=\bfE$, 则 $R(\bfB)=$\fillblank{\visible<3->{$n$}}.
			\item 若 $\begin{pmatrix}
				1&1&1\\0&1&-1\\2&3&a+2
			\end{pmatrix}$ 和 $\begin{pmatrix}
				1&2&2\\2&1&1\\a+3&a+6&a+4
			\end{pmatrix}$ 等价, 则\fillbrace{\visible<4->{B}}.
			\begin{taskschoice}(4)
				() $a=-1$
				() $a\neq-1$
				() $a\neq 1$
				() $a=1$
			\end{taskschoice}
			\item 设四阶方阵 $\bfA=(\bma_1,\bma_2,\bma_3,\bma_4)$ 满足 $\bma_1+\bma_2-2\bma_3={\bf0},\bma_2+5\bma_4={\bf0}$, 则 $R(\bfA^*)=$\fillblank{\visible<5->{$0$}}.
		\end{enumerate}
	\end{exercise}
\end{frame}
...
\begin{frame}{例: 秩与线性相关}
	...
	\begin{exercise}
		\begin{enumerate}
			\item 设 $\bfA$ 是 $n$ 阶方阵, 且其行列式 $|\bfA|=0$. 下列说法正确的是\fillbrace{\visible<4->{C}}.
			\begin{taskschoice}(1)
				() $\bfA$ 必有一列元素全为零
				() $\bfA$ 必有两列元素对应成比例
			...
			\item 若
				...
				\bma_2=(0,1,5,0)^\rmT,
				\bma_3=(2,1,t+1,4)^\rmT\]
				线性相关, 则 $t=$\fillblank{\visible<5->{$3$}}.
		\end{enumerate}
	\end{exercise}
\end{frame}
...
\begin{frame}{例: 线性相关与线性无关}
	...
	\begin{exercise}
		\begin{enumerate}
			\item 设矩阵 $\bfA$ 经初等行变换化为 $\bfB$, 则二者的\fillbrace{\visible<4->{A}}.
			\begin{taskschoice}(1)
				() 行向量组等价, 列向量组同相关性
				() 行向量组同相关性, 列向量组等价
				() 行向量组未必等价, 列向量组同相关性
