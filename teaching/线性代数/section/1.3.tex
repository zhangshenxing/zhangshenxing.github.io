...
\begin{frame}{克拉默法则的证明}
	...
	\begin{proofe}
		...
		\onslide<+->{
			...
			将所有的 $j\neq i$ 列乘 $x_j$ 并加到第 $i$ 列, 则第 $i$ 列元素变成了 $b_1,\dots,b_n$.
		}\onslide<+->{%
		因此 $x_i|\bfA|=|\bfA_i|$.
		}
	\end{proofe}
	\onslide<+->
	后面我们将会知道, $|\bfA|\neq0$ 实际上方程有唯一解的充分必要条件.
	\onslide<+->
	因此我们定义的行列式确实起到了线性方程组的``判别式''的作用.
	
	\onslide<+->
	注意到如果 $(a_1,\dots,a_n)$ 和 $(b_1,\dots,b_n)$ 都是上述方程组的解, 那么 $x_i=\lambda a_i+(1-\lambda)b_i$ 也是该方程组的解.
	\onslide<+->
	也就是说, 如果上述方程组解不唯一, 那么一定有无穷多解.
	\onslide<+->
	所以当 $|\bfA|=0$, 那么方程无解或有无穷多解.
\end{frame}
...
\begin{frame}{齐次线性方程组}
	...
	称之为\emph{齐次线性方程组}. 否则称之为\emph{非齐次线性方程组}.

	\onslide<+->
	显然 $x_1=\cdots=x_n=0$ 是齐次线性方程组的解, 称为\emph{零解}. 其它解被称为\emph{非零解}.
	\onslide<+->
	因此此时系数矩阵行列式 $|\bfA|=0\iff$ 方程有(无穷多)非零解.
	\onslide<+->
	\begin{exercise}
		如果 $\laeq{
...
\begin{frame}{克拉默法则的应用}
	...
	\begin{proofe}
		...
		\end{align*}
		\onslide<+->{由于这是三条不同直线, 因此 $a,b,c$ 不可能全部相等, 从而 $a+b+c=0$.}
	\end{proofe}
	\onslide<+->
