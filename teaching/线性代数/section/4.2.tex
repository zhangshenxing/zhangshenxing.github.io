...
\begin{frame}{函数项级数与幂级数}
	...
	\begin{definition}
		\begin{itemize}
			...
			\item 对于 $z_0\in D$, 若级数 $\suml_{n=1}^\infty f_n(z_0)$ 收敛, 则称 \emph{$\suml_{n=1}^\infty f_n(z)$ 在 $z_0$ 处收敛}, 相应级数的值称为它的\emph{和}.
			\item 若 $\suml_{n=1}^\infty f_n(z)$ 在 $D$ 上处处收敛, 则它的和是一个函数, 称为\emph{和函数}.
...
\begin{frame}{阿贝尔定理}
	...
	\begin{block}{阿贝尔定理}
		\begin{enumerate}
			\item 若 $\suml_{n=0}^\infty c_nz^n$ 在 $z_0\neq 0$ 处收敛, 则对任意 $|z|<|z_0|$ 的 $z$, 该级数必绝对收敛.
...
\begin{frame}{幂级数的收敛半径}
	...
	\begin{itemize}
		\item 若 $R=+\infty$, 由阿贝尔定理可知 $\suml_{n=0}^\infty c_nz^n$ 处处绝对收敛.
		\item 若 $0<R<+\infty$, 则 $\suml_{n=0}^\infty c_nz^n$ 在 $|z|<R$ 上绝对收敛, 在 $|z|>R$ 上发散.
		\item 若 $R=0$, 则 $\suml_{n=0}^\infty c_nz^n$ 仅在 $z=0$ 处收敛, 对任意 $z\neq 0$ 都发散.
	\end{itemize}
	\onslide<+->
...
\begin{frame}{典型例题: 收敛半径的计算}
	\onslide<+->
	\begin{exercise}
		幂级数 $\suml_{n=0}^\infty(1+i)^nz^n$ 的收敛半径为\fillblank[2cm][2mm]{\visible<+->{$\dfrac{\sqrt2}2$}}.
	\end{exercise}
\end{frame}
...
\begin{frame}{幂级数的解析性质}
	...
	\onslide<+->
	若函数 $g(z)$ 在该幂级数收敛的点处和 $f(z)$ 均相同, 则 $g(z)$ \alert{一定在收敛圆周上有奇点}.
	\onslide<+->
