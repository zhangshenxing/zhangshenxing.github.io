...
\begin{frame}{抓石子游戏}
  \begin{itemize}
    \item 幼儿园里有两个小朋友 \alertn{A}lice和\emphn{B}ob, 他们从地上抓起一把石子, 然后从Alice开始, 轮流从石子堆中取走石子.
  ...
  \begin{itemize}
    ...
    \begin{center}
      ...
      \begin{tikzpicture}
        ...
        \draw[visible on=<9>] (2,0) node[below=7mm] {\small 小赢?中赢?大赢?};
        \draw[visible on=<10->,red] (2,0) node[below=3mm] {$2$};
        \draw[visible on=<10>,red] (2,0) node[below=7mm] {\small 中赢!};
        ...
        \end{scope}
        \draw[visible on=<11->,red] (3,0) circle (\r) node {$3$} node[below=3mm] {$3$};
        ...
        \end{scope}
        \draw[visible on=<12->,blue] (4,0) circle (\r) node {$4$} node[below=3mm] {$0$};
        \begin{scope}[visible on=<13->,red]
      ...
      \end{tikzpicture}}
    \end{center}
    \onslide<+->\onslide<+->\onslide<+->\onslide<+->\onslide<+->\onslide<+->\onslide<+->\onslide<+->\onslide<+->\onslide<+->\onslide<+->
    ...
    \begin{center}
      \begin{tabular}{cccc}
        ...
        \begin{tikzpicture}[overlay,shift={(-.1,.2)}]
          \draw[decorate,decoration={brace,amplitude=5},blue,thick] (0,-1.3)--(0,0);
          \draw (-1.5,-.65) node[align=left] {$c>b+1$\smallskip\\$r\le b-4$ 偶};
        \end{tikzpicture}
        $r>b-2t-2$&
...
\begin{frame}{三元集合: $S=\set{1,b,c}$ 的复杂情形}
  \onslide<+->
  此时 $\CG$ 已经较为复杂.
  \onslide<+->
  例如: 若 $0<r=2v<b-2t-2$, 则
  \begin{center}
    \begin{tabular}{cl}
      \topcolorrule
...
\begin{frame}{应用: 最终二分序列}
  ...
  \begin{theorem}
    ...
    如果 $S$ 是如下情形之一:
    \begin{enumerate}[<*>]
      \item $S=\set{a,a+2,(2a+2)t+1}$;\label{enum:bi1}
