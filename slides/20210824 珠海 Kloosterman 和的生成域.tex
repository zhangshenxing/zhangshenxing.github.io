...
\author{张神星}
\institute{2021年珠海代数数论研讨会\quad 广东珠海}
\date{2021年8月24日}
...
\frame{
\frametitle{$L$ 函数}
前两个问题已经有相当多的文献中研究过. 我们简要回顾下指数和的基本性质.
...
\frame{\frametitle{主要结论}
\begin{theorem}
若 $p$ 充分大(相对于 $n,d,\bchi$) 且 $\chi_i$ 之间不相差非平凡的 $n$ 次(不必本原)特征, 则 $\Kl_n(\psi,\bchi,q,a)$ 生成 $\BQ(\mu_{pc})^H$, 其中 $H$ 包含的 $\sigma_t\tau_w$ 满足: 存在整数 $\beta$ 和特征 $\eta$ 使得
...
\begin{theorem}
若 $p>\max\set{(2n^{2d}+1)^2,(3n-1)C_\bchi-n}$ 且 $\chi_i$ 之间不相差非平凡的 $n$ 次特征, 则 $\Kl_n(\psi,\bchi,q,a)$ 生成 $\BQ(\mu_{pc})^H$, 其中 $H$ 包含的 $\sigma_t\tau_w$ 满足: 存在整数 $\beta$ 和特征 $\eta$ 使得
  \[t=\lambda a_1^\beta,\lambda^{n_1}=1,\ \bchi^w=\eta\bchi^{q_1^\beta},\ \eta(a)=\prod\bchi^w(t).\]
这里, $n_1=(n,p-1)$, $q_1=\#\BF_p(a^{(p-1)/n_1})$, $a_1\in\BF_p^\times$ 满足 $a_1^{n/n_1}=\bfN_{\BF_{q_1}/\BF_p}(a^{(1-p)/n_1})=a^{(1-q_1)/n_1}$.
\end{theorem}
}
...
由于 $L$ 函数
	\[L(t,\Kl)=\exp\left(\sum_{k=1}^\infty S_k\frac{t^k}{k}\right)\]
...
然而目前我们仅能证明
	\[r\mid p^n(n!,p^n)\prod_{p\neq\ell\le n+1}\ell^{[n/(\ell-1)]}.\]
最后我们(大胆)猜测下 $p>3ndc$ 时成立.
}
