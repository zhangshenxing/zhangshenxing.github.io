...
\title{不同椭圆曲线的二次扭之比较}
\author{张神星}
\invitedby[nju]{南京大学~金陵数论与代数几何会议}
\date{2022年9月25日}
...
\begin{itemize}
  ...
  \item 如果下述三种情形之一成立:
	...
	\item $a_i,b_i$ 是奇数且 $2\mmid c_i$; 
	\hfill\emph{(例如 $y^2=x(x-1)(x+1)$)}
	\item $2\mmid a_i,b_i,4\mid c_i$, 且 $E_i^{(n)}$ 没有 $4$ 阶有理点, 
	\hfill\emph{(例如 $y^2=x(x-2)(x+2)$)}
	\end{itemize}
...
\begin{frame}{计算 Selmer 群: 比较 $\Sel_2'\bigl(E^{(n)}\bigr)$ 和 $\Sel_2'(E)$}
	...
	因此
	\[
		\alertm{\Sel_2'\bigl(E^{(n)}\bigr)=\Sel_2\bigl(E^{(n)}\bigr)/E[2]}
	\]
...
\[0\ra \frac{E(\BQ)[2]}{2E(\BQ)[4]}\to\Sel_2(E)\to\Sel_4(E)\to\Im\Sel_4(E)\to 0.\]
  ...
  \item 因此 \alert{Cassels 配对非退化等价于}
	\[
		\alertm{\rank_\BZ E(\BQ)=0,\quad \Sha(E/\BQ)[2^\infty]\cong(\BZ/2\BZ)^{2t}.}
	\]
\end{itemize}
\end{frame}
...
其中 $A,B,C$ 是互素的非零奇数.
  ...
  \item 若能选取适当的 $Q_{i,j}$ 和 $P_{i,v}$, 使得
	\[
		\alertm{[L_{1,i}(P_{1,v}),d_i']_v}=\emphm{[L_{2,i}(P_{2,v}),d_i']_v},
	\]
...
\begin{itemize}
  \item $\bm{p\mid n,p\nmid d_1,p\mid d_2,p\mid d_3}$.
	...
	设
	\[
		\alertm{Q_{1,1}=(\alpha,\beta,\gamma)\in H_{1,1}(\BQ)},\quad
		\emphm{Q_{2,1}=(\alpha,A\beta,A\gamma)\in H_{2,1}(\BQ)}.
	\]
	\[
		\alertm{P_{1,p}=(1,0,u,v),\quad
		L_{1,1}(P_{1,p})=a_1n\alpha-d_3\gamma v+d_2\beta u,}
	\]
	\[
		\emphm{P_{2,p}=(1,0,Cu,Bv),\quad
		L_{2,1}(P_{2,p})=Aa_1n\alpha-Bd_3\gamma v+Cd_2\beta u,}
	\]
	\end{itemize}
	\onslide<+-> 
	\[
		\alertm{L_{1,1}(P_{1,p})}\emphm{L_{2,1}(P_{2,p})}=\tfrac{(A+B)(B+C)(C+A)}2\left(\frac{a_1n\alpha}{b+c}+\frac{d_2\beta u}{a+b}-\frac{d_3\gamma v}{a+c}\right)^2.\]
	\begin{itemize}
